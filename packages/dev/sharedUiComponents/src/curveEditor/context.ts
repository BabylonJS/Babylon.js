...
import type { KeyPointComponent } from "./keyPointComponent";

/**
 * Interface for a key point component that can be tracked by the context
 */
export interface IKeyPointComponent {
    props: {
        keyId: number;
        curve: ICurve;
    };
...
/**
 * Interface for a curve in the curve editor
 */
...
/**
 * Shared context interface for the curve editor
 * Both v1 and v2 implementations should conform to this interface
 */
export interface ICurveEditorContext {
    // State
    activeAnimations: Animation[];
    activeKeyPoints: Nullable<IKeyPointComponent[]>;
    mainKeyPoint: Nullable<IKeyPointComponent>;
    activeFrame: number;
...
/**
 * Shared Context class for the curve editor.
 * This is the main context used by both v1 and v2 curve editor implementations.
 */
export class Context implements ICurveEditorContext {
    title: string;
    ...
    // Setter methods for v2 React-style usage
    public setActiveAnimations(animations: Animation[]) {
        this.activeAnimations = animations;
        this.onActiveAnimationChanged.notifyObservers({});
    }

    public setActiveKeyPoints(keyPoints: Nullable<KeyPointComponent[]>) {
        this.activeKeyPoints = keyPoints;
        this.onActiveKeyPointChanged.notifyObservers();
    }

    public setActiveFrame(frame: number) {
        this.activeFrame = frame;
        this.onPlayheadMoved.notifyObservers(frame);
    }

    public setFromKey(frame: number) {
        this.fromKey = frame;
        this.onRangeUpdated.notifyObservers();
    }

    public setToKey(frame: number) {
        this.toKey = frame;
        this.onRangeUpdated.notifyObservers();
    }

    public setClipLength(length: number) {
        this.clipLength = length;
    }

    public setReferenceMaxFrame(frame: number) {
        this.referenceMaxFrame = frame;
    }

    public setFocusedInput(focused: boolean) {
