...
import { ApplyWhiteBalanceOptions } from "../../../src/PostProcesses/thinImageProcessingPostProcess";
import { ImageProcessingConfiguration } from "../../../src/Materials/imageProcessingConfiguration.pure";

// Regression coverage: ApplyWhiteBalanceOptions is applied both inside ThinImageProcessingPostProcess's own
...
describe("ApplyWhiteBalanceOptions", () => {
    ...
    it("applies tint and enables white balance", () => {
        const configuration = new ImageProcessingConfiguration();
        ApplyWhiteBalanceOptions(configuration, { tint: 25 });
        expect(configuration.whiteBalanceEnabled).toBe(true);
    ...
    it("applies both temperature and tint", () => {
        const configuration = new ImageProcessingConfiguration();
        ApplyWhiteBalanceOptions(configuration, { temperature: 4500, tint: -10 });
        expect(configuration.whiteBalanceEnabled).toBe(true);
