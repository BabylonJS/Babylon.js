...
export const _readTexturePixelsSync: IReadTextureEngineExtension["_readTexturePixelsSync"] = function (
    ...
    if (!fes._dummyFramebuffer) {
        ...
        fes._dummyFramebuffer = dummy;
    }
    gl.bindFramebuffer(gl.FRAMEBUFFER, fes._dummyFramebuffer);
    ...
    let readType = texture.type !== undefined ? _getWebGLTextureType(fes, texture.type) : gl.UNSIGNED_BYTE;
    ...
    gl.bindFramebuffer(gl.FRAMEBUFFER, fes._currentFramebuffer);

    return buffer;
};

export const _readTexturePixels = async function (
    engineState: IWebGLEnginePublic,
    texture: InternalTexture,
    width: number,
    height: number,
    ...
    return _readTexturePixelsSync(engineState, texture, width, height, faceIndex, level, buffer, flushRenderer, noDataConversion, x, y);
};

export const readTextureEngineExtension: IReadTextureEngineExtension = {
    _readTexturePixels,
    _readTexturePixelsSync,
};

export default readTextureEngineExtension;
