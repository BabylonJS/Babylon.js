...
export class IblShadowsRenderer {
    ...
    // private _colorCache = [
    //     new Color4(IblShadowsRenderer._DEPTH_CLEAR_VALUE, IblShadowsRenderer._DEPTH_CLEAR_VALUE, 0, 0),
    //     new Color4(-IblShadowsRenderer._MIN_DEPTH, IblShadowsRenderer._MAX_DEPTH, 0, 0),
    //     new Color4(0, 0, 0, 0),
    // ];
        ...
        if (this._useRenderPasses) {
            for (let i = 0; i < this._passCount + 1; ++i) {
                if (!this._renderPassIds[i]) {
                    this._renderPassIds[i] = this._engine.createRenderPassId(`IblShadowsRenderer - pass #${i}`);
                }
            }
        }
    }
