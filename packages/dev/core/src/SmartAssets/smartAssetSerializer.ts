 ...
 * not persisted — it is regenerated by `SmartAssetManager.loadAllAsync` from
 * each `AssetContainer` after load. Persisting it would risk staleness.
 */
...
export function SerializeSmartAssetMap(manager: SmartAssetManager, baseUrl?: string): ISerializedSmartAssetMap {
    ...
    for (const [key, registeredUrl] of Array.from(manager.getAll())) {
