...
// _____________________________ Base Diffuse Layer IBL _______________________________________
    ...
    // Purely empirical blend between diffuse and specular lobes when roughness gets very high.
        ...
        var baseSpecularEnvironmentLightRetro: vec3f = baseSpecularEnvironmentLight;
        let retroViewDirectionW: vec3f = normalize(reflect(-viewDirectionW, normalW));
        #ifdef ANISOTROPIC_BASE
            let baseGeoInfoRetro: geometryInfoAnisoOutParams = geometryInfoAniso(
                normalW,
                retroViewDirectionW,
                specular_roughness,
                geometricNormalW,
                vec3f(geometry_tangent.x, geometry_tangent.y, specular_roughness_anisotropy),
                TBN
            );
            ...
            baseSpecularEnvironmentLightRetro = mix(
                ...
                specularAlphaG * specularAlphaG * max(1.0f - baseGeoInfoRetro.anisotropy, 0.3f)
            );
        #else
    ...
    if (coat_weight > 0.0) {
        ...
        let modifiedFuzzRoughness: f32 = clamp(fuzz_roughness * fuzz_roughness * (1.0f + 0.5f * environmentFuzzBrdf.y), 0.0f, 1.0f);
        ...
        let fuzzIblFresnel: f32 = min(environmentFuzzBrdf.z * 2.0f, 1.0f);
        // Generate a normal on the fiber cylinder evenly spaced around the side of the cylinder that we can see.
        let viewTangent: vec2f = vec2f(dot(viewDirectionW, fuzzTangent), dot(viewDirectionW, fuzzBitangent));
        let centerAngle: f32 = atan2(viewTangent.y, viewTangent.x);
        for (var i: i32 = 0; i < i32(FUZZ_IBL_SAMPLES); i++) {
            // Sample only the visible ±90° of cylinder normals
            let angle: f32 = centerAngle + (f32(i) / f32(FUZZ_IBL_SAMPLES) - 0.5f + noise.x / f32(FUZZ_IBL_SAMPLES)) * 3.141592f;
            let fiberCylinderNormal: vec3f = normalize(cos(angle) * fuzzTangent + sin(angle) * fuzzBitangent);
            let fuzzReflectionCoords: vec3f = (uniforms.reflectionMatrix * vec4f(fiberCylinderNormal, 0.0f)).xyz;
        ...
        // If coat is present and either coat or base normal maps are present, we will sample the irradiance for the diffuse component of the fuzz layer.
        ...
        // Therefore, we'll blend fuzz toward the diffuse environment light based on the modified roughness.
    ...
    #ifdef RETROREFLECTION
        var slab_glossy_retro_ibl: vec3f = vec3f(0.0f);
        var slab_metal_retro_ibl: vec3f = vec3f(0.0f);
    #endif
    ...
    #ifdef RETROREFLECTION
        slab_glossy_retro_ibl = baseSpecularEnvironmentLightRetro * uniforms.vLightingIntensity.z;
    #endif
    ...
    #ifdef RETROREFLECTION
        slab_metal_retro_ibl = baseSpecularEnvironmentLightRetro * conductorIblFresnel * uniforms.vLightingIntensity.z;
    #endif
    ...
    var material_dielectric_gloss_ibl: vec3f = material_dielectric_base_ibl * (1.0f - dielectricIblFresnel) + slab_glossy_ibl * dielectricIblColoredFresnel;
    #ifdef RETROREFLECTION
        let material_dielectric_gloss_retro_ibl: vec3f =
            material_dielectric_base_ibl * (1.0f - dielectricIblFresnel) + slab_glossy_retro_ibl * dielectricIblColoredFresnel;
