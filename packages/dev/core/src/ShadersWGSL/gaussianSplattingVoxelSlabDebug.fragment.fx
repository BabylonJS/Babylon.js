varying vNormalizedPosition: vec3f;
varying vAlpha: f32;
varying vPatchPosition: vec2f;

uniform stepSize: f32;

@fragment
fn main(input: FragmentInputs) -> FragmentOutputs {
    let A: f32 = -dot(input.vPatchPosition, input.vPatchPosition);
    if (A < -input.vAlpha) { discard; }

    var normPos: vec3f = input.vNormalizedPosition.xyz;
    let chunkSize: f32 = uniforms.stepSize * f32(MAX_DRAW_BUFFERS);
    let positionInChunk: f32 = fract(normPos.z / chunkSize);
    let slab: f32 = floor(positionInChunk * f32(MAX_DRAW_BUFFERS)) / f32(MAX_DRAW_BUFFERS);

    if (normPos.x < 0.0 || normPos.y < 0.0 || normPos.z < 0.0 ||
        normPos.x > 1.0 || normPos.y > 1.0 || normPos.z > 1.0) {
        fragmentOutputs.color = vec4f(0.0, 0.0, 0.0, 0.0);
    } else {
        fragmentOutputs.color = vec4f(slab, 0.0, 0.0, 0.75);
    }
}
