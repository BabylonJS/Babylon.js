import { SnapshotRendering } from "../constants";
...
export class WebGPUSnapshotRendering {
    ...
    public set enabled(activate: boolean) {
        ...
        if (activate) {
            ...
            this._mode = SnapshotRendering.STANDARD; // need to reset to standard for the recording pass to avoid some code being bypassed
        }
    }
    ...
    public endRenderPass(currentRenderPass: GPURenderPassEncoder): boolean {
        ...
        if (this._mode === SnapshotRendering.FAST) {
