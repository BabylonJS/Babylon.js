...
void compute_box_normal(out vec3 normal, const in AABB3f box,
...
bool ray_triangle_intersection(out vec2 barys, out float distance,
                               const vec3 ray_orig, const vec3 ray_dir,
                               const mat3 vertices) {
...
// NOTE: VOXEL_MARCHING_NUM_MIPS refers to physical (rather than logical) mips,
// since mip 0 contains a 2x2x2 block of voxels
...
struct VoxelTraverseStack {
  ...
  // Each time we traverse a mip level in the graph, it pops 1 node, then pushes
  // up to 4 nodes on the stack Thus the worst case stack size is 3 nodes for
  // the first NUM_MIPS - 1 mips, and 4 for the last mip So, for a logical size
  // of 128, we have 7 mip levels, which translates to a stack size of 22
...
#if VOXEL_MARCH_DIAGNOSTIC_INFO_OPTION
bool hierarchical_march(
    Ray ray_vs, out VoxelMarchDiagnosticInfo voxel_march_diagnostic_info) {
#else
...
bool voxel_pos_in_bounds(ivec3 voxel_pos) {
  return all(greaterThanEqual(voxel_pos, ivec3(0))) &&
         all(lessThan(voxel_pos, ivec3(VOXEL_GRID_RESOLUTION)));
}

uint compute_packed_voxel_id(const ivec3 logical_voxel_coords) {
  return uint((logical_voxel_coords.x & 1) |
              ((logical_voxel_coords.y & 1) << 1) |
              ((logical_voxel_coords.z & 1) << 2));
}

// Convert from logical voxel coordinates to physical texel coordinate + access
// mask
ivec3 compute_physical_voxel_texcoords(const ivec3 logical_voxel_coords,
                                       out uint mask) {
