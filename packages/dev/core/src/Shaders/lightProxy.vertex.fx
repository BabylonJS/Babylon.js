...
void main(void) {
    ...
    vBatch = gl_InstanceID / CLUSTLIGHT_BATCH;
    ...
    tilePosition.y = (tilePosition.y + float(vBatch) * tileMaskResolution.y) / tileMaskResolution.z;

    // We don't care about depth and don't want it to be clipped so set Z to 0
    gl_Position = vec4(tilePosition.xy / tileMaskResolution.xy * 2.0 - 1.0, 0, 1);
}
