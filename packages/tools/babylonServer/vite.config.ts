...
function babylonServerPlugin(): Plugin {
    ...
    /**
     * Maps source directories to their UMD npm workspace name.
     * When a file under a source dir changes, the corresponding UMD package is rebuilt.
     */
    const sourceToPackage: Array<{ srcDir: string; pkg: string }> = [
        { srcDir: path.join(DEV_ROOT, "core/src"), pkg: "babylonjs" },
        { srcDir: path.join(DEV_ROOT, "gui/src"), pkg: "babylonjs-gui" },
        { srcDir: path.join(DEV_ROOT, "loaders/src"), pkg: "babylonjs-loaders" },
        { srcDir: path.join(DEV_ROOT, "serializers/src"), pkg: "babylonjs-serializers" },
        { srcDir: path.join(DEV_ROOT, "materials/src"), pkg: "babylonjs-materials" },
        { srcDir: path.join(DEV_ROOT, "postProcesses/src"), pkg: "babylonjs-post-process" },
        { srcDir: path.join(DEV_ROOT, "proceduralTextures/src"), pkg: "babylonjs-procedural-textures" },
        { srcDir: path.join(DEV_ROOT, "inspector/src"), pkg: "babylonjs-inspector-legacy" },
        { srcDir: path.join(DEV_ROOT, "inspector-v2/src"), pkg: "babylonjs-inspector" },
        { srcDir: path.join(DEV_ROOT, "addons/src"), pkg: "babylonjs-addons" },
        { srcDir: path.join(TOOLS_ROOT, "nodeEditor/src"), pkg: "babylonjs-node-editor" },
        { srcDir: path.join(TOOLS_ROOT, "nodeGeometryEditor/src"), pkg: "babylonjs-node-geometry-editor" },
        { srcDir: path.join(TOOLS_ROOT, "nodeRenderGraphEditor/src"), pkg: "babylonjs-node-render-graph-editor" },
        { srcDir: path.join(TOOLS_ROOT, "nodeParticleEditor/src"), pkg: "babylonjs-node-particle-editor" },
        { srcDir: path.join(TOOLS_ROOT, "guiEditor/src"), pkg: "babylonjs-gui-editor" },
        { srcDir: path.join(TOOLS_ROOT, "accessibility/src"), pkg: "babylonjs-accessibility" },
        { srcDir: path.join(TOOLS_ROOT, "ktx2Decoder/src"), pkg: "babylonjs-ktx2decoder" },
    ];
        ...
        configureServer(server) {
            ...
            if (enableWatch) {
                ...
                srcWatcher.on("change", (file) => {
                    ...
                    const entry = sourceToPackage.find((s) => file.startsWith(s.srcDir));
                    if (!entry) return;

                    console.log(`\x1b[90m  changed: ${path.relative(REPO_ROOT, file)}\x1b[0m`);

                    // Debounce — multiple files may change at once (e.g. save-all)
                    const existing = debounceTimers.get(entry.pkg);
                    if (existing) clearTimeout(existing);
                    debounceTimers.set(
                        entry.pkg,
                        setTimeout(() => {
                            debounceTimers.delete(entry.pkg);
                            rebuildPackage(entry.pkg, server);
                        }, DEBOUNCE_MS)
                    );
                });
                ...
                console.log(`\x1b[36m👀 Watching source files for ${sourceToPackage.length} packages (live reload: ${enableLiveReload ? "on" : "off"})\x1b[0m`);
            }
...
export default defineConfig((_env) => {
    const cdnPort = parseInt(process.env.CDN_PORT ?? "1337", 10);
