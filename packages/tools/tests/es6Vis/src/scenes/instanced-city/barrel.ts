/**
 * instanced-city — barrel import style.
 * Thin instances, SolidParticleSystem, mesh LOD levels.
 */
...
export function run(canvas: HTMLCanvasElement): void {
    ...
    const buildingTemplate = MeshBuilder.CreateBox("buildingTemplate", { width: 2, height: 1, depth: 2 }, scene);
    ...
    const trunk = MeshBuilder.CreateCylinder("trunk", { diameter: 0.3, height: 1.5 }, scene);
    const canopy = MeshBuilder.CreateSphere("canopy", { diameter: 2, segments: 8 }, scene);
    ...
    // LOD sphere
    const detailSphere = MeshBuilder.CreateSphere("lodSphere", { diameter: 4, segments: 64 }, scene);
    ...
    const lodMid = MeshBuilder.CreateSphere("lodMid", { diameter: 4, segments: 16 }, scene);
    lodMid.material = lodMat;
    const lodLow = MeshBuilder.CreateSphere("lodLow", { diameter: 4, segments: 8 }, scene);
