/**
 * pbr-node-material - deep import style (legacy, non-pure paths).
 */
...
export function run(canvas: HTMLCanvasElement): void {
    ...
    const envTex = CubeTexture.CreateFromPrefilteredData("https://assets.babylonjs.com/environments/environmentSpecular.env", scene);
    ...
    const nodeMat = NodeMaterial.CreateDefault("nodeMat", scene);
