/**
 * particle-fountain — pure import style.
 */
...
export function run(canvas: HTMLCanvasElement): void {
    ...
    const base = CreateCylinder("base", { diameterTop: 1, diameterBottom: 1.5, height: 1 }, scene);
    base.position.y = 0.5;
    ...
    const emitter = CreateSphere("emitter", { diameter: 0.4, segments: 16 }, scene);
    ...
    for (let i = 0; i < 3; i++) {
        const orb = CreateSphere(`orb${i}`, { diameter: 0.5, segments: 16 }, scene);
