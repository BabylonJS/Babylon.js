/**
 * postprocess-pipeline — pure import style.
 */
...
export function run(canvas: HTMLCanvasElement): void {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        switch (type) {
            case 0:
                mesh = CreateSphere(`sphere${i}`, { diameter: 1 + Math.random() }, scene);
                break;
            case 1:
                mesh = CreateBox(`box${i}`, { size: 0.8 + Math.random() * 0.8 }, scene);
                break;
            default:
                mesh = CreateCylinder(`cyl${i}`, { diameter: 0.5 + Math.random() * 0.5, height: 1 + Math.random() * 2 }, scene);
                break;
        }
    ...
    for (let i = 0; i < 5; i++) {
        const light = CreateSphere(`emissive${i}`, { diameter: 0.5 }, scene);
