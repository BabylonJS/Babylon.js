/**
 * textured-ground — barrel import style.
 * PBR material, procedural noise texture, torus knot, shadows.
 */
import {
    Engine,
    Scene,
...
export function run(canvas: HTMLCanvasElement): void {
    ...
    const knot = MeshBuilder.CreateTorusKnot("knot", { radius: 1.5, tube: 0.5, radialSegments: 128, tubularSegments: 64 }, scene);
    knot.position.y = 2.5;
    ...
    noiseTex.animationSpeedFactor = 0; // Frozen for deterministic screenshots
    ...
    const cyl = MeshBuilder.CreateCylinder("cyl", { diameter: 1, height: 3 }, scene);
