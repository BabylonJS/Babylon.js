...
export function parseColor(color: string | null | undefined): Nullable<IColor4Like> {
    if (!color) {
        return null;
    }
...
function colorToHex(color: IColor4Like): string {
...
export function coerceNumericAttribute(value: string | null): Nullable<number> {
    return value == null ? null : Number(value);
}

export function coerceCameraOrbitOrTarget(value: string | null): Nullable<[number, number, number]> {
    if (!value) {
        return null;
    }
...
export function coerceToneMapping(value: string | null): Nullable<ToneMapping> {
...
export function coerceShadowQuality(value: string | null): Nullable<ShadowQuality> {
...
export interface ViewerElementBase {
 ...
 */
export abstract class ViewerElementBase<ViewerClass extends IViewer = IViewer, Options extends ViewerBaseOptions = ViewerBaseOptions> extends LitElement {
     ...
     */
    protected constructor(protected readonly _options: Options = {} as Options) {
        super();
    }
    ...
    private readonly _propertyBindings = [
        this._createPropertyBinding(
            "clearColor",
            (viewer) => viewer.onClearColorChanged,
            (viewer) => (viewer.clearColor = this.clearColor ?? { r: 0, g: 0, b: 0, a: 0 }),
            (viewer) => (this.clearColor = viewer.clearColor)
        ),
        this._createPropertyBinding(
            "skyboxBlur",
            (viewer) => viewer.onEnvironmentConfigurationChanged,
            (viewer) => (viewer.environmentConfig = { blur: this.skyboxBlur ?? viewer.environmentConfig.blur }),
            (viewer) => (this.skyboxBlur = viewer.environmentConfig.blur)
        ),
        this._createPropertyBinding(
            "environmentIntensity",
            (viewer) => viewer.onEnvironmentConfigurationChanged,
            (viewer) => (viewer.environmentConfig = { intensity: this.environmentIntensity ?? viewer.environmentConfig.intensity }),
            (viewer) => (this.environmentIntensity = viewer.environmentConfig.intensity)
        ),
        this._createPropertyBinding(
            "environmentRotation",
            (viewer) => viewer.onEnvironmentConfigurationChanged,
            (viewer) => (viewer.environmentConfig = { rotation: this.environmentRotation ?? viewer.environmentConfig.rotation }),
            (viewer) => (this.environmentRotation = viewer.environmentConfig.rotation)
        ),
        this._createPropertyBinding(
            "toneMapping",
            (viewer) => viewer.onPostProcessingChanged,
            (viewer) => {
                if (this.toneMapping) {
        ...
        this._createPropertyBinding(
            "contrast",
            (viewer) => viewer.onPostProcessingChanged,
            (viewer) => (viewer.postProcessing = { contrast: this.contrast ?? undefined }),
            (viewer) => (this.contrast = viewer.postProcessing.contrast)
        ),
        this._createPropertyBinding(
            "exposure",
            (viewer) => viewer.onPostProcessingChanged,
            (viewer) => (viewer.postProcessing = { exposure: this.exposure ?? undefined }),
            (viewer) => (this.exposure = viewer.postProcessing.exposure)
        ),
        this._createPropertyBinding(
            "ssao",
            (viewer) => viewer.onPostProcessingChanged,
            (viewer) => (viewer.postProcessing = { ssao: this.ssao ?? undefined }),
            (viewer) => (this.ssao = viewer.postProcessing.ssao)
        ),
        this._createPropertyBinding(
            "cameraAutoOrbit",
            (viewer) => viewer.onCameraAutoOrbitChanged,
            (viewer) => (viewer.cameraAutoOrbit = { enabled: this.cameraAutoOrbit }),
            (viewer) => (this.cameraAutoOrbit = viewer.cameraAutoOrbit.enabled)
        ),
        this._createPropertyBinding(
            "cameraAutoOrbitSpeed",
            (viewer) => viewer.onCameraAutoOrbitChanged,
            (viewer) => (viewer.cameraAutoOrbit = { speed: this.cameraAutoOrbitSpeed ?? undefined }),
            (viewer) => (this.cameraAutoOrbitSpeed = viewer.cameraAutoOrbit.speed)
        ),
        this._createPropertyBinding(
            "cameraAutoOrbitDelay",
            (viewer) => viewer.onCameraAutoOrbitChanged,
            (viewer) => (viewer.cameraAutoOrbit = { delay: this.cameraAutoOrbitDelay ?? undefined }),
            (viewer) => (this.cameraAutoOrbitDelay = viewer.cameraAutoOrbit.delay)
        ),
        this._createPropertyBinding(
            "animationSpeed",
            (viewer) => viewer.onAnimationSpeedChanged,
            (viewer) => (viewer.animationSpeed = this.animationSpeed),
            (viewer) => {
        ...
        this._createPropertyBinding(
            "selectedAnimation",
            (viewer) => viewer.onSelectedAnimationChanged,
            (viewer) => (viewer.selectedAnimation = this.selectedAnimation ?? viewer.selectedAnimation),
            (viewer) => (this.selectedAnimation = viewer.selectedAnimation)
        ),
        this._createPropertyBinding(
            "selectedMaterialVariant",
            (viewer) => viewer.onSelectedMaterialVariantChanged,
            (viewer) => (viewer.selectedMaterialVariant = this.selectedMaterialVariant ?? viewer.selectedMaterialVariant ?? ""),
            (viewer) => (this.selectedMaterialVariant = viewer.selectedMaterialVariant)
        ),
        this._createPropertyBinding(
            "hotSpots",
            (viewer) => viewer.onHotSpotsChanged,
            (viewer) => (viewer.hotSpots = this.hotSpots ?? viewer.hotSpots),
            (viewer) => (this.hotSpots = viewer.hotSpots)
        ),
        this._createPropertyBinding(
            "camerasAsHotSpots",
            (viewer) => viewer.onCamerasAsHotSpotsChanged,
            (viewer) => (viewer.camerasAsHotSpots = this.camerasAsHotSpots ?? viewer.camerasAsHotSpots),
            (viewer) => (this.camerasAsHotSpots = viewer.camerasAsHotSpots)
        ),
    ] as const;
    ...
    private _createPropertyBinding(
        ...
        const tryUpdateViewer = (viewer: ViewerClass) => {
            try {
                updateViewer(viewer);
            } catch (error) {
                Logger.Error(error);
        ...
        return {
            property,
            onInitialized: (viewer: ViewerClass) => {
