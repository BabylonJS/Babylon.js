...
Write the visual test scene as a normal Babylon Playground `createScene` function and save it to a temporary `.js` file. Working in a file avoids painful string escaping and makes iteration easier.
...
Use `.github/scripts/visual-testing/save-snippet.js` to post the temp playground file directly to the Babylon snippet server. This is usually more reliable than manually saving through the Playground UI.
...
Remove temporary Playground files, remove any temporary files generated by `playwright-cli` for local inspection (typically under `.playwright-cli/`), and stop any long-running servers you started specifically for the task.
...
| `referenceImage` | No | `title` | PNG filename in `ReferenceImages/` |
...
| Scene content | Suggested `renderCount` | Reason |
| --- | --- | --- |
| Static geometry and materials | `1` | Ready on the first frame |
| Shadows and reflections | `1` | Usually ready immediately |
| Procedural textures | `5` to `10` | May need a few frames to settle |
| Async asset loading | `50+` | Needs time for network, parse, and render |
| Animations | Target frame count | Capture the intended pose |
| Particle systems | `60` to `120` | Needs time to emit and spread |
| Physics simulations | `60` to `120` | Needs time to settle or interact |
| Noise or random effects | `60` to `120` | Needs time to become visually stable |

Rules of thumb:

- If the scene includes time-dependent behavior, `renderCount` should be greater than `1`.
- If the result is noisy or nondeterministic, use a higher `renderCount` and revisit `errorRatio` only if necessary.
- When in doubt, `60` is often safer than a value that produces flaky captures.
...
WebGPU runs require a real Chrome installation rather than Chromium. If ChromeWebGPU is unavailable locally, the WebGPU project may be skipped locally, but the test still needs to pass in CI.
...
- Transparency and alpha-heavy scenes can produce larger diffs.
