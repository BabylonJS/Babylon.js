 ...
 */
function simulateEvent(cameraInput, event) {
    var pointerInfo = {};
    switch (event.type) {
        case "pointerdown":
            pointerInfo = { type: BABYLON.PointerEventTypes.POINTERDOWN, event: event };
            // Cast "camera" to <any> to relax "private" classification.
            cameraInput._pointerInput(pointerInfo, undefined);
            break;
        case "pointerup":
            pointerInfo = { type: BABYLON.PointerEventTypes.POINTERUP, event: event };
            // Cast "camera" to <any> to relax "private" classification.
            cameraInput._pointerInput(pointerInfo, undefined);
            break;
        case "pointermove":
            pointerInfo = { type: BABYLON.PointerEventTypes.POINTERMOVE, event: event };
            // Cast "camera" to <any> to relax "private" classification.
            cameraInput._pointerInput(pointerInfo, undefined);
            break;
        case "blur":
            // Cast "camera" to <any> to relax "private" classification.
            cameraInput._onLostFocus();
            break;
        case "POINTERDOUBLETAP":
            // Not a real DOM event. Just a shortcut to trigger
            // PointerEventTypes.POINTERMOVE on the Input class.
            pointerInfo = { type: BABYLON.PointerEventTypes.POINTERDOUBLETAP, event: event };
            // Cast "camera" to <any> to relax "private" classification.
            cameraInput._pointerInput(pointerInfo, undefined);
            break;
        default:
...
function StubCameraInput() {
    ...
    var cameraInput = new BABYLON.ArcRotateCameraPointersInput();
    /**
     * Reset all counters.
     */
    cameraInput.reset = function () {
     ...
     */
    cameraInput.onTouch = function (point, offsetX, offsetY) {
        cameraInput.countOnTouch++;
        cameraInput.lastOnTouch = { point: point, offsetX: offsetX, offsetY: offsetY };
    };
    cameraInput.onDoubleTap = function (type) {
        cameraInput.countOnDoubleTap++;
        cameraInput.lastOnDoubleTap = type;
    };
    cameraInput.onMultiTouch = function (pointA, pointB, previousPinchSquaredDistance, pinchSquaredDistance, previousMultiTouchPanPosition, multiTouchPanPosition) {
        cameraInput.countOnMultiTouch++;
        cameraInput.lastOnMultiTouch = {
            pointA: pointA,
            pointB: pointB,
            previousPinchSquaredDistance: previousPinchSquaredDistance,
            pinchSquaredDistance: pinchSquaredDistance,
            previousMultiTouchPanPosition: previousMultiTouchPanPosition,
            multiTouchPanPosition: multiTouchPanPosition,
        };
    };
    cameraInput.onButtonDown = function (evt) {
        cameraInput.countOnButtonDown++;
        var buttonCount = cameraInput.pointB !== null ? 2 : 1;
        cameraInput.lastOnButtonDown = { evt: evt, buttonCount: buttonCount };
    };
    cameraInput.onButtonUp = function (evt) {
        cameraInput.countOnButtonUp++;
        var buttonCount = cameraInput.pointA !== null ? 1 : 0;
        cameraInput.lastOnButtonUp = { evt: evt, buttonCount: buttonCount };
    };
    cameraInput.onContextMenu = function (evt) {
        cameraInput.countOnContextMenu++;
        cameraInput.lastOnContextMenu = evt;
    };
    cameraInput.onLostFocus = function () {
...
describe("BaseCameraPointersInput", function () {
    ...
    describe("one button drag", function () {
        it('calls "onTouch" method', function () {
            var event = eventTemplate(this._canvas);
            // Button down.
...
describe("ArcRotateCameraInput", function () {
    ...
    var ValChange;
    (function (ValChange) {
        ValChange[ValChange["Increase"] = 0] = "Increase";
        ValChange[ValChange["Same"] = 1] = "Same";
        ValChange[ValChange["Decrease"] = 2] = "Decrease";
        ValChange[ValChange["DontCare"] = 3] = "DontCare";
    })(ValChange || (ValChange = {}));
    var interestingValues = ["inertialPanningX", "inertialPanningY", "inertialAlphaOffset", "inertialBetaOffset", "inertialRadiusOffset"];
    function resetCameraPos(camera, cameraCachePos) {
        camera.alpha = 10;
        camera.beta = 20;
        camera.radius = 30;
        ...
        interestingValues.forEach(function (key) {
            cameraCachePos[key] = camera[key];
        });
    }
    function verifyChanges(camera, cameraCachePos, toCheck) {
        var result = true;
        interestingValues.forEach(function (key) {
            if (toCheck[key] === undefined) {
                toCheck[key] = ValChange.Same;
            }
            var r = toCheck[key] === ValChange.DontCare || (toCheck[key] === ValChange.Decrease && camera[key] < cameraCachePos[key]) || (toCheck[key] === ValChange.Same && camera[key] === cameraCachePos[key]) || (toCheck[key] === ValChange.Increase && camera[key] > cameraCachePos[key]);
            if (!r) {
                console.log("Incorrect value for ".concat(key, ", previous: ").concat(cameraCachePos[key], ", current: ").concat(camera[key]));
            }
            result = result && r;
            cameraCachePos[key] = camera[key];
        });
