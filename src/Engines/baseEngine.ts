...
import { _DepthCullingState } from '../States/depthCullingState';
import { _StencilState } from '../States/stencilState';
import { _AlphaState } from '../States/alphaCullingState';
...
export class BaseEngine {
     ...
     */
    public onContextLostObservable = new Observable<BaseEngine>();
    /**
     * Observable signaled when a context restored event is raised
     */
    public onContextRestoredObservable = new Observable<BaseEngine>();
    ...
    constructor(canvasOrContext: Nullable<HTMLCanvasElement | WebGLRenderingContext>, antialias?: boolean, options?: EngineOptions, adaptToDeviceRatio: boolean = false) {
        ...
        if ((<HTMLCanvasElement>canvasOrContext).getContext) {
            ...
            if (navigator && navigator.userAgent) {
                ...
                for (var exception of BaseEngine.ExceptionList) {
        ...
        console.log(`Babylon.js v${BaseEngine.Version} - ${this.description}`);
    }
     ...
     */
    public createEffect(baseName: any, attributesNamesOrOptions: string[] | EffectCreationOptions, uniformsNamesOrEngine: string[] | BaseEngine, samplers?: string[], defines?: string, fallbacks?: EffectFallbacks,
    ...
    private _compileShader(source: string, type: string, defines: Nullable<string>, shaderVersion: string): WebGLShader {
        return this._compileRawShader(BaseEngine._ConcatenateShader(source, defines, shaderVersion), type);
    }
    ...
    public createDynamicTexture(width: number, height: number, generateMipMaps: boolean, samplingMode: number): InternalTexture {
        ...
        if (generateMipMaps) {
            width = this.needPOTTextures ? BaseEngine.GetExponentOfTwo(width, this._caps.maxTextureSize) : width;
            height = this.needPOTTextures ? BaseEngine.GetExponentOfTwo(height, this._caps.maxTextureSize) : height;
        }
    ...
    private _prepareWebGLTexture(texture: InternalTexture, scene: Nullable<ISceneLike>, width: number, height: number, invertY: boolean, noMipmap: boolean, isCompressed: boolean,
        ...
        var potWidth = Math.min(maxTextureSize, this.needPOTTextures ? BaseEngine.GetExponentOfTwo(width, maxTextureSize) : width);
        var potHeight = Math.min(maxTextureSize, this.needPOTTextures ? BaseEngine.GetExponentOfTwo(height, maxTextureSize) : height);
    ...
    public static NearestPOT(x: number): number {
        var c = BaseEngine.CeilingPOT(x);
        var f = BaseEngine.FloorPOT(x);
        return (c - x) > (x - f) ? f : c;
    }
    ...
    public static GetExponentOfTwo(value: number, max: number, mode = Constants.SCALEMODE_NEAREST): number {
        ...
        switch (mode) {
            case Constants.SCALEMODE_FLOOR:
                pot = BaseEngine.FloorPOT(value);
                break;
            case Constants.SCALEMODE_NEAREST:
                pot = BaseEngine.NearestPOT(value);
                break;
            case Constants.SCALEMODE_CEILING:
            default:
                pot = BaseEngine.CeilingPOT(value);
                break;
        }
