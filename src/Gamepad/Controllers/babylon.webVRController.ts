namespace BABYLON {
  export abstract class WebVRController extends PoseEnabledController {
    ...
    constructor(vrGamepad: any) {
      super(vrGamepad);
      this._buttons = new Array<ExtendedGamepadButton>(
        vrGamepad.buttons.length
      );
    ...
    public update() {
      ...
      for (var index = 0; index < this._buttons.length; index++) {
        this._setButtonValue(
          this.browserGamepad.buttons[index],
          this._buttons[index],
          index
        );
      }
    ...
    public abstract initControllerMesh(
      scene: Scene,
    ...
    private _checkChanges(
      ...
      this._changes.changed =
        this._changes.pressChanged ||
        this._changes.touchChanged ||
        this._changes.valueChanged;
