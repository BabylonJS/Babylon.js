  ...
  export class PoseEnabledControllerHelper {
    public static InitiateController(vrGamepad: any) {
      ...
      if (vrGamepad.id.indexOf("Oculus Touch") !== -1) {
        ...
        vrGamepad.id.indexOf(WindowsMotionController.GAMEPAD_ID_PREFIX) === 0
      ) {
        ...
        vrGamepad.id.indexOf(GearVRController.GAMEPAD_ID_PREFIX) === 0
      ) {
    ...
    updateFromDevice(poseData: DevicePose) {
      if (poseData) {
        ...
        if (poseData.position) {
          ...
          this._deviceRoomPosition.scaleToRef(
