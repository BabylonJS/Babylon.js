  ...
  export class WindowsMotionController extends WebVRController {
    ...
    public update() {
      ...
      if (this._loadedMeshInfo) {
        if (this.browserGamepad.axes) {
          ...
          for (
            let axis = 0;
            axis < this._mapping.axisMeshNames.length;
            axis++
          ) {
    ...
    protected handleButtonChange(
      ...
      let observable = (<any>this)[
        (<any>this._mapping.buttonObservableNames)[buttonName]
      ];
      if (observable) {
    ...
    public initControllerMesh(
      ...
      if (SceneLoader.IsPluginForExtensionAvailable(".glb")) {
        ...
        Tools.Warn(
          "You need to reference GLTF loader to load Windows Motion Controllers model. Falling back to generic models"
        );
      ...
      SceneLoader.ImportMesh(
        ...
        (scene: Scene, message: string) => {
          Tools.Log(message);
          Tools.Warn(
            "Failed to retrieve controller model from the remote server: " +
              path +
              filename
          );
          if (!forceDefault) {
    ...
    private createMeshInfo(rootNode: AbstractMesh): LoadedMeshInfo {
      ...
      for (i = 0; i < this._mapping.buttons.length; i++) {
        var buttonMeshName = (<any>this._mapping.buttonMeshNames)[
          this._mapping.buttons[i]
        ];
        if (!buttonMeshName) {
          Tools.Log(
      ...
      // Pointing Ray
      loadedMeshInfo.pointingPoseNode = getChildByName(
        rootNode,
        this._mapping.pointingPoseMeshName
      );
      if (!loadedMeshInfo.pointingPoseNode) {
        Tools.Warn(
          "Missing pointing pose mesh with name: " +
            this._mapping.pointingPoseMeshName
        );
      }
