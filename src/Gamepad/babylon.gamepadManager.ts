    ...
    constructor(private _scene?: Scene) {
      ...
      this._onGamepadDisconnectedEvent = evt => {
        ...
        for (var i in this._babylonGamepads) {
          if (this._babylonGamepads[i].index === gamepad.index) {
            ...
            this.onGamepadDisconnectedObservable.notifyObservers(
              disconnectedGamepad
            );
            break;
          }
      ...
      if (this._gamepadSupport) {
        ...
        if (this._gamepadEventSupported) {
          ...
          window.addEventListener(
            "gamepaddisconnected",
            this._onGamepadDisconnectedEvent,
            false
          );
        } else {
          this._startMonitoringGamepads();
        ...
        if (gamepads[i]) {
          if (!this._babylonGamepads[gamepads[i].index]) {
            ...
            if (!this._babylonGamepads[i].isConnected) {
              ...
              this.onGamepadConnectedObservable.notifyObservers(
                this._babylonGamepads[i]
              );
            }
          }
        }
