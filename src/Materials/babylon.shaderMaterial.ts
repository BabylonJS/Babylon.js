  ...
  export class ShaderMaterial extends Material {
    ...
    public isReady(mesh?: AbstractMesh, useInstances?: boolean): boolean {
      ...
      if (
        ...
        defines.push(
          "#define BonesPerMesh " + (mesh.skeleton.bones.length + 1)
        );
    ...
    public bind(world: Matrix, mesh?: Mesh): void {
      ...
      if (this._effect && this.getScene().getCachedMaterial() !== this) {
        ...
        if (this._options.uniforms.indexOf("projection") !== -1) {
          this._effect.setMatrix(
            "projection",
            this.getScene().getProjectionMatrix()
          );
        }
    ...
    public serialize(): any {
      ...
      for (name in this._textureArrays) {
        ...
        var array = this._textureArrays[name];
        for (var index = 0; index < array.length; index++) {
          serializationObject.textureArrays[name].push(
    ...
    public static Parse(
      ...
      for (name in source.textures) {
        material.setTexture(name, <Texture>Texture.Parse(
      ...
      for (name in source.textureArrays) {
        ...
        for (var index = 0; index < array.length; index++) {
          textureArray.push(<Texture>Texture.Parse(
            array[index],
            scene,
            rootUrl
          ));
