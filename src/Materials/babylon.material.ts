namespace BABYLON {
  ...
  export class Material implements IAnimatable {
    ...
    @serialize() public checkReadyOnEveryCall = false;

    @serialize() public checkReadyOnlyOnce = false;

    @serialize() public state = "";

    @serialize() public alpha = 1.0;

    @serialize("backFaceCulling") protected _backFaceCulling = true;
    ...
    @serialize() public sideOrientation: number;
    ...
    @serialize("alphaMode") private _alphaMode: number = Engine.ALPHA_COMBINE;
    ...
    @serialize() private _needDepthPrePass = false;
    ...
    @serialize() public disableDepthWrite = false;

    @serialize() public forceDepthWrite = false;

    @serialize() public separateCullingPass = false;

    @serialize("fogEnabled") private _fogEnabled = true;
    ...
    @serialize() public pointSize = 1.0;

    @serialize() public zOffset = 0;
    ...
    public bindView(effect: Effect): void {
      if (!this._useUBO) {
        ...
        this.bindSceneUniformBuffer(
          effect,
          this.getScene().getSceneUniformBuffer()
        );
      }
    }
    ...
    public forceCompilation(
      ...
      options?: Partial<{ clipPlane: boolean }>
    ): void {
      let localOptions = {
        clipPlane: false,
        ...options
            ...
            if (this.storeEffectOnSubMeshes) {
              for (var subMesh of mesh.subMeshes) {
                ...
                if (forceDisposeEffect && subMesh._materialEffect) {
                  this._scene
                    .getEngine()
                    ._releaseEffect(subMesh._materialEffect);
                }
              }
            } else {
    ...
    public static ParseMultiMaterial(
      ...
      for (
        var matIndex = 0;
        matIndex < parsedMultiMaterial.materials.length;
        matIndex++
      ) {
