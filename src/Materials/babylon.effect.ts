namespace BABYLON {
  export class EffectFallbacks {
    ...
    public reduce(currentDefines: string): string {
      ...
      if (
        this._mesh &&
        this._mesh.computeBonesUsingShaders &&
        this._mesh.numBoneInfluencers > 0
      ) {
        this._mesh.computeBonesUsingShaders = false;
        currentDefines = currentDefines.replace(
          "#define NUM_BONE_INFLUENCERS " + this._mesh.numBoneInfluencers,
        ...
        for (var index = 0; index < scene.meshes.length; index++) {
          ...
          if (
            otherMesh.material === this._mesh.material &&
  ...
  export class Effect {
    ...
    constructor(
      ...
      fallbacks: Nullable<EffectFallbacks> = null,
      ...
      if ((<EffectCreationOptions>attributesNamesOrOptions).attributes) {
        ...
        this._uniformsNames = (<string[]>uniformsNamesOrEngine).concat(
          <string[]>samplers
        );
      ...
      this._loadVertexShader(vertexSource, vertexCode => {
        this._processIncludes(vertexCode, vertexCodeWithIncludes => {
          this._processShaderConversion(
            vertexCodeWithIncludes,
            false,
            migratedVertexCode => {
              this._loadFragmentShader(fragmentSource, fragmentCode => {
                this._processIncludes(
                  fragmentCode,
                  fragmentCodeWithIncludes => {
                    this._processShaderConversion(
                      fragmentCodeWithIncludes,
                      true,
    ...
    private _dumpShadersSource(
      ...
      var shaderVersion =
        this._engine.webGLVersion > 1 ? "#version 300 es\n" : "";
      ...
      var formattedVertexCode =
        "\n1\t" +
        vertexCode.replace(regex, function() {
          return "\n" + i++ + "\t";
        });
      i = 2;
      var formattedFragmentCode =
        "\n1\t" +
        fragmentCode.replace(regex, function() {
          return "\n" + i++ + "\t";
        });
      ...
      if (this.name.vertexElement) {
        Tools.Error(
          "Vertex shader: " + this.name.vertexElement + formattedVertexCode
        );
        Tools.Error(
          "Fragment shader: " +
            this.name.fragmentElement +
            formattedFragmentCode
        );
      } else if (this.name.vertex) {
        Tools.Error("Vertex shader: " + this.name.vertex + formattedVertexCode);
        Tools.Error(
          "Fragment shader: " + this.name.fragment + formattedFragmentCode
        );
      } else {
    ...
    private _processShaderConversion(
      ...
      var hasDrawBuffersExtension =
        preparedSourceCode.search(
    ...
    private _processIncludes(
      ...
      while (match != null) {
        ...
        if (Effect.IncludesShadersStore[includeFile]) {
              ...
              for (var i = minIndex; i < maxIndex; i++) {
                if (!this._engine.supportsUniformBuffers) {
                  ...
                  sourceIncludeContent = sourceIncludeContent.replace(
                    /light\{X\}.(\w*)/g,
                    (str: string, p1: string) => {
                      return p1 + "{X}";
                    }
                  );
                }
                includeContent +=
                  sourceIncludeContent.replace(/\{X\}/g, i.toString()) + "\n";
              }
            } else {
              if (!this._engine.supportsUniformBuffers) {
                // Ubo replacement
          ...
          var includeShaderUrl =
            Engine.ShadersRepository + "ShadersInclude/" + includeFile + ".fx";
    ...
    public getSpecificUniformLocations(
      names: string[]
    ...
    public _prepareEffect() {
      ...
      try {
        ...
        if (
          ...
          this._program = engine.createRawShaderProgram(
            this._vertexSourceCodeOverride,
            this._fragmentSourceCodeOverride,
            undefined,
        ...
        this._program.__SPECTOR_rebuildProgram = this._rebuildProgram.bind(
          this
        );
    ...
    public setTexture(channel: string, texture: Nullable<BaseTexture>): void {
      this._engine.setTexture(
        this._samplers.indexOf(channel),
        this.getUniform(channel),
        texture
      );
    }
    ...
    public setTextureFromPostProcess(
      ...
      this._engine.setTextureFromPostProcess(
        this._samplers.indexOf(channel),
        postProcess
      );
    }
    ...
    public setVector2(uniformName: string, vector2: Vector2): Effect {
      if (this._cacheFloat2(uniformName, vector2.x, vector2.y)) {
        this._engine.setFloat2(
          this.getUniform(uniformName),
          vector2.x,
          vector2.y
        );
      }
