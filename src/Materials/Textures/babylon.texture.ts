namespace BABYLON {
  export class Texture extends BaseTexture {
    ...
    // Members
    @serialize() public url: Nullable<string>;

    @serialize() public uOffset = 0;

    @serialize() public vOffset = 0;

    @serialize() public uScale = 1.0;

    @serialize() public vScale = 1.0;

    @serialize() public uAng = 0;

    @serialize() public vAng = 0;

    @serialize() public wAng = 0;
    ...
    public getReflectionTextureMatrix(): Matrix {
      ...
      if (
        ...
        if (this.coordinatesMode === Texture.PROJECTION_MODE) {
          if (
            this._cachedProjectionMatrixId ===
    ...
    public clone(): Texture {
      return SerializationHelper.Clone(() => {
        return new Texture(
          this._texture ? this._texture.url : null,
          this.getScene(),
          this._noMipmap,
          this._invertY,
          this._samplingMode
    ...
    public serialize(): any {
      ...
      if (
        typeof this._buffer === "string" &&
        this._buffer.substr(0, 5) === "data:"
      ) {
        serializationObject.base64String = this._buffer;
        serializationObject.name = serializationObject.name.replace(
          "data:",
          ""
        );
      }
    ...
    public static Parse(
      ...
      if (parsedTexture.customType) {
        ...
        var parsedCustomTexture: any = customTexture.Parse(
          parsedTexture,
          scene,
          rootUrl
        );
        if (
          ...
          parsedCustomTexture.updateSamplingMode &&
          parsedCustomTexture._samplingMode
        ) {
          if (
            parsedCustomTexture._samplingMode !== parsedTexture.samplingMode
          ) {
      ...
      var texture = SerializationHelper.Parse(
        () => {
          ...
          if (parsedTexture.mirrorPlane) {
            var mirrorTexture = new MirrorTexture(
              parsedTexture.name,
            ...
            mirrorTexture.mirrorPlane = Plane.FromArray(
              parsedTexture.mirrorPlane
            );

            return mirrorTexture;
          } else if (parsedTexture.isRenderTarget) {
            var renderTargetTexture = new RenderTargetTexture(
              parsedTexture.name,
              parsedTexture.renderTargetSize,
              scene,
              generateMipMaps
