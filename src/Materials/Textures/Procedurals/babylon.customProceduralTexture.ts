      ...
      let noConfigFile = () => {
        Tools.Log(
          "No config file found in " +
            jsonUrl +
            " trying to use ShadersStore or DOM element"
        );
        try {
      ...
      for (var i = 0; i < this._config.sampler2Ds.length; i++) {
        this.setTexture(
          ...
          new Texture(
            this._texturePath +
              "/" +
              this._config.sampler2Ds[i].textureRelativeUrl,
            this.getScene()
          )
        );
      }
        ...
        for (var j = 0; j < this._config.uniforms.length; j++) {
          ...
          switch (uniform.type) {
            ...
            case "color3":
              this.setColor3(
                uniform.name,
                new Color3(uniform.r, uniform.g, uniform.b)
              );
              break;
            case "color4":
              this.setColor4(
                uniform.name,
                new Color4(uniform.r, uniform.g, uniform.b, uniform.a)
              );
              break;
            case "vector2":
            ...
            case "vector3":
              this.setVector3(
                uniform.name,
                new Vector3(uniform.x, uniform.y, uniform.z)
              );
              break;
          }
        }
