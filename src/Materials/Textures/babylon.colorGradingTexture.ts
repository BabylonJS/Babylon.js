namespace BABYLON {
  ...
  export class ColorGradingTexture extends BaseTexture {
      ...
      var callback = (text: string | ArrayBuffer) => {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (size != 0) {
            ...
            var pixelStorageIndex =
              (pixelIndexW +
                pixelIndexSlice * size +
                pixelIndexH * size * size) *
              4;
    ...
    public clone(): ColorGradingTexture {
      var newTexture = new ColorGradingTexture(
        this.url,
        <Scene>this.getScene()
      );
