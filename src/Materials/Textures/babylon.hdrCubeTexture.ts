  ...
  export class HDRCubeTexture extends BaseTexture {
    ...
    constructor(
      ...
      if (size) {
        ...
        this._usePMREMGenerator =
          caps.textureLOD && caps.textureFloat && !this._useInGammaSpace;
      }
    ...
    private loadBabylonTexture() {
      ...
      var mipmapGenerator =
        ...
        scene.getEngine().getCaps().textureFloat
          ? (data: ArrayBufferView[]): Array<Array<Float32Array>> => {
              ...
              for (var level = 0; level < mipLevels; level++) {
                ...
                for (var faceIndex = 0; faceIndex < 6; faceIndex++) {
                  var faceData = floatArrayView.subarray(
                    startIndex,
                    startIndex + faceSize
                  );
      ...
      var callback = (buffer: ArrayBuffer) => {
        ...
        sphericalPolynomial.x.copyFromFloats(
          floatArrayView[2],
          floatArrayView[3],
          floatArrayView[4]
        );
        sphericalPolynomial.y.copyFromFloats(
          floatArrayView[5],
          floatArrayView[6],
          floatArrayView[7]
        );
        sphericalPolynomial.z.copyFromFloats(
          floatArrayView[8],
          floatArrayView[9],
          floatArrayView[10]
        );
        sphericalPolynomial.xx.copyFromFloats(
          floatArrayView[11],
          floatArrayView[12],
          floatArrayView[13]
        );
        sphericalPolynomial.yy.copyFromFloats(
          floatArrayView[14],
          floatArrayView[15],
          floatArrayView[16]
        );
        sphericalPolynomial.zz.copyFromFloats(
          floatArrayView[17],
          floatArrayView[18],
          floatArrayView[19]
        );
        sphericalPolynomial.xy.copyFromFloats(
          floatArrayView[20],
          floatArrayView[21],
          floatArrayView[22]
        );
        sphericalPolynomial.yz.copyFromFloats(
          floatArrayView[23],
          floatArrayView[24],
          floatArrayView[25]
        );
        sphericalPolynomial.zx.copyFromFloats(
          floatArrayView[26],
          floatArrayView[27],
          floatArrayView[28]
        );
          ...
          if (!mipmapGenerator && dataFace) {
            ...
            for (var i = 0; i < this._size * this._size; i++) {
              ...
              if (this._useInGammaSpace) {
                dataFace[i * 3 + 0] = Math.pow(
                  dataFace[i * 3 + 0],
                  ToGammaSpace
                );
                dataFace[i * 3 + 1] = Math.pow(
                  dataFace[i * 3 + 1],
                  ToGammaSpace
                );
                dataFace[i * 3 + 2] = Math.pow(
                  dataFace[i * 3 + 2],
                  ToGammaSpace
                );
              }
      ...
      if (scene) {
        this._texture = (<any>scene.getEngine()).createRawCubeTextureFromUrl(
          this.url,
          scene,
          this._size,
    ...
    private loadHDRTexture() {
      var callback = (buffer: ArrayBuffer): Nullable<ArrayBufferView[]> => {
        ...
        if (this._generateHarmonics) {
          var sphericalPolynomial = CubeMapToSphericalPolynomialTools.ConvertCubeMapToSphericalPolynomial(
            data
          );
          this.sphericalPolynomial = sphericalPolynomial;
        }
        ...
        for (var j = 0; j < 6; j++) {
          ...
          var dataFace = <Float32Array>(<any>data)[
            HDRCubeTexture._facesMapping[j]
          ];
    ...
    public clone(): HDRCubeTexture {
      ...
      var newTexture = new HDRCubeTexture(
        this.url,
        scene,
        size,
        this._noMipmap,
    ...
    public static Parse(
      ...
      scene: Scene,
      rootUrl: string
    ): Nullable<HDRCubeTexture> {
      var texture = null;
      if (parsedTexture.name && !parsedTexture.isRenderTarget) {
        var size = parsedTexture.isBABYLONPreprocessed
          ? null
          : parsedTexture.size;
        texture = new HDRCubeTexture(
          ...
          parsedTexture.usePMREMGenerator
        );
     ...
     */
    public static generateBabylonHDROnDisk(
      url: string,
      size: number,
     ...
     * @return The packed binary data.
     */
    public static generateBabylonHDR(
      url: string,
      size: number,
      ...
      if (!Tools.IsExponentOfTwo(size)) {
        // Need to check engine.needPOTTextures
        return;
      }
