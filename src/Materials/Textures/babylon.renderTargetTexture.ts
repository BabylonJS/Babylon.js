namespace BABYLON {
  export class RenderTargetTexture extends Texture {
     ...
     */
    public renderList: Nullable<Array<AbstractMesh>> = new Array<
      AbstractMesh
    >();
    ...
    constructor(
      ...
      this._resizeObserver = this.getScene()!
        .getEngine()
        .onResizeObservable.add(() => {});
      ...
      if (isCube) {
        ...
        this._texture = scene
          .getEngine()
          .createRenderTargetTexture(this._size, this._renderTargetOptions);
      }
    }
    ...
    public set samples(value: number) {
      ...
      this._samples = scene
        .getEngine()
        .updateRenderTargetTextureSampleCount(this._texture, value);
    }
    ...
    public render(
      ...
      var currentRenderList = this.renderList
        ? this.renderList
        : scene.getActiveMeshes().data;
      var currentRenderListLength = this.renderList
      ...
      for (
        ...
        if (mesh) {
          ...
          if (
            ...
            for (
              var subIndex = 0;
              subIndex < mesh.subMeshes.length;
              subIndex++
            ) {
      ...
      for (
        var particleIndex = 0;
        particleIndex < scene.particleSystems.length;
        particleIndex++
      ) {
    ...
    private renderToTarget(
      ...
      // Bind
      if (this._postProcessManager) {
        this._postProcessManager._prepareFrame(
          this._texture,
          this._postProcesses
        );
      } else if (
        !useCameraPostProcess ||
        !scene.postProcessManager._prepareFrame(this._texture)
      ) {
        if (this._texture) {
          engine.bindFramebuffer(
            this._texture,
            this.isCube ? faceIndex : undefined,
            undefined,
            undefined,
            this.ignoreCameraViewport
          );
        }
      }
      ...
      if (this._postProcessManager) {
        this._postProcessManager._finalizeFrame(
          false,
          this._texture,
          faceIndex,
          this._postProcesses,
