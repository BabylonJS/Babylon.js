namespace BABYLON {
  export class CubeTexture extends BaseTexture {
    ...
    public static CreateFromImages(
      files: string[],
      scene: Scene,
      noMipmap?: boolean
    ) {
    ...
    constructor(
      ...
      format: number = Engine.TEXTUREFORMAT_RGBA,
      prefiltered = false,
      forcedExtension: any = null
    ) {
      super(scene);
      ...
      const extension = forcedExtension
        ? forcedExtension
        : lastDot > -1 ? rootUrl.substring(lastDot).toLowerCase() : "";
    ...
    public static Parse(
      ...
      if (parsedTexture.animations) {
        for (
          ...
          animationIndex < parsedTexture.animations.length;
          animationIndex++
        ) {
