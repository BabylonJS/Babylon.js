namespace BABYLON {
  export class BaseTexture {
    ...
    @serialize("hasAlpha") private _hasAlpha = false;
    ...
    @serialize() public getAlphaFromRGB = false;

    @serialize() public level = 1;

    @serialize() public coordinatesIndex = 0;
    ...
    @serialize() public wrapU = Texture.WRAP_ADDRESSMODE;

    @serialize() public wrapV = Texture.WRAP_ADDRESSMODE;

    @serialize() public wrapR = Texture.WRAP_ADDRESSMODE;

    @serialize()
    public anisotropicFilteringLevel = BaseTexture.DEFAULT_ANISOTROPIC_FILTERING_LEVEL;

    @serialize() public isCube = false;

    @serialize() public is3D = false;

    @serialize() public gammaSpace = true;

    @serialize() public invertZ = false;

    @serialize() public lodLevelInAlpha = false;

    @serialize() public lodGenerationOffset = 0.0;

    @serialize() public lodGenerationScale = 0.8;

    @serialize() public isRenderTarget = false;
    ...
    public getBaseSize(): ISize {
      if (!this.isReady() || !this._texture) return Size.Zero();
    ...
    public get textureType(): number {
      ...
      return this._texture.type !== undefined
        ? this._texture.type
    ...
    public get textureFormat(): number {
      ...
      return this._texture.format !== undefined
    ...
    public readPixels(faceIndex = 0): Nullable<ArrayBufferView> {
      ...
      if (this._texture.isCube) {
        return engine._readTexturePixels(
          this._texture,
          size.width,
          size.height,
          faceIndex
    ...
    public get sphericalPolynomial(): Nullable<SphericalPolynomial> {
      ...
      if (!this._texture._sphericalPolynomial) {
        this._texture._sphericalPolynomial = CubeMapToSphericalPolynomialTools.ConvertCubeMapTextureToSphericalPolynomial(
          this
        );
      }
    ...
    public static WhenAllReady(
      ...
      for (var i = 0; i < textures.length; i++) {
        ...
        if (texture.isReady()) {
          ...
          var onLoadObservable = (texture as any)
            .onLoadObservable as Observable<Texture>;
