...
namespace BABYLON {
  ...
  export class VirtualJoystick {
    ...
    private _touches: StringDictionary<
      { x: number; y: number; prevX: number; prevY: number } | PointerEvent
    >;
    ...
    constructor(leftJoystick?: boolean) {
      ...
      VirtualJoystick.vjCanvas.addEventListener(
        "pointerdown",
    ...
    private _onPointerUp(e: PointerEvent) {
      if (this._joystickPointerID == e.pointerId) {
        ...
        var touch = <{
          ...
          prevX: number;
          prevY: number;
        }>this._touches.get(e.pointerId.toString());
        if (touch) {
          VirtualJoystick.vjCanvasContext.clearRect(
            touch.prevX - 44,
            touch.prevY - 44,
        ...
        this._touches.forEach((key, touch) => {
          if ((<PointerEvent>touch).pointerId === this._joystickPointerID) {
            ...
            VirtualJoystick.vjCanvasContext.arc(
              this._joystickPointerStartPos.x,
              this._joystickPointerStartPos.y,
              40,
              0,
              Math.PI * 2,
              true
            ...
            VirtualJoystick.vjCanvasContext.strokeStyle = this._joystickColor;
            VirtualJoystick.vjCanvasContext.arc(
              this._joystickPointerPos.x,
              this._joystickPointerPos.y,
              40,
              0,
              Math.PI * 2,
              true
    ...
    public releaseCanvas() {
      if (VirtualJoystick.vjCanvas) {
        VirtualJoystick.vjCanvas.removeEventListener(
          "pointerdown",
          this._onPointerDownHandlerRef
        );
        VirtualJoystick.vjCanvas.removeEventListener(
