namespace BABYLON {
  export class Database {
    ...
    private idbFactory = <IDBFactory>(window.indexedDB ||
      window.mozIndexedDB ||
          ...
          if (xhr.status === 200 || Tools.ValidateXHRData(xhr, 1)) {
            try {
              ...
              if (
                ...
                !isNaN(parseInt(manifestFile.version))
              ) {
          ...
          request.onupgradeneeded = (event: IDBVersionChangeEvent) => {
            ...
            if (this.db) {
              try {
                ...
                this.db.createObjectStore("textures", {
                  keyPath: "textureUrl"
                });
              } catch (ex) {
                Tools.Error(
                  "Error while creating object stores. Exception: " + ex.message
                );
                handleError();
              }
            }
    ...
    private _loadImageFromDBAsync(
      ...
      if (this.isSupported && this.db !== null) {
        ...
        var getRequest: IDBRequest = transaction
      ...
      if (this.isSupported) {
        ...
        xhr.addEventListener(
          ...
          () => {
            if (
              xhr.status === 200 ||
              Tools.ValidateXHRData(xhr, !useArrayBuffer ? 1 : 6)
            ) {
              // Blob as response (XHR2)
              ...
              if (!this.hasReachedQuota && this.db) {
                ...
                if (targetStore === "scenes") {
                  newFile = {
                    sceneUrl: url,
                    data: fileData,
                    version: this.manifestVersionFound
                  };
                } else {
