namespace BABYLON {
  ...
  export class DDSTools {
    ...
    public static GetDDSInfo(arrayBuffer: any): DDSInfo {
      ...
      var dxgiFormat =
        fourCC === FOURCC_DX10 ? extendedHeader[off_dxgiFormat] : 0;
    ...
    public static UploadDDSLevels(
      ...
      if (!info.isFourCC && !info.isRGB && !info.isLuminance) {
        Tools.Error(
          "Unsupported format, must contain a FourCC, RGB or LUMINANCE code"
        );
        return;
      }
      ...
      if (info.isFourCC) {
        ...
        switch (fourCC) {
          case FOURCC_DXT1:
            blockBytes = 8;
            internalFormat = (<WEBGL_compressed_texture_s3tc>ext)
      ...
      if (computeFormats) {
        ...
        internalFormat = engine._getRGBABufferInternalSizedFormat(
          info.textureType
        );
      }
          ...
          if (lodIndex === -1 || lodIndex === mip) {
            ...
            if (!info.isCompressed && info.isFourCC) {
              ...
              if (bpp === 24) {
                ...
                byteArray = DDSTools._GetRGBArrayBuffer(
                  width,
                  height,
                  dataOffset,
                  dataLength,
              ...
              var paddedRowSize =
                Math.floor((width + unpackAlignment - 1) / unpackAlignment) *
                unpackAlignment;
              ...
              engine._uploadCompressedDataToTexture(
                sampler,
                i,
                internalFormat,
                width,
