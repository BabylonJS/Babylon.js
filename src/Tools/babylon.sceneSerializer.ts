namespace BABYLON {
  ...
  var serializeGeometry = (
    geometry: Geometry,
    serializationGeometries: any
  ): any => {
  ...
  var finalizeSingleMesh = (mesh: Mesh, serializationObject: any) => {
    ...
    if (
      mesh.delayLoadState === Engine.DELAYLOADSTATE_LOADED ||
      mesh.delayLoadState === Engine.DELAYLOADSTATE_NONE
    ) {
      //serialize material
      if (mesh.material) {
        if (mesh.material instanceof StandardMaterial) {
          ...
          if (
            !serializationObject.materials.some(
              (mat: Material) => mat.id === (<Material>mesh.material).id
            )
          ) {
          ...
          serializationObject.multiMaterials =
            serializationObject.multiMaterials || [];
          if (
            !serializationObject.multiMaterials.some(
              (mat: Material) => mat.id === (<Material>mesh.material).id
            )
          ) {
  ...
  export class SceneSerializer {
    ...
    public static Serialize(scene: Scene): any {
      ...
      // Scene
      serializationObject.useDelayedTextureLoading =
      ...
      for (index = 0; index < scene.transformNodes.length; index++) {
        serializationObject.transformNodes.push(
          scene.transformNodes[index].serialize()
        );
      }
          ...
          if (!mesh.doNotSerialize) {
            if (
              ...
              serializationObject.meshes.push(
                serializeMesh(mesh, serializationObject)
              );
            }
          }
        }
      ...
      for (index = 0; index < scene.particleSystems.length; index++) {
        serializationObject.particleSystems.push(
          scene.particleSystems[index].serialize()
        );
      }
      ...
      for (index = 0; index < scene.lensFlareSystems.length; index++) {
        serializationObject.lensFlareSystems.push(
          scene.lensFlareSystems[index].serialize()
        );
      }
      ...
      for (index = 0; index < scene.lights.length; index++) {
        ...
        if (shadowGenerator) {
          serializationObject.shadowGenerators.push(
            shadowGenerator.serialize()
          );
        }
      }
      ...
      for (index = 0; index < scene.soundTracks.length; index++) {
        ...
        for (
          var soundId = 0;
          soundId < soundtrack.soundCollection.length;
          soundId++
        ) {
          serializationObject.sounds.push(
            soundtrack.soundCollection[soundId].serialize()
          );
        }
      }
    ...
    public static SerializeMesh(
      toSerialize: any /* Mesh || Mesh[] */,
      ...
      if (withParents || withChildren) {
        ...
        for (var i = 0; i < toSerialize.length; ++i) {
          ...
          if (
            withParents &&
            toSerialize[i].parent &&
            toSerialize.indexOf(toSerialize[i].parent) < 0
          ) {
