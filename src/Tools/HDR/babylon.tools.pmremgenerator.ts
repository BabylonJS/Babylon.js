...
namespace BABYLON.Internals {
    ...
    export class PMREMGenerator {
        ...
        private static CP_UDIR = 0;
        private static CP_VDIR = 1;
        private static CP_FACEAXIS = 2;
        
        //used to index cube faces
        private static CP_FACE_X_POS = 0;
        private static CP_FACE_X_NEG = 1;
        private static CP_FACE_Y_POS = 2;
        private static CP_FACE_Y_NEG = 3;
        private static CP_FACE_Z_POS = 4;
        private static CP_FACE_Z_NEG = 5;
        ...
        private static CP_EDGE_LEFT = 0;
        private static CP_EDGE_RIGHT = 1;
        private static CP_EDGE_TOP = 2;
        private static CP_EDGE_BOTTOM = 3;
        
        //corners of CUBE map (P or N specifys if it corresponds to the 
        //  positive or negative direction each of X, Y, and Z
        private static CP_CORNER_NNN = 0;
        private static CP_CORNER_NNP = 1;
        private static CP_CORNER_NPN = 2;
        private static CP_CORNER_NPP = 3;
        private static CP_CORNER_PNN = 4;
        private static CP_CORNER_PNP = 5;
        private static CP_CORNER_PPN = 6;
        private static CP_CORNER_PPP = 7;

        private static _vectorTemp: Vector4 = new Vector4(0, 0, 0, 0);
        ...
        private _normCubeMap: ArrayBufferView[];
        ...
        constructor(public input: ArrayBufferView[],
            ...
            public maxNumMipLevels: number,
            public numChannels: number,
            public isFloat: boolean,
            public specularPower: number,
            public cosinePowerDropPerMip: number,
            public excludeBase: boolean,
            public fixup: boolean) {
        ...
        //
        //--------------------------------------------------------------------------------------
        private filterCubeMapMipChain(): void {
        ...
        //--------------------------------------------------------------------------------------
        private getBaseFilterAngle(cosinePower: number): number {
        ...
        // 
        //--------------------------------------------------------------------------------------
        private precomputeFilterLookupTables(srcCubeMapWidth: number): void {
            var srcTexelAngle: number;
            var iCubeFace: number;
        ...
        // below
        //--------------------------------------------------------------------------------------
        private buildNormalizerSolidAngleCubemap(size: number) {
            var iCubeFace: number;
            var u: number;
            var v: number;
        ...
        //--------------------------------------------------------------------------------------
        // SL BEGIN
        private texelCoordToVect(faceIdx: number, u: number, v: number, size: number, fixup: boolean): Vector4 {
            var nvcU: number;
            var nvcV: number;
            var tempVec: Vector4;
        ...
        */
        ...
        private vectToTexelCoord(x: number, y: number, z: number, size: number): Vector4 {
            var maxCoord: number;
            var faceIdx: number;
        ...
        //--------------------------------------------------------------------------------------
        private areaElement(x: number, y: number): number {
            return Math.atan2(x * y, Math.sqrt(x * x + y * y + 1));
        }

        private texelCoordSolidAngle(faceIdx: number, u: number, v: number, size: number): number {
            // transform from [0..res - 1] to [- (1 - 1 / res) .. (1 - 1 / res)]
            // (+ 0.5f is for texel center addressing)
        ...
        // defined next
        //--------------------------------------------------------------------------------------
        private filterCubeSurfaces(srcCubeMap: ArrayBufferView[], srcSize: number, dstCubeMap: ArrayBufferView[], dstSize: number, filterConeAngle: number,
            specularPower: number): void {
        ...
        //--------------------------------------------------------------------------------------
        private clearFilterExtents(filterExtents: CMGBoundinBox[]): void {
        ...
        //
        //--------------------------------------------------------------------------------------
        private determineFilterExtents(centerTapDir: Vector4, srcSize: number, bboxSize: number,
            filterExtents: CMGBoundinBox[]): void {

            //neighboring face and bleed over amount, and width of BBOX for
            // left, right, top, and bottom edges of this face
            var bleedOverAmount: number[] = [0, 0, 0, 0];
            var bleedOverBBoxMin: number[] = [0, 0, 0, 0];
            var bleedOverBBoxMax: number[] = [0, 0, 0, 0];

            var neighborFace: number;
            var neighborEdge: number;
            var oppositeFaceIdx: number;
