namespace BABYLON {
  ...
  export class LoadFileError extends Error {
    ...
    private static _setPrototypeOf: (
      o: any,
      proto: object | null
    ) => any = (Object as any).setPrototypeOf ||
      ((o, proto) => {
        o.__proto__ = proto;
        return o;
      });
  ...
  export class Tools {
     ...
     */
    public static CorsBehavior:
      | string
      | ((url: string | string[]) => string) = "anonymous";
    ...
    public static Instantiate(className: string): any {
      if (
        ...
        Tools.RegisteredExternalClasses[className]
      ) {
    ...
    public static GetExponentOfTwo(
      value: number,
      max: number,
      mode = Engine.SCALEMODE_NEAREST
    ): number {
      let pot;
    ...
    public static ExtractMinAndMaxIndexed(
    ...
    public static ExtractMinAndMax(
      positions: FloatArray,
      start: number,
      count: number,
      bias: Nullable<Vector2> = null,
      ...
      for (var index = start; index < start + count; index++) {
        var current = new Vector3(
          positions[index * stride],
          positions[index * stride + 1],
    ...
    public static Vector2ArrayFeeder(
      array: Array<Vector2> | Float32Array
    ): (i: number) => Nullable<Vector2> {
      return (index: number) => {
        let isFloatArray =
          (<Float32Array>array).BYTES_PER_ELEMENT !== undefined;
    ...
    public static ExtractMinAndMaxVector2(
      feeder: (index: number) => Vector2,
      bias: Nullable<Vector2> = null
    ): { minimum: Vector2; maximum: Vector2 } {
    ...
    public static SetCorsBehavior(
      ...
      if (Tools.CorsBehavior) {
        if (
          typeof Tools.CorsBehavior === "string" ||
          Tools.CorsBehavior instanceof String
        ) {
    ...
    public static LoadImage(
      ...
      if (
        url.substr(0, 5) !== "data:" &&
        database &&
        database.enableTexturesOffline &&
        Database.IsUASupportingBlobStorage
      ) {
            ...
            try {
              ...
              try {
                blobURL = URL.createObjectURL(
                  FilesInput.FilesToLoad[textureName],
                  { oneTimeOnly: true }
                );
              } catch (ex) {
      ...
      if (url.indexOf("file:") !== -1) {
        ...
        if (FilesInput.FilesToLoad[fileName]) {
          return Tools.ReadFile(
            FilesInput.FilesToLoad[fileName],
            onSuccess,
            onProgress,
            useArrayBuffer
          );
          ...
          const onReadyStateChange = () => {
            ...
            if (request.readyState === (XMLHttpRequest.DONE || 4)) {
              ...
              if (
                ...
                (!Tools.IsWindowObjectExist() && request.status === 0)
              ) {
                onSuccess(
                  !useArrayBuffer
    ...
    public static ReadFileAsDataURL(
      fileToLoad: Blob,
      callback: (data: any) => void,
      progressCallback: (this: MSBaseReader, ev: ProgressEvent) => any
    ): IFileRequest {
    ...
    public static DeepCopy(
      ...
      for (var prop in source) {
        if (
          prop[0] === "_" &&
          (!mustCopyList || mustCopyList.indexOf(prop) === -1)
        ) {
          continue;
        }
    ...
    public static RegisterTopRootEvents(
      events: { name: string; handler: Nullable<(e: FocusEvent) => any> }[]
    ): void {
    ...
    public static UnregisterTopRootEvents(
      events: { name: string; handler: Nullable<(e: FocusEvent) => any> }[]
    ): void {
      for (var index = 0; index < events.length; index++) {
        var event = events[index];
    ...
    public static CreateScreenshotUsingRenderTarget(
      ...
      if (antialiasing) {
        texture.addPostProcess(
          new FxaaPostProcess("antialiasing", 1.0, scene.activeCamera)
        );
      }
    ...
    static get AllLogLevel(): number {
      return (
        Tools._MessageLogLevel | Tools._WarningLogLevel | Tools._ErrorLogLevel
      );
    }
    ...
    private static _WarnEnabled(message: string): void {
      ...
      var entry =
        "<div style='color:orange'>" + formattedMessage + "</div><br>";
      Tools._AddLogEntry(entry);
    }
    ...
    public static set PerformanceLogLevel(level: number) {
      if (
        (level & Tools.PerformanceUserMarkLogLevel) ===
      ...
      if (
        (level & Tools.PerformanceConsoleLogLevel) ===
    ...
    public static StartPerformanceCounter: (
      counterName: string,
      condition?: boolean
    ) => void = Tools._StartPerformanceCounterDisabled;
    public static EndPerformanceCounter: (
      counterName: string,
      condition?: boolean
    ) => void = Tools._EndPerformanceCounterDisabled;
     ...
     */
    public static getFullClassName(
      object: any,
     ...
     */
    public static hashCodeFromStream(
      feeder: (index: number) => number
    ): number {
