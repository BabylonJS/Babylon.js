namespace BABYLON {
  export class DirectionalLight extends ShadowLight {
    ...
    @serialize() public autoUpdateExtends = true;
    ...
    protected _setDefaultAutoExtendShadowProjectionMatrix(
      ...
      if (this.autoUpdateExtends || this._orthoLeft === Number.MAX_VALUE) {
        ...
        for (var meshIndex = 0; meshIndex < renderList.length; meshIndex++) {
          ...
          for (
            var index = 0;
            index < boundingBox.vectorsWorld.length;
            index++
          ) {
            Vector3.TransformCoordinatesToRef(
              boundingBox.vectorsWorld[index],
              viewMatrix,
              tempVector3
            );
    ...
    public transferToEffect(
      ...
      if (this.computeTransformedInformation()) {
        this._uniformBuffer.updateFloat4(
          "vLightData",
          this.transformedDirection.x,
          this.transformedDirection.y,
          this.transformedDirection.z,
          1,
          lightIndex
        );
        return this;
