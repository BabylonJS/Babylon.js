namespace BABYLON {
  export class Light extends Node {
    ...
    @serializeAsColor3() public diffuse = new Color3(1.0, 1.0, 1.0);

    @serializeAsColor3() public specular = new Color3(1.0, 1.0, 1.0);

    @serialize() public intensity = 1.0;

    @serialize() public range = Number.MAX_VALUE;
     ...
     */
    @serialize() private _renderPriority: number;
     ...
     */
    @serialize() public shadowEnabled: boolean = true;
    ...
    @serialize("excludeWithLayerMask") private _excludeWithLayerMask = 0;
    ...
    @serialize("lightmapMode") private _lightmapMode = 0;
    ...
    public toString(fullDetails?: boolean): string {
      ...
      ret +=
        ", type: " +
        ["Point", "Directional", "Spot", "Hemispheric"][this.getTypeID()];
      if (this.animations) {
    ...
    public canAffectMesh(mesh: AbstractMesh): boolean {
      ...
      if (
        ...
        this.includedOnlyMeshes.indexOf(mesh) === -1
      ) {
        return false;
      }
      ...
      if (
        ...
        (this.includeOnlyWithLayerMask & mesh.layerMask) === 0
      ) {
        return false;
      }
      ...
      if (
        ...
        this.excludeWithLayerMask & mesh.layerMask
      ) {
        return false;
      }
    ...
    public clone(name: string): Nullable<Light> {
      let constructor = Light.GetConstructorFromName(
        this.getTypeID(),
        name,
        this.getScene()
      );
    ...
    static GetConstructorFromName(
      ...
      switch (type) {
        ...
        case 2:
          return () =>
            new SpotLight(name, Vector3.Zero(), Vector3.Zero(), 0, 0, scene);
        case 3:
    ...
    public static Parse(parsedLight: any, scene: Scene): Nullable<Light> {
      ...
      if (parsedLight.animations) {
        for (
          ...
          animationIndex < parsedLight.animations.length;
          animationIndex++
        ) {
      ...
      if (parsedLight.autoAnimate) {
        scene.beginAnimation(
          light,
          parsedLight.autoAnimateFrom,
          parsedLight.autoAnimateTo,
          parsedLight.autoAnimateLoop,
          parsedLight.autoAnimateSpeed || 1.0
        );
      }
        ...
        case Light.LIGHTTYPEID_DIRECTIONALLIGHT:
          switch (photometricMode) {
            ...
            case Light.INTENSITYMODE_LUMINANCE:
              ...
              let solidAngle =
                2.0 * Math.PI * (1.0 - Math.cos(apexAngleRadians));
