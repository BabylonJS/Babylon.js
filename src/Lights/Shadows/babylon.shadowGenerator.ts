namespace BABYLON {
  ...
  export interface IShadowGenerator {
    ...
    forceCompilation(
      onCompiled?: (generator: ShadowGenerator) => void,
      options?: Partial<{ useInstances: boolean }>
    ): void;
  ...
  export class ShadowGenerator implements IShadowGenerator {
    ...
    public get depthScale(): number {
      return this._depthScale !== undefined
        ? this._depthScale
    ...
    public set filter(value: number) {
      ...
      if (this._light.needCube()) {
        if (value === ShadowGenerator.FILTER_BLUREXPONENTIALSHADOWMAP) {
          ...
          value === ShadowGenerator.FILTER_BLURCLOSEEXPONENTIALSHADOWMAP
        ) {
    ...
    public get useBlurVarianceShadowMap(): boolean {
      Tools.Warn(
        "VSM are now replaced by ESM. Please use useBlurExponentialShadowMap instead."
      );
    ...
    public set useExponentialShadowMap(value: boolean) {
      if (
        !value &&
        this.filter !== ShadowGenerator.FILTER_EXPONENTIALSHADOWMAP
      ) {
        return;
      }
      this.filter = value
    ...
    public set useBlurExponentialShadowMap(value: boolean) {
      if (
        !value &&
        this.filter !== ShadowGenerator.FILTER_BLUREXPONENTIALSHADOWMAP
      ) {
        return;
      }
      this.filter = value
    ...
    public set useCloseExponentialShadowMap(value: boolean) {
      if (
        !value &&
        this.filter !== ShadowGenerator.FILTER_CLOSEEXPONENTIALSHADOWMAP
      ) {
        return;
      }
      this.filter = value
    ...
    public get useBlurCloseExponentialShadowMap(): boolean {
      return (
        this.filter === ShadowGenerator.FILTER_BLURCLOSEEXPONENTIALSHADOWMAP
      );
    }
    ...
    private _initializeShadowMap(): void {
      ...
      this._shadowMap.customRenderFunction = this._renderForShadowMap.bind(
        this
      );
    ...
    private _initializeBlurRTTAndPostProcesses(): void {
      ...
      if (!this.useKernelBlur || this.blurScale !== 1.0) {
        this._shadowMap2 = new RenderTargetTexture(
      ...
      if (this.isReady(subMesh, hardwareInstancedRendering)) {
        ...
        if (scene.activeCamera) {
          this._effect.setFloat2(
            ...
            this.getLight().getDepthMinZ(scene.activeCamera) +
              this.getLight().getDepthMaxZ(scene.activeCamera)
          );
        }
        ...
        if (material && material.needAlphaTesting()) {
          ...
          if (alphaTexture) {
            ...
            this._effect.setMatrix(
              "diffuseMatrix",
              alphaTexture.getTextureMatrix() || this._defaultTextureMatrix
            );
          }
        }
        ...
        if (mesh.useBones && mesh.computeBonesUsingShaders) {
          this._effect.setMatrices(
            "mBones",
            (<Skeleton>mesh.skeleton).getTransformMatrices(mesh)
          );
        }
    ...
    public forceCompilation(
      ...
      var checkReady = () => {
        ...
        while (
          this.isReady(subMeshes[currentIndex], localOptions.useInstances)
        ) {
          currentIndex++;
    ...
    public isReady(subMesh: SubMesh, useInstances: boolean): boolean {
      ...
      if (mesh.useBones && mesh.computeBonesUsingShaders) {
        ...
        defines.push(
          "#define BonesPerMesh " + ((<Skeleton>mesh.skeleton).bones.length + 1)
        );
      } else {
    ...
    public bindShadowLight(lightIndex: string, effect: Effect): void {
      ...
      light._uniformBuffer.updateFloat4(
        "shadowsInfo",
        this.getDarkness(),
        this.blurScale / shadowMap.getSize().width,
        this.depthScale,
      ...
      light._uniformBuffer.updateFloat2(
        ...
        this.getLight().getDepthMinZ(camera) +
          this.getLight().getDepthMaxZ(camera),
        lightIndex
      );
    }
    ...
    public getTransformMatrix(): Matrix {
      ...
      if (
        ...
        Matrix.LookAtLHToRef(
          lightPosition,
          lightPosition.add(this._lightDirection),
          Vector3.Up(),
          this._viewMatrix
        );
    ...
    public serialize(): any {
      ...
      if (shadowMap.renderList) {
        for (
          var meshIndex = 0;
          meshIndex < shadowMap.renderList.length;
          meshIndex++
        ) {
    ...
    public static Parse(
      ...
      for (
        var meshIndex = 0;
        meshIndex < parsedShadowGenerator.renderList.length;
        meshIndex++
      ) {
        var meshes = scene.getMeshesByID(
          parsedShadowGenerator.renderList[meshIndex]
        );
      ...
      shadowGenerator.forceBackFacesOnly =
        parsedShadowGenerator.forceBackFacesOnly;
