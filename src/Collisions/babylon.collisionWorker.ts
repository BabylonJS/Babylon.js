...
namespace BABYLON {
  ...
  export class CollideWorker {
    ...
    public collideWithWorld(
      ...
      for (var i = 0; i < len; ++i) {
        ...
        if (parseInt(uniqueId) != excludedMeshUniqueId) {
          ...
          if (mesh.checkCollisions) this.checkCollision(mesh);
        }
      }
      ...
      this.collideWithWorld(
        ...
        velocity,
        maximumRetry,
        excludedMeshUniqueId
      );
    }

    private checkCollision(mesh: SerializedMesh) {
      if (
        !this.collider._canDoCollision(
          Vector3.FromArray(mesh.sphereCenter),
          mesh.sphereRadius,
      ...
      Matrix.ScalingToRef(
        1.0 / this.collider._radius.x,
        1.0 / this.collider._radius.y,
        1.0 / this.collider._radius.z,
        this.collisionsScalingMatrix
      );
      var worldFromCache = Matrix.FromArray(mesh.worldMatrixFromCache);
      worldFromCache.multiplyToRef(
        this.collisionsScalingMatrix,
        this.collisionTranformationMatrix
      );

      this.processCollisionsForSubMeshes(
        this.collisionTranformationMatrix,
        mesh
      );
      //return colTransMat;
    ...
    private processCollisionsForSubMeshes(
      ...
      for (var index = 0; index < len; index++) {
        ...
        // Bounding test
        if (len > 1 && !this.checkSubmeshCollision(subMesh)) continue;
    ...
    private collideForSubMesh(
      subMesh: SerializedSubMesh,
      transformMatrix: Matrix,
      meshGeometry: SerializedGeometry
    ): void {
      if (!(<any>meshGeometry)["positionsArray"]) {
        (<any>meshGeometry)["positionsArray"] = [];
        for (
          var i = 0, len = meshGeometry.positions.length;
          i < len;
          i = i + 3
        ) {
          var p = Vector3.FromArray([
            meshGeometry.positions[i],
      ...
      if (
        ...
        !(<any>subMesh)["_lastColliderTransformMatrix"].equals(transformMatrix)
      ) {
        (<any>subMesh)[
          "_lastColliderTransformMatrix"
        ...
        for (var i = start; i < end; i++) {
          (<any>subMesh)["_lastColliderWorldVertices"].push(
            Vector3.TransformCoordinates(
              (<any>meshGeometry)["positionsArray"][i],
              transformMatrix
            )
          );
        }
      ...
      // Collide
      this.collider._collide(
        (<any>subMesh)["_trianglePlanes"],
        (<any>subMesh)["_lastColliderWorldVertices"],
        <any>meshGeometry.indices,
        subMesh.indexStart,
    ...
    private checkSubmeshCollision(subMesh: SerializedSubMesh): boolean {
      return this.collider._canDoCollision(
        Vector3.FromArray(subMesh.sphereCenter),
  ...
  export class CollisionDetectorTransferable implements ICollisionDetector {
    ...
    public onCollision(payload: CollidePayload) {
      ...
      var colliderWorker = new CollideWorker(
        collider,
        this._collisionCache,
        finalPosition
      );
      colliderWorker.collideWithWorld(
        Vector3.FromArray(payload.collider.position),
        Vector3.FromArray(payload.collider.velocity),
        payload.maximumRetry,
