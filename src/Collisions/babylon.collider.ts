namespace BABYLON {
  ...
  var getLowestRoot: (
    ...
    c: number,
    maxR: number
  ) => { root: number; found: boolean } = (function() {
  ...
  export class Collider {
    ...
    public _canDoCollision(
      ...
      if (
        !intersectBoxAASphere(
          vecMin,
          vecMax,
          this._basePointWorld,
          this._velocityWorldLength + max
        )
      )
        return false;
    ...
    public _testTriangle(
      ...
      if (
        !hasMaterial &&
        !trianglePlane.isFrontFacingTo(this._normalizedVelocity, 0)
      )
        return;

      var signedDistToTrianglePlane = trianglePlane.signedDistanceTo(
        this._basePoint
      );
      ...
      if (!found) {
        ...
        a =
          edgeSquaredLength * -velocitySquaredLength +
          edgeDotVelocity * edgeDotVelocity;
        b =
          edgeSquaredLength *
            (2.0 * Vector3.Dot(this._velocity, this._baseToVertex)) -
          2.0 * edgeDotVelocity * edgeDotBaseToVertex;
        c =
          edgeSquaredLength * (1.0 - this._baseToVertex.lengthSquared()) +
          edgeDotBaseToVertex * edgeDotBaseToVertex;

        lowestRoot = getLowestRoot(a, b, c, t);
        if (lowestRoot.found) {
          var f =
            (edgeDotVelocity * lowestRoot.root - edgeDotBaseToVertex) /
            edgeSquaredLength;
    ...
    public _getResponse(pos: Vector3, vel: Vector3): void {
      ...
      this._slidePlaneNormal.scaleToRef(
        this._epsilon,
        this._displacementVector
      );
      ...
      this._slidePlaneNormal.scaleInPlace(
        Plane.SignedDistanceToPlaneFromPositionAndNormal(
          ...
          this._slidePlaneNormal,
          this._destinationPoint
        )
      );
