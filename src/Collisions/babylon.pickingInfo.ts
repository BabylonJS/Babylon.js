  ...
  export class PickingInfo {
    ...
    public getNormal(
      ...
      if (useVerticesNormals) {
        var normals = <FloatArray>this.pickedMesh.getVerticesData(
        ...
        var positions = <FloatArray>this.pickedMesh.getVerticesData(
          VertexBuffer.PositionKind
        );

        var vertex1 = Vector3.FromArray(
          positions,
          indices[this.faceId * 3] * 3
        );
      ...
      if (useWorldCoordinates) {
        result = Vector3.TransformNormal(
          result,
          this.pickedMesh.getWorldMatrix()
        );
      }
