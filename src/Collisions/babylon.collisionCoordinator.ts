namespace BABYLON {
  ...
  export interface BabylonMessage {
    ...
    payload:
      | InitPayload
      | CollidePayload
      | UpdatePayload /*any for TS under 1.4*/;
  }
  ...
  export class CollisionCoordinatorWorker implements ICollisionCoordinator {
    ...
    private _collisionsCallbackArray: Array<
      Nullable<
        (
          collisionIndex: number,
          newPosition: Vector3,
    ...
    public static SerializeGeometry = function(
      geometry: Geometry
    ): SerializedGeometry {
      return {
        id: geometry.id,
    ...
    public getNewPosition(
      ...
      if (
        this._collisionsCallbackArray[collisionIndex] ||
        this._collisionsCallbackArray[collisionIndex + 100000]
      )
        return;
    ...
    public init(scene: Scene): void {
      ...
      var workerUrl = WorkerIncluded
        ? Engine.CodeRepository + "Collisions/babylon.collisionWorker.js"
        : URL.createObjectURL(
            new Blob([CollisionWorker], { type: "application/javascript" })
          );
    ...
    private _afterRender = () => {
      ...
      if (
        this._toRemoveGeometryArray.length == 0 &&
        this._toRemoveMeshesArray.length == 0 &&
        Object.keys(this._addUpdateGeometriesList).length == 0 &&
        Object.keys(this._addUpdateMeshesList).length == 0
      ) {
        return;
      }
      ...
      for (var id in payload.updatedGeometries) {
        if (payload.updatedGeometries.hasOwnProperty(id)) {
          ...
          serializable.push(
            (<UpdatePayload>message.payload).updatedGeometries[id].indices
              .buffer
          );
          serializable.push(
            (<UpdatePayload>message.payload).updatedGeometries[id].normals
              .buffer
          );
          serializable.push(
            (<UpdatePayload>message.payload).updatedGeometries[id].positions
              .buffer
          );
        }
      }
    ...
    private _onMessageFromWorker = (e: MessageEvent) => {
      ...
      switch (returnData.taskType) {
        ...
        case WorkerTaskType.COLLIDE:
          ...
          let callback = this._collisionsCallbackArray[
            returnPayload.collisionId
          ];

          if (callback) {
            let mesh = this._scene.getMeshByUniqueID(
              returnPayload.collidedMeshUniqueId
            );
  ...
  export class CollisionCoordinatorLegacy implements ICollisionCoordinator {
    ...
    private _collideWithWorld(
      ...
      var collisionMask = excludedMesh
        ? excludedMesh.collisionMask
