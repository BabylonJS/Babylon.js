namespace BABYLON {
  ...
  export class ParticleSystem
    implements IDisposable, IAnimatable, IParticleSystem {
    /**
    ...
    constructor(
      ...
      this._vertexData = new Float32Array(
        capacity * this._vertexBufferSize * 4
      );
      ...
      var positions = this._vertexBuffer.createVertexBuffer(
        VertexBuffer.PositionKind,
        0,
        3
      );
      var colors = this._vertexBuffer.createVertexBuffer(
      ...
      if (this._isAnimationSheetEnabled) {
        var cellIndexBuffer = this._vertexBuffer.createVertexBuffer(
          "cellIndex",
          11,
          1
        );
     ...
     * @ignore (for internal use only)
     */
    public _appendParticleVertexWithAnimation(
      index: number,
      particle: Particle,
      offsetX: number,
      offsetY: number
    ...
    private _update(newParticles: number): void {
      ...
      if ((<AbstractMesh>this.emitter).position) {
        ...
        worldMatrix = Matrix.Translation(
          emitterPosition.x,
          emitterPosition.y,
          emitterPosition.z
        );
      ...
      for (var index = 0; index < newParticles; index++) {
        ...
        var emitPower = Scalar.RandomRange(
          this.minEmitPower,
          this.maxEmitPower
        );
        ...
        if (this.startPositionFunction) {
          ...
          this.particleEmitterType.startPositionFunction(
            worldMatrix,
            particle.position,
            particle
          );
        ...
        if (this.startDirectionFunction) {
          ...
          this.particleEmitterType.startDirectionFunction(
            emitPower,
            worldMatrix,
            particle.direction,
            particle
        ...
        particle.lifeTime = Scalar.RandomRange(
          this.minLifeTime,
          this.maxLifeTime
        );

        particle.size = Scalar.RandomRange(this.minSize, this.maxSize);
        particle.angularSpeed = Scalar.RandomRange(
    ...
    public animate(): void {
      ...
      this._scaledUpdateSpeed =
        this.updateSpeed * this._scene.getAnimationRatio();
      ...
      if (this.manualEmitCount > -1) {
        ...
        this._newPartsExcess +=
          this.emitRate * this._scaledUpdateSpeed - newParticles;
      }
    ...
    private _appendParticleVertexes: Nullable<
      (offset: number, particle: Particle) => void
    > = null;

    private _appenedParticleVertexesWithSheet(
      offset: number,
      particle: Particle
    ) {
    ...
    private _appenedParticleVertexesNoSheet(
      offset: number,
      particle: Particle
    ) {
    ...
    public render(): number {
      ...
      if (this._isAnimationSheetEnabled) {
        ...
        effect.setFloat3(
          "particlesInfos",
          this.spriteCellWidth / baseSize.width,
          this.spriteCellHeight / baseSize.height,
          baseSize.width / this.spriteCellWidth
        );
      }
     ...
     * @returns the emitter
     */
    public createDirectedSphereEmitter(
      radius = 1,
      direction1 = new Vector3(0, 1.0, 0),
      direction2 = new Vector3(0, 1.0, 0)
    ): SphereDirectedParticleEmitter {
      var particleEmitter = new SphereDirectedParticleEmitter(
        radius,
        direction1,
        direction2
      );
    ...
    public clone(name: string, newEmitter: any): ParticleSystem {
      ...
      if (this.customShader != null) {
        ...
        var defines: string =
          program.shaderOptions.defines.length > 0
            ? program.shaderOptions.defines.join("\n")
            : "";
        custom = this._scene
          .getEngine()
      ...
      if (this.particleTexture) {
        result.particleTexture = new Texture(
    ...
    public static Parse(
      ...
      if (parsedParticleSystem.emitterId) {
        particleSystem.emitter = scene.getLastMeshByID(
          parsedParticleSystem.emitterId
        );
      } else {
        particleSystem.emitter = Vector3.FromArray(
          parsedParticleSystem.emitter
        );
      }
      ...
      if (parsedParticleSystem.animations) {
        for (
          ...
          animationIndex < parsedParticleSystem.animations.length;
          animationIndex++
        ) {
      ...
      particleSystem.spriteCellChangeSpeed =
        parsedParticleSystem.spriteCellChangeSpeed;
