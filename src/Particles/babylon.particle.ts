namespace BABYLON {
  ...
  export class Particle {
    ...
    private updateCellIndexWithSpeedCalculated(
      scaledUpdateSpeed: number
    ): void {
      //   (ageOffset / scaledUpdateSpeed) / available cells
      var numberOfScaledUpdatesPerCell =
        (this.lifeTime - this.age) /
        scaledUpdateSpeed /
        (this.particleSystem.endSpriteCellID + 1 - this.cellIndex);
    ...
    private updateCellIndexWithCustomSpeed(): void {
      if (
        this._currentFrameCounter >= this.particleSystem.spriteCellChangeSpeed
      ) {
        this.cellIndex++;
        this._currentFrameCounter = 0;
        if (this.cellIndex > this.particleSystem.endSpriteCellID) {
