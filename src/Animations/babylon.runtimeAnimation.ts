  ...
  export class RuntimeAnimation {
    ...
    private _interpolate(
        ...
        Math.min(
          ...
          Math.floor(
            keys.length *
              ...
              (keys[keys.length - 1].frame - keys[0].frame)
          ) - 1
        )
      );
      ...
      if (keys[startKeyIndex].frame >= currentFrame) {
        while (
          startKeyIndex - 1 >= 0 &&
          keys[startKeyIndex].frame >= currentFrame
        ) {
          startKeyIndex--;
        }
      }
      ...
      for (var key = startKeyIndex; key < keys.length; key++) {
        ...
        if (endKey.frame >= currentFrame) {
          ...
          var useTangent =
            startKey.outTangent !== undefined && endKey.inTangent !== undefined;
          ...
          switch (this._animation.dataType) {
            ...
            case Animation.ANIMATIONTYPE_FLOAT:
              var floatValue = useTangent
                ? this._animation.floatInterpolateFunctionWithTangents(
                    startValue,
                    startKey.outTangent * frameDelta,
                    endValue,
                    endKey.inTangent * frameDelta,
                    gradient
                  )
                : this._animation.floatInterpolateFunction(
                    startValue,
                    endValue,
                    gradient
                  );
            ...
            case Animation.ANIMATIONTYPE_QUATERNION:
              var quatValue = useTangent
                ? this._animation.quaternionInterpolateFunctionWithTangents(
                    startValue,
                ...
                : this._animation.quaternionInterpolateFunction(
                    startValue,
                    endValue,
                    gradient
                  );
            ...
            case Animation.ANIMATIONTYPE_VECTOR3:
              var vec3Value = useTangent
                ? this._animation.vector3InterpolateFunctionWithTangents(
                    startValue,
                    startKey.outTangent.scale(frameDelta),
                    endValue,
                    endKey.inTangent.scale(frameDelta),
                ...
                : this._animation.vector3InterpolateFunction(
                    startValue,
                    endValue,
                    gradient
                  );
            ...
            case Animation.ANIMATIONTYPE_VECTOR2:
              var vec2Value = useTangent
                ? this._animation.vector2InterpolateFunctionWithTangents(
                    startValue,
                    startKey.outTangent.scale(frameDelta),
                    endValue,
                    endKey.inTangent.scale(frameDelta),
                ...
                : this._animation.vector2InterpolateFunction(
                    startValue,
                    endValue,
                    gradient
                  );
            ...
            case Animation.ANIMATIONTYPE_SIZE:
              switch (loopMode) {
                case Animation.ANIMATIONLOOPMODE_CYCLE:
                case Animation.ANIMATIONLOOPMODE_CONSTANT:
                  return this._animation.sizeInterpolateFunction(
                    startValue,
                    endValue,
                    gradient
                  );
                case Animation.ANIMATIONLOOPMODE_RELATIVE:
                  return this._animation
                    ...
                    .add(offsetValue.scale(repeatCount));
              }
            // Color3
            case Animation.ANIMATIONTYPE_COLOR3:
              switch (loopMode) {
                case Animation.ANIMATIONLOOPMODE_CYCLE:
                case Animation.ANIMATIONLOOPMODE_CONSTANT:
                  return this._animation.color3InterpolateFunction(
                    startValue,
                    endValue,
                    gradient
                  );
              ...
              switch (loopMode) {
                ...
                case Animation.ANIMATIONLOOPMODE_CONSTANT:
                  if (Animation.AllowMatricesInterpolation) {
                    return this._animation.matrixInterpolateFunction(
                      startValue,
                      endValue,
                      gradient
                    );
      ...
      if (this._animation.enableBlending && this._blendingFactor <= 1.0) {
        ...
        if (this._originalBlendValue.prototype) {
          ...
          if (this._originalBlendValue.prototype.Lerp) {
            ...
            destination[
              path
            ] = this._originalBlendValue.construtor.prototype.Lerp(
              currentValue,
              this._originalBlendValue,
              this._blendingFactor
            );
          } else {
          ...
          // Direct value
          destination[path] =
            this._originalBlendValue * (1.0 - this._blendingFactor) +
            this._blendingFactor * currentValue;
        }
    ...
    public _prepareForSpeedRatioChange(newSpeedRatio: number): void {
      let newRatio =
        this._previousDelay *
        (this._animation.framePerSecond * newSpeedRatio) /
        1000.0;
    ...
    public animate(
      ...
      var ratio =
        delay * (this._animation.framePerSecond * speedRatio) / 1000.0 +
        this._ratioOffset;
      ...
      if (
        ((to > from && ratio > range) || (from > to && ratio < range)) &&
        !loop
      ) {
        ...
        if (this._animation.loopMode !== Animation.ANIMATIONLOOPMODE_CYCLE) {
          ...
          if (!this._offsetsCache[keyOffset]) {
            var fromValue = this._interpolate(
              from,
              0,
              Animation.ANIMATIONLOOPMODE_CYCLE
            );
            var toValue = this._interpolate(
              to,
              0,
              Animation.ANIMATIONLOOPMODE_CYCLE
            );
      ...
      var currentValue = this._interpolate(
        currentFrame,
        repeatCount,
        this._animation.loopMode,
        offsetValue,
        highLimitValue
      );
      ...
      for (var index = 0; index < events.length; index++) {
        ...
        if (
          (range > 0 &&
            currentFrame >= events[index].frame &&
