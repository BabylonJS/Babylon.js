namespace BABYLON {
  export class Animatable {
    ...
    public getRuntimeAnimationByTargetProperty(
      property: string
    ): Nullable<RuntimeAnimation> {
      var runtimeAnimations = this._runtimeAnimations;
        ...
        if (idx > -1) {
          ...
          for (var index = runtimeAnimations.length - 1; index >= 0; index--) {
            if (
              ...
              runtimeAnimations[index].animation.name != animationName
            ) {
              continue;
            }
    ...
    public _animate(delay: number): boolean {
      ...
      for (index = 0; index < runtimeAnimations.length; index++) {
        var animation = runtimeAnimations[index];
        var isRunning = animation.animate(
          delay - this._localDelayOffset,
          this.fromFrame,
          this.toFrame,
          this.loopAnimation,
          this._speedRatio
