namespace BABYLON {
  ...
  export class BackEase extends EasingFunction implements IEasingFunction {
    ...
    public easeInCore(gradient: number): number {
      ...
      return (
        Math.pow(gradient, 3.0) -
        gradient * num * Math.sin(3.1415926535897931 * gradient)
      );
    }
  }
  ...
  export class BounceEase extends EasingFunction implements IEasingFunction {
    ...
    public easeInCore(gradient: number): number {
      ...
      var num2 = num7 - num8;
      return (
        -Math.pow(1.0 / bounciness, y - num3) /
        (num2 * num2) *
        (num6 - num2) *
        (num6 + num2)
      );
  ...
  export class ElasticEase extends EasingFunction implements IEasingFunction {
    ...
    public easeInCore(gradient: number): number {
      ...
      return (
        num2 *
        Math.sin((6.2831853071795862 * num3 + 1.5707963267948966) * gradient)
      );
    }
  }
  ...
  export class ExponentialEase extends EasingFunction
    ...
    public easeInCore(gradient: number): number {
      ...
      return (
        (Math.exp(this.exponent * gradient) - 1.0) /
        (Math.exp(this.exponent) - 1.0)
      );
    }
  }
  ...
  export class BezierCurveEase extends EasingFunction
    ...
    public easeInCore(gradient: number): number {
      return BezierCurve.interpolate(
        gradient,
        this.x1,
        this.y1,
        this.x2,
