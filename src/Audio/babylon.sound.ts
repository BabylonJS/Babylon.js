  ...
  export class Sound {
            ...
            case "Array":
              ...
              for (var i = 0; i < urls.length; i++) {
                ...
                if (
                  ...
                  Engine.audioEngine.isMP3supported
                ) {
                  codecSupportedFound = true;
                }
                if (
                  url.indexOf(".ogg", url.length - 4) !== -1 &&
                  Engine.audioEngine.isOGGsupported
                ) {
                  codecSupportedFound = true;
                }
    ...
    public dispose() {
      if (Engine.audioEngine.canUseWebAudio && this._isReadyToPlay) {
        ...
        if (this._connectedMesh && this._registerFunc) {
          this._connectedMesh.unregisterAfterWorldMatrixUpdate(
            this._registerFunc
          );
    ...
    private _soundLoaded(audioData: ArrayBuffer) {
      ...
      Engine.audioEngine.audioContext.decodeAudioData(
        ...
        (err: any) => {
          Tools.Error(
            "Error while decoding audio data for: " +
              this.name +
              " / Error: " +
              err
          );
    ...
    public updateOptions(options: any) {
      if (options) {
        ...
        this.useCustomAttenuation =
          options.useCustomAttenuation || this.useCustomAttenuation;
    ...
    public setDirectionalCone(
      ...
      if (coneOuterAngle < coneInnerAngle) {
        Tools.Error(
          "setDirectionalCone(): outer angle of the cone must be superior or equal to the inner angle."
        );
        return;
      }
    ...
    public updateDistanceFromListener() {
      if (
        ...
        var distance = this._connectedMesh.getDistanceToCamera(
          this._scene.activeCamera
        );
        this._soundGain.gain.value = this._customAttenuationFunction(
          this._volume,
          distance,
          this.maxDistance,
          this.refDistance,
    ...
    public play(time?: number, offset?: number) {
      if (
        ...
        try {
          ...
          var startTime = time
            ? Engine.audioEngine.audioContext.currentTime + time
            ...
            if (this.spatialSound && this._soundPanner) {
              ...
              if (this._isDirectional) {
                ...
                if (this._connectedMesh) {
                  ...
                  this._soundPanner.setOrientation(
                    this._localDirection.x,
                    this._localDirection.y,
                    this._localDirection.z
                  );
                }
              }
            }
          ...
          if (this._streaming) {
            if (!this._streamingSource) {
              this._streamingSource = Engine.audioEngine.audioContext.createMediaElementSource(
          ...
          Tools.Error(
            "Error while trying to play audio: " + this.name + ", " + ex.message
          );
        }
      }
    }
    ...
    public pause() {
      if (this.isPlaying) {
        ...
        if (this._streaming) {
          ...
          this.stop(0);
          this._startOffset +=
            Engine.audioEngine.audioContext.currentTime - this._startTime;
        }
      }
    }
    ...
    public setVolume(newVolume: number, time?: number) {
      if (Engine.audioEngine.canUseWebAudio && this._soundGain) {
        if (time && Engine.audioEngine.audioContext) {
          ...
          this._soundGain.gain.setValueAtTime(
            this._soundGain.gain.value,
            Engine.audioEngine.audioContext.currentTime
          );
          this._soundGain.gain.linearRampToValueAtTime(
            newVolume,
            Engine.audioEngine.audioContext.currentTime + time
          );
        } else {
    ...
    public attachToMesh(meshToConnectTo: AbstractMesh) {
      ...
      this._registerFunc = (connectedMesh: TransformNode) =>
        this._onRegisterAfterWorldMatrixUpdate(connectedMesh);
    ...
    public static Parse(
      ...
      if (parsedSound.isDirectional) {
        newSound.setDirectionalCone(
          parsedSound.coneInnerAngle || 360,
          parsedSound.coneOuterAngle || 360,
          parsedSound.coneOuterGain || 0
        );
        if (parsedSound.localDirectionToMesh) {
          var localDirectionToMesh = Vector3.FromArray(
            parsedSound.localDirectionToMesh
          );
