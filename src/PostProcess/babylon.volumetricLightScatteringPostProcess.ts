namespace BABYLON {
  ...
  export class VolumetricLightScatteringPostProcess extends PostProcess {
     ...
     * @type {Vector3}
     */
    @serializeAsVector3() public customMeshPosition: Vector3 = Vector3.Zero();

    /**
     * Set if the post-process should use a custom position for the light source (true) or the internal mesh position (false)
     * @type {boolean}
     */
    @serialize() public useCustomMeshPosition: boolean = false;

    /**
     * If the post-process should inverse the light scattering direction
     * @type {boolean}
     */
    @serialize() public invert: boolean = true;

    /**
     * The internal mesh used by the post-process
     * @type {boolean}
     */
    @serializeAsMeshReference() public mesh: Mesh;
     ...
     */
    @serialize() public excludedMeshes = new Array<AbstractMesh>();

    /**
     * Controls the overall intensity of the post-process
     * @type {number}
     */
    @serialize() public exposure = 0.3;

    /**
     * Dissipates each sample's contribution in range [0, 1]
     * @type {number}
     */
    @serialize() public decay = 0.96815;

    /**
     * Controls the overall intensity of each sample
     * @type {number}
     */
    @serialize() public weight = 0.58767;

    /**
     * Controls the density of each sample
     * @type {number}
     */
    @serialize() public density = 0.926;
    ...
    constructor(
      ...
      this._viewPort = new Viewport(0, 0, 1, 1).toGlobal(
      ...
      this.onApplyObservable.add((effect: Effect) => {
        ...
        effect.setTexture(
          "lightScatteringSampler",
          this._volumetricLightScatteringRTT
        );
    ...
    private _isReady(subMesh: SubMesh, useInstances: boolean): boolean {
      ...
      if (this._cachedDefines !== join) {
        ...
        this._volumetricLightScatteringPass = mesh
          .getScene()
          .getEngine()
          .createEffect(
            {
    ...
    private _meshExcluded(mesh: AbstractMesh) {
      if (
        ...
        this.excludedMeshes.indexOf(mesh) !== -1
      ) {
        return true;
      }
      ...
      var renderSubMesh = (subMesh: SubMesh): void => {
        ...
        if (this._isReady(subMesh, hardwareInstancedRendering)) {
          ...
          if (mesh === this.mesh) {
            ...
            this._volumetricLightScatteringPass.setMatrix(
              "viewProjection",
            ...
            if (material && material.needAlphaTesting()) {
              ...
              this._volumetricLightScatteringPass.setTexture(
                "diffuseSampler",
                alphaTexture
              );
            ...
            if (
              ...
              this._volumetricLightScatteringPass.setMatrices(
                "mBones",
        ...
        if (transparentSubMeshes.length) {
          ...
          for (index = 0; index < transparentSubMeshes.length; index++) {
            ...
            if (boundingInfo && scene.activeCamera) {
              ...
              submesh._distanceToCamera = boundingInfo.boundingSphere.centerWorld
                .subtract(scene.activeCamera.position)
                .length();
            }
          }

          var sortedArray = transparentSubMeshes.data.slice(
            0,
            transparentSubMeshes.length
          );
