  ...
  export class BlurPostProcess extends PostProcess {
    ...
    constructor(
      ...
      this.onApplyObservable.add((effect: Effect) => {
        effect.setFloat2(
          "delta",
          1 / this.width * this.direction.x,
          1 / this.height * this.direction.y
        );
      });
    ...
    protected _updateParameters(): void {
      ...
      for (let i = 0; i <= centerIndex; i += 2) {
        ...
        if (singleCenterSample) {
          ...
          let weightLinear = weights[i] + weights[j] * (sharedCell ? 0.5 : 1);
      ...
      for (let i = 0; i < varyingCount; i++) {
        defines += `#define KERNEL_OFFSET${i} ${this._glslFloat(
          offsets[i]
        )}\r\n`;
        defines += `#define KERNEL_WEIGHT${i} ${this._glslFloat(
          weights[i]
        )}\r\n`;
      }
      ...
      for (let i = freeVaryingVec2; i < offsets.length; i++) {
        defines += `#define KERNEL_DEP_OFFSET${depCount} ${this._glslFloat(
          offsets[i]
        )}\r\n`;
        defines += `#define KERNEL_DEP_WEIGHT${depCount} ${this._glslFloat(
          weights[i]
        )}\r\n`;
        depCount++;
      }
