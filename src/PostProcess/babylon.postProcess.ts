  ...
  export class PostProcess {
    ...
    public get texelSize(): Vector2 {
      ...
      if (this._forcedOutputTexture) {
        this._texelSize.copyFromFloats(
          1.0 / this._forcedOutputTexture.width,
          1.0 / this._forcedOutputTexture.height
        );
      }
    ...
    constructor(
      ...
      samplingMode: number = Texture.NEAREST_SAMPLINGMODE,
      engine?: Engine,
      reusable?: boolean,
    ...
    public activate(
      ...
      var requiredWidth =
        ((sourceTexture
          ? sourceTexture.width
          : this._engine.getRenderWidth(true)) *
          <number>this._options) |
        0;
      var requiredHeight =
        ((sourceTexture
          ? sourceTexture.height
          : this._engine.getRenderHeight(true)) *
          <number>this._options) |
        0;

      var desiredWidth =
        (<PostProcessOptions>this._options).width || requiredWidth;
      var desiredHeight =
        (<PostProcessOptions>this._options).height || requiredHeight;
      ...
      if (!this._shareOutputWithPostProcess && !this._forcedOutputTexture) {
        ...
        if (
          this.renderTargetSamplingMode === Texture.TRILINEAR_SAMPLINGMODE ||
          this.alwaysForcePOT
        ) {
          if (!(<PostProcessOptions>this._options).width) {
            desiredWidth = engine.needPOTTextures
              ? Tools.GetExponentOfTwo(desiredWidth, maxSize, this.scaleMode)
              : desiredWidth;
          }

          if (!(<PostProcessOptions>this._options).height) {
            desiredHeight = engine.needPOTTextures
              ? Tools.GetExponentOfTwo(desiredHeight, maxSize, this.scaleMode)
              : desiredHeight;
          }
        }
        ...
        if (this.width !== desiredWidth || this.height !== desiredHeight) {
          ...
          let textureOptions = {
            ...
            generateDepthBuffer:
              forceDepthStencil || camera._postProcesses.indexOf(this) === 0,
            generateStencilBuffer:
              (forceDepthStencil ||
                camera._postProcesses.indexOf(this) === 0) &&
          ...
          this._textures.push(
            this._engine.createRenderTargetTexture(textureSize, textureOptions)
          );
        ...
        this._textures.forEach(texture => {
          if (texture.samples !== this.samples) {
            this._engine.updateRenderTargetTextureSampleCount(
              texture,
              this.samples
            );
          }
      ...
      if (this.enablePixelPerfectMode) {
        this._scaleRatio.copyFromFloats(
          requiredWidth / desiredWidth,
      ...
      if (this.autoClear && this.alphaMode === Engine.ALPHA_DISABLE) {
        this._engine.clear(
          this.clearColor ? this.clearColor : scene.clearColor,
          true,
          true,
          true
        );
    ...
    public apply(): Nullable<Effect> {
      ...
      if (this.alphaConstants) {
        this.getEngine().setAlphaConstants(
          this.alphaConstants.r,
          this.alphaConstants.g,
          this.alphaConstants.b,
          this.alphaConstants.a
        );
      }
