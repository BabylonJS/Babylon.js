namespace BABYLON {
  export class PostProcessRenderEffect {
    ...
    constructor(
      engine: Engine,
      name: string,
      getPostProcess: () => Nullable<PostProcess>,
    ...
    public _attachCameras(cameras: any): void {
      ...
      for (var i = 0; i < cams.length; i++) {
        ...
        this._postProcesses[cameraKey] =
          this._postProcesses[cameraKey] || this._getPostProcess();
    ...
    public _detachCameras(cameras: any): void {
      ...
      for (var i = 0; i < cams.length; i++) {
        ...
        camera.detachPostProcess(
          this._postProcesses[this._singleInstance ? 0 : cameraName]
        );
    ...
    public _enable(cameras: any): void {
      ...
      for (var i = 0; i < cams.length; i++) {
        ...
        for (var j = 0; j < this._indicesForCamera[cameraName].length; j++) {
          if (
            camera._postProcesses[this._indicesForCamera[cameraName][j]] ===
            undefined
          ) {
            cameras[i].attachPostProcess(
              this._postProcesses[this._singleInstance ? 0 : cameraName],
              this._indicesForCamera[cameraName][j]
            );
          }
        }
    ...
    private _linkParameters(): void {
      for (var index in this._postProcesses) {
        ...
        this._postProcesses[index].onBeforeRenderObservable.add(
          (effect: Effect) => {
    ...
    private _linkTextures(effect: Effect): void {
      for (var renderPassName in this._renderPasses) {
        effect.setTexture(
          renderPassName,
          this._renderPasses[renderPassName].getRenderTexture()
        );
      }

      for (var renderEffectName in this._renderEffectAsPasses) {
        effect.setTextureFromPostProcess(
          renderEffectName + "Sampler",
          this._renderEffectAsPasses[renderEffectName].getPostProcess()
        );
      }
    }
  }
