...
namespace BABYLON {
  export class LensRenderingPipeline extends PostProcessRenderPipeline {
    ...
    constructor(
      ...
      this._chromaticAberration = parameters.chromatic_aberration
        ? parameters.chromatic_aberration
        : 0;
      this._distortion = parameters.distortion ? parameters.distortion : 0;
      this._highlightsGain =
        parameters.dof_gain !== undefined ? parameters.dof_gain : -1;
      this._highlightsThreshold = parameters.dof_threshold
        ? parameters.dof_threshold
        : 1;
      this._dofDistance =
        parameters.dof_focus_distance !== undefined
          ? parameters.dof_focus_distance
          : -1;
      ...
      this._dofPentagon =
        parameters.dof_pentagon !== undefined ? parameters.dof_pentagon : true;
      this._blurNoise =
        parameters.blur_noise !== undefined ? parameters.blur_noise : true;
    ...
    private _createDepthOfFieldPostProcess(ratio: number): void {
      ...
      this._depthOfFieldPostProcess.onApply = (effect: Effect) => {
        ...
        effect.setTextureFromPostProcess(
          "textureSampler",
          this._highlightsPostProcess
        );
        effect.setTextureFromPostProcess(
          "highlightsSampler",
          this._depthOfFieldPostProcess
        );
    ...
    private _createGrainTexture(): void {
      ...
      this._grainTexture = new DynamicTexture(
        "LensNoiseTexture",
        size,
        this._scene,
        false,
