namespace BABYLON {
  export class SSAO2RenderingPipeline extends PostProcessRenderPipeline {
     ...
     * @type {number}
     */
    @serialize() public totalStrength: number = 1.0;

    /**
     * Maximum depth value to still render AO. A smooth falloff makes the dimming more natural, so there will be no abrupt shading change.
     * @type {number}
     */
    @serialize() public maxZ: number = 100.0;

    /**
     * In order to save performances, SSAO radius is clamped on close geometry. This ratio changes by how much
     * @type {number}
     */
    @serialize() public minZAspect: number = 0.2;

    /**
     * Number of samples used for the SSAO calculations. Default value is 8
     * @type {number}
     */
    @serialize("samples") private _samples: number = 8;
    ...
    public set samples(n: number) {
      this._ssaoPostProcess.updateEffect(
        "#define SAMPLES " + n + "\n#define SSAO"
      );
      this._samples = n;
     ...
     * @type {boolean}
     */
    @serialize("expensiveBlur") private _expensiveBlur: boolean = true;
     ...
     * @type {number}
     */
    @serialize() public radius: number = 2.0;
     ...
     * @type {number}
     */
    @serialize() public base: number = 0.1;
    ...
    private _createBlurPostProcess(ssaoRatio: number, blurRatio: number): void {
      ...
      this._blurHPostProcess.onApply = (effect: Effect) => {
        ...
        effect.setFloat(
          "outSize",
          this._ssaoCombinePostProcess.width > 0
            ? this._ssaoCombinePostProcess.width
            : this._originalColorPostProcess.width
        );
      ...
      this._blurVPostProcess.onApply = (effect: Effect) => {
        ...
        effect.setFloat(
          "outSize",
          this._ssaoCombinePostProcess.height > 0
            ? this._ssaoCombinePostProcess.height
            : this._originalColorPostProcess.height
        );
        effect.setFloat("near", this._scene.activeCamera.minZ);
        effect.setFloat("far", this._scene.activeCamera.maxZ);
        effect.setFloat("radius", this.radius);
    ...
    private _generateHemisphere(): number[] {
      ...
      var i = 0;
      while (i < numSamples) {
        vector = new Vector3(rand(-1.0, 1.0), rand(-1.0, 1.0), rand(0.3, 1.0));
        vector.normalize();
        scale = i / numSamples;
    ...
    private _createSSAOPostProcess(ratio: number): void {
      ...
      this._ssaoPostProcess.onApply = (effect: Effect) => {
        ...
        effect.setFloat2(
          "texelSize",
          1 / this._ssaoPostProcess.width,
          1 / this._ssaoPostProcess.height
        );
        effect.setFloat("radius", this.radius);
        ...
        effect.setFloat(
          "xViewport",
          Math.tan(this._scene.activeCamera.fov / 2) *
            this._scene
              .getEngine()
              .getAspectRatio(this._scene.activeCamera, true)
        );
        effect.setFloat(
          "yViewport",
          Math.tan(this._scene.activeCamera.fov / 2)
