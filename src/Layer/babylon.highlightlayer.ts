namespace BABYLON {
  ...
  export class HighlightLayer {
    ...
    private _excludedMeshes: Nullable<{
    ...
    constructor(
      ...
      this._options.alphaBlendingMode =
    ...
    private createTextureAndPostProcesses(): void {
      var blurTextureWidth =
        this._mainTextureDesiredSize.width * this._options.blurTextureSizeRatio;
      var blurTextureHeight =
        this._mainTextureDesiredSize.height *
        this._options.blurTextureSizeRatio;
      blurTextureWidth = this._engine.needPOTTextures
        ? Tools.GetExponentOfTwo(blurTextureWidth, this._maxSize)
        : blurTextureWidth;
      blurTextureHeight = this._engine.needPOTTextures
        ? Tools.GetExponentOfTwo(blurTextureHeight, this._maxSize)
        : blurTextureHeight;
      ...
      var renderSubMesh = (subMesh: SubMesh): void => {
        ...
        if (
          this._isReady(subMesh, hardwareInstancedRendering, emissiveTexture)
        ) {
          engine.enableEffect(this._glowMapGenerationEffect);
          mesh._bind(
            subMesh,
            this._glowMapGenerationEffect,
            Material.TriangleFillMode
          );

          this._glowMapGenerationEffect.setMatrix(
            "viewProjection",
            scene.getTransformMatrix()
          );
          if (highlightLayerMesh) {
            this._glowMapGenerationEffect.setFloat4(
              "color",
          ...
          if (material && material.needAlphaTesting()) {
            ...
            if (alphaTexture) {
              this._glowMapGenerationEffect.setTexture(
                "diffuseSampler",
                alphaTexture
              );
          ...
          if (mesh.useBones && mesh.computeBonesUsingShaders && mesh.skeleton) {
            this._glowMapGenerationEffect.setMatrices(
              "mBones",
          ...
          mesh._processRendering(
            ...
            (isInstance, world) =>
              this._glowMapGenerationEffect.setMatrix("world", world)
          );
        } else {
    ...
    private _isReady(
      ...
      emissiveTexture: Nullable<Texture>
    ): boolean {
      let material = subMesh.getMaterial();
      ...
      if (this._cachedDefines !== join) {
        ...
        this._glowMapGenerationEffect = this._scene
          .getEngine()
          .createEffect(
            "glowMapGeneration",
            attribs,
            ...
            ["diffuseSampler", "emissiveSampler"],
            join
          );
      }
    ...
    public render(): void {
      ...
      // Check
      if (!currentEffect.isReady() || !this._blurTexture.isReady()) return;
      ...
      engine.setStencilFunctionReference(
        this._instanceGlowingMeshStencilReference
      );
      ...
      this.setMainTextureSize();
      if (
        size.width !== this._mainTextureDesiredSize.width ||
        size.height !== this._mainTextureDesiredSize.height
      ) {
    ...
    public addMesh(mesh: Mesh, color: Color3, glowEmissiveOnly = false) {
      ...
      if (meshHighlight) {
        ...
        this._meshes[mesh.uniqueId] = {
          ...
          observerDefault: mesh.onAfterRenderObservable.add(
            this.defaultStencilReference
          ),
    ...
    private setMainTextureSize(): void {
      if (this._options.mainTextureFixedSize) {
        ...
        this._mainTextureDesiredSize.width =
          this._engine.getRenderWidth() * this._options.mainTextureRatio;
        this._mainTextureDesiredSize.height =
          this._engine.getRenderHeight() * this._options.mainTextureRatio;
    ...
    private defaultStencilReference(mesh: Mesh) {
      mesh
        .getScene()
        .getEngine()
        .setStencilFunctionReference(HighlightLayer.normalMeshStencilReference);
    }
