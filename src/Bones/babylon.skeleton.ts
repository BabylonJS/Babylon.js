namespace BABYLON {
  export class Skeleton implements IAnimatable {
    ...
    public toString(fullDetails?: boolean): string {
      ...
      ret += `, nAnimationRanges: ${
        this._ranges ? Object.keys(this._ranges).length : "none"
      }`;
      if (fullDetails) {
        ret += ", Ranges: {";
        let first = true;
    ...
    public getBoneIndexByName(name: string): number {
      for (
        var boneIndex = 0, cache = this.bones.length;
        boneIndex < cache;
        boneIndex++
      ) {
    ...
    public copyAnimationRange(
      ...
      if (this.bones.length !== sourceBones.length) {
        Tools.Warn(
          `copyAnimationRange: this rig has ${
            this.bones.length
          } bones, while source as ${sourceBones.length}`
        );
        ret = false;
      }

      var skelDimensionsRatio =
        rescaleAsRequired && this.dimensionsAtRest && source.dimensionsAtRest
          ? this.dimensionsAtRest.divide(source.dimensionsAtRest)
          : null;
      ...
      for (i = 0, nBones = this.bones.length; i < nBones; i++) {
        ...
        if (sourceBone) {
          ret =
            ret &&
            this.bones[i].copyAnimationRange(
              sourceBone,
              name,
              frameOffset,
              rescaleAsRequired,
          ...
          Tools.Warn(
            "copyAnimationRange: not same rig, missing source bone " + boneName
          );
          ret = false;
        }
      }
      ...
      if (range) {
        this._ranges[name] = new AnimationRange(
          name,
          range.from + frameOffset,
    ...
    public beginAnimation(
      ...
      return this._scene.beginAnimation(
        this,
        range.from,
        range.to,
        loop,
    ...
    public _computeTransformMatrices(
      targetMatrix: Float32Array,
      initialSkinMatrix: Nullable<Matrix>
    ): void {
      ...
      for (var index = 0; index < this.bones.length; index++) {
        ...
        if (parentBone) {
          bone
            .getLocalMatrix()
            .multiplyToRef(parentBone.getWorldMatrix(), bone.getWorldMatrix());
        } else {
          if (initialSkinMatrix) {
            bone
              .getLocalMatrix()
              .multiplyToRef(initialSkinMatrix, bone.getWorldMatrix());
          } else {
    ...
    public prepare(): void {
      ...
      if (this.needInitialSkinMatrix) {
        for (
          var index = 0;
          index < this._meshesWithPoseMatrix.length;
          index++
        ) {
          ...
          if (
            ...
            mesh._bonesTransformMatrices.length !== 16 * (this.bones.length + 1)
          ) {
            mesh._bonesTransformMatrices = new Float32Array(
              16 * (this.bones.length + 1)
            );
          }
        ...
        if (
          ...
          this._transformMatrices.length !== 16 * (this.bones.length + 1)
        ) {
          this._transformMatrices = new Float32Array(
            16 * (this.bones.length + 1)
          );
        }
    ...
    public getAnimatables(): IAnimatable[] {
      if (
        !this._animatables ||
        this._animatables.length !== this.bones.length
      ) {
        this._animatables = [];
    ...
    public clone(name: string, id: string): Skeleton {
      ...
      for (var index = 0; index < this.bones.length; index++) {
        ...
        var bone = new Bone(
          ...
          result,
          parentBone,
          source.getBaseMatrix().clone(),
          source.getRestPose().clone()
        );
    ...
    public static Parse(parsedSkeleton: any, scene: Scene): Skeleton {
      ...
      if (parsedSkeleton.dimensionsAtRest) {
        skeleton.dimensionsAtRest = Vector3.FromArray(
          parsedSkeleton.dimensionsAtRest
        );
      }
      ...
      for (index = 0; index < parsedSkeleton.bones.length; index++) {
        ...
        var rest: Nullable<Matrix> = parsedBone.rest
          ? Matrix.FromArray(parsedBone.rest)
          : null;
        var bone = new Bone(
          parsedBone.name,
          skeleton,
          parentBone,
          Matrix.FromArray(parsedBone.matrix),
          rest
        );
