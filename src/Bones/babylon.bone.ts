namespace BABYLON {
  export class Bone extends Node {
    ...
    private static _tmpMats: Matrix[] = [
      Matrix.Identity(),
      Matrix.Identity(),
      Matrix.Identity(),
      Matrix.Identity(),
    ...
    public setParent(
      ...
      updateDifferenceMatrix: boolean = true
    ): void {
    ...
    public _updateDifferenceMatrix(rootMatrix?: Matrix): void {
      ...
      if (this._parent) {
        rootMatrix.multiplyToRef(
          this._parent._absoluteTransform,
      ...
      this._scalingDeterminant =
        this._absoluteTransform.determinant() < 0 ? -1 : 1;
    }
    ...
    public copyAnimationRange(
      ...
      if (this.animations.length === 0) {
        this.animations.push(
          new Animation(
            this.name,
            "_matrix",
            source.animations[0].framePerSecond,
      ...
      var parentScalingReqd =
        rescaleAsRequired &&
        sourceParent &&
        sourceBoneLength &&
        this.length &&
        sourceBoneLength !== this.length;
      var parentRatio =
        parentScalingReqd && parent && sourceParent
          ? parent.length / sourceParent.length
          : 1;

      var dimensionsScalingReqd =
        rescaleAsRequired &&
        !parent &&
        skelDimensionsRatio &&
        (skelDimensionsRatio.x !== 1 ||
          skelDimensionsRatio.y !== 1 ||
          skelDimensionsRatio.z !== 1);
    ...
    public setScale(
      ...
      if (
        this.animations[0] &&
        !this.animations[0].hasRunningRuntimeAnimations
      ) {
        if (!scaleChildren) {
      ...
      this.scale(
        x / this._scaleVector.x,
        y / this._scaleVector.y,
        z / this._scaleVector.z,
        scaleChildren
      );
    }
    ...
    public scale(x: number, y: number, z: number, scaleChildren = false): void {
      ...
      if (parent) {
        locMat.multiplyToRef(
          parent.getAbsoluteTransform(),
        ...
        if (space == Space.WORLD) {
          if (mesh) {
            ...
            parent
              ...
              .multiplyToRef(parentScale, parentScale);
          } else {
    ...
    public getPosition(
      space = Space.LOCAL,
      mesh: Nullable<AbstractMesh> = null
    ): Vector3 {
     ...
     */
    public getRotationQuaternionToRef(
      space = Space.LOCAL,
      mesh: Nullable<AbstractMesh> = null,
      result: Quaternion
    ): void {
      if (space == Space.LOCAL) {
        this.getLocalMatrix().decompose(
          Bone._tmpVecs[0],
          result,
          Bone._tmpVecs[1]
        );
     ...
     */
    public getAbsolutePositionFromLocal(
      position: Vector3,
      mesh: Nullable<AbstractMesh> = null
    ): Vector3 {
      var result = Vector3.Zero();
     ...
     */
    public getAbsolutePositionFromLocalToRef(
      position: Vector3,
      mesh: Nullable<AbstractMesh> = null,
      result: Vector3
    ): void {
     ...
     */
    public getLocalPositionFromAbsolute(
      position: Vector3,
      mesh: Nullable<AbstractMesh> = null
    ): Vector3 {
      var result = Vector3.Zero();
     ...
     */
    public getLocalPositionFromAbsoluteToRef(
      position: Vector3,
      mesh: Nullable<AbstractMesh> = null,
      result: Vector3
    ): void {
