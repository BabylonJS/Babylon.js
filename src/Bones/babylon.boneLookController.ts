  ...
  export class BoneLookController {
    ...
    set minYaw(value: number) {
      ...
      if (this._maxYaw != null) {
        this._midYawConstraint =
          this._getAngleDiff(this._minYaw, this._maxYaw) * 0.5 + this._minYaw;
    ...
    public update(): void {
      ...
      if (checkYaw || checkPitch) {
        ...
        if (checkPitch) {
          ...
          xzlen = Math.sqrt(
            localTarget.x * localTarget.x + localTarget.z * localTarget.z
          );
        ...
        if (checkYaw) {
          ...
          if (yaw > this._maxYaw || yaw < this._minYaw) {
            ...
            if (this._yawRange > Math.PI) {
              if (
                this._isAngleBetween(yaw, this._maxYaw, this._midYawConstraint)
              ) {
                localTarget.z = this._maxYawCos * xzlen;
                localTarget.x = this._maxYawSin * xzlen;
                newYaw = this._maxYaw;
              } else if (
                this._isAngleBetween(yaw, this._midYawConstraint, this._minYaw)
              ) {
          ...
          if (this._slerping && this._yawRange > Math.PI) {
            ...
            var angBtwMidYaw = this._getAngleBetween(
              boneYaw,
              this._midYawConstraint
            );
      ...
      if (this.slerpAmount < 1) {
        if (!this._slerping) {
          this.bone.getRotationQuaternionToRef(
            Space.WORLD,
            this.mesh,
            this._boneQuat
          );
