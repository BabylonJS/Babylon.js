namespace BABYLON {
  ...
  export class EngineCapabilities {
    ...
    public textureAnisotropicFilterExtension: Nullable<
  ...
  export class Engine {
     ...
     */
    public static MarkAllMaterialsAsDirty(
      flag: number,
      predicate?: (mat: Material) => boolean
    ): void {
      for (
        var engineIndex = 0;
        engineIndex < Engine.Instances.length;
        engineIndex++
      ) {
        ...
        for (
          var sceneIndex = 0;
          sceneIndex < engine.scenes.length;
          sceneIndex++
        ) {
    ...
    public onVRDisplayChangedObservable = new Observable<
    ...
    public get emptyTexture(): InternalTexture {
      if (!this._emptyTexture) {
        this._emptyTexture = this.createRawTexture(
          new Uint8Array(4),
          1,
          1,
    ...
    public get emptyTexture3D(): InternalTexture {
      if (!this._emptyTexture3D) {
        this._emptyTexture3D = this.createRawTexture3D(
          new Uint8Array(4),
          1,
          1,
          1,
    ...
    constructor(
      ...
      if ((<HTMLCanvasElement>canvasOrContext).getContext) {
        ...
        if (!options.disableWebGL2Support) {
          try {
            this._gl = <any>(canvas.getContext("webgl2", options) ||
        ...
        if (!this._gl) {
          ...
          try {
            this._gl = <WebGLRenderingContext>(canvas.getContext(
              "webgl",
              options
      ...
      // Viewport
      var limitDeviceRatio =
        options.limitDeviceRatio || window.devicePixelRatio || 1.0;
      this._hardwareScalingLevel = adaptToDeviceRatio
        ? 1.0 / Math.min(limitDeviceRatio, window.devicePixelRatio || 1.0)
        : 1.0;
      this.resize();
      ...
      this._linkTrackers(
        this._firstBoundInternalTextureTracker,
        this._lastBoundInternalTextureTracker
      );
      ...
      this._badDesktopOS = /^((?!chrome|android).)*safari/i.test(
        navigator.userAgent
      );
    ...
    private _initGLContext(): void {
      ...
      this._caps.maxTexturesImageUnits = this._gl.getParameter(
        this._gl.MAX_TEXTURE_IMAGE_UNITS
      );
      this._caps.maxCombinedTexturesImageUnits = this._gl.getParameter(
        this._gl.MAX_COMBINED_TEXTURE_IMAGE_UNITS
      );
      this._caps.maxVertexTextureImageUnits = this._gl.getParameter(
        this._gl.MAX_VERTEX_TEXTURE_IMAGE_UNITS
      );
      this._caps.maxTextureSize = this._gl.getParameter(
        this._gl.MAX_TEXTURE_SIZE
      );
      this._caps.maxCubemapTextureSize = this._gl.getParameter(
        this._gl.MAX_CUBE_MAP_TEXTURE_SIZE
      );
      this._caps.maxRenderTextureSize = this._gl.getParameter(
        this._gl.MAX_RENDERBUFFER_SIZE
      );
      this._caps.maxVertexAttribs = this._gl.getParameter(
        this._gl.MAX_VERTEX_ATTRIBS
      );
      this._caps.maxVaryingVectors = this._gl.getParameter(
        this._gl.MAX_VARYING_VECTORS
      );
      this._caps.maxFragmentUniformVectors = this._gl.getParameter(
        this._gl.MAX_FRAGMENT_UNIFORM_VECTORS
      );
      this._caps.maxVertexUniformVectors = this._gl.getParameter(
      ...
      var rendererInfo: any = this._gl.getExtension(
        "WEBGL_debug_renderer_info"
      );
      if (rendererInfo != null) {
        this._glRenderer = this._gl.getParameter(
          rendererInfo.UNMASKED_RENDERER_WEBGL
        );
        this._glVendor = this._gl.getParameter(
      ...
      this._caps.maxAnisotropy = this._caps.textureAnisotropicFilterExtension
        ? this._gl.getParameter(
            this._caps.textureAnisotropicFilterExtension
              .MAX_TEXTURE_MAX_ANISOTROPY_EXT
          )
        : 0;
      this._caps.uintIndices =
        this._webGLVersion > 1 ||
        this._gl.getExtension("OES_element_index_uint") !== null;
      this._caps.fragmentDepthSupported =
        this._webGLVersion > 1 ||
      ...
      if (this._caps.timerQuery) {
        if (this._webGLVersion === 1) {
          this._gl.getQuery = (<any>this._caps.timerQuery).getQueryEXT.bind(
            this._caps.timerQuery
          );
        }
        this._caps.canUseTimestampForTimerQuery =
          this._gl.getQuery(
            this._caps.timerQuery.TIMESTAMP_EXT,
            this._caps.timerQuery.QUERY_COUNTER_BITS_EXT
          ) > 0;
      }

      // Checks if some of the format renders first to allow the use of webgl inspector.
      this._caps.colorBufferFloat =
        this._webGLVersion > 1 &&
      ...
      this._caps.textureFloatRender =
        this._caps.textureFloat && this._canRenderToFloatFramebuffer();
      ...
      if (this._webGLVersion > 1) {
        ...
        if (drawBuffersExtension !== null) {
          this._caps.drawBuffersExtension = true;
          this._gl.drawBuffers = drawBuffersExtension.drawBuffersWEBGL.bind(
            drawBuffersExtension
          );
          this._gl.DRAW_FRAMEBUFFER = this._gl.FRAMEBUFFER;

          for (var i = 0; i < 16; i++) {
            (<any>this._gl)["COLOR_ATTACHMENT" + i + "_WEBGL"] =
              drawBuffersExtension["COLOR_ATTACHMENT" + i + "_WEBGL"];
          }
        } else {
      ...
      if (this._webGLVersion > 1) {
        ...
        var depthTextureExtension = this._gl.getExtension(
          "WEBGL_depth_texture"
        );
      ...
      if (this._webGLVersion > 1) {
        ...
        var vertexArrayObjectExtension = this._gl.getExtension(
          "OES_vertex_array_object"
        );

        if (vertexArrayObjectExtension != null) {
          this._caps.vertexArrayObject = true;
          this._gl.createVertexArray = vertexArrayObjectExtension.createVertexArrayOES.bind(
            vertexArrayObjectExtension
          );
          this._gl.bindVertexArray = vertexArrayObjectExtension.bindVertexArrayOES.bind(
            vertexArrayObjectExtension
          );
          this._gl.deleteVertexArray = vertexArrayObjectExtension.deleteVertexArrayOES.bind(
            vertexArrayObjectExtension
          );
        } else {
      ...
      if (this._webGLVersion > 1) {
        ...
        var instanceExtension = <ANGLE_instanced_arrays>this._gl.getExtension(
          "ANGLE_instanced_arrays"
        );

        if (instanceExtension != null) {
          this._caps.instancedArrays = true;
          this._gl.drawArraysInstanced = instanceExtension.drawArraysInstancedANGLE.bind(
            instanceExtension
          );
          this._gl.drawElementsInstanced = instanceExtension.drawElementsInstancedANGLE.bind(
            instanceExtension
          );
          this._gl.vertexAttribDivisor = instanceExtension.vertexAttribDivisorANGLE.bind(
            instanceExtension
          );
        } else {
      ...
      if (this._gl.getShaderPrecisionFormat) {
        var highp = this._gl.getShaderPrecisionFormat(
    ...
    public getAspectRatio(camera: Camera, useScreen = false): number {
      ...
      return (
        this.getRenderWidth(useScreen) *
        viewport.width /
        (this.getRenderHeight(useScreen) * viewport.height)
      );
    }
    ...
    public get drawCalls(): number {
      Tools.Warn(
        "drawCalls is deprecated. Please use SceneInstrumentation class"
      );
      return 0;
    }
    ...
    public get drawCallsPerfCounter(): Nullable<PerfCounter> {
      Tools.Warn(
        "drawCallsPerfCounter is deprecated. Please use SceneInstrumentation class"
      );
      return null;
    }
    ...
    public _renderLoop(): void {
      ...
      if (this._activeRenderLoops.length > 0) {
        ...
        this._frameHandler = Tools.QueueNewFrame(
    ...
    public resize(): void {
      ...
      if (!(this._vrDisplay && this._vrDisplay.isPresenting)) {
        var width = this._renderingCanvas
          ? this._renderingCanvas.clientWidth
          : window.innerWidth;
        var height = this._renderingCanvas
          ? this._renderingCanvas.clientHeight
          : window.innerHeight;

        this.setSize(
          width / this._hardwareScalingLevel,
          height / this._hardwareScalingLevel
        );
      }
    }
    ...
    public unBindMultiColorAttachmentFramebuffer(
      ...
      if (textures[0]._MSAAFramebuffer) {
        ...
        for (var i = 0; i < textures.length; i++) {
          ...
          attachments[i] = (<any>gl)[
            this.webGLVersion > 1
              ? "COLOR_ATTACHMENT" + i
              : "COLOR_ATTACHMENT" + i + "_WEBGL"
          ];
    ...
    public createUniformBuffer(elements: FloatArray): WebGLBuffer {
      ...
      if (elements instanceof Float32Array) {
        ...
        this._gl.bufferData(
          this._gl.UNIFORM_BUFFER,
          new Float32Array(<number[]>elements),
          this._gl.STATIC_DRAW
        );
      }
      ...
      if (count === undefined) {
        ...
        if (elements instanceof Float32Array) {
          this._gl.bufferSubData(
            this._gl.UNIFORM_BUFFER,
            0,
            <Float32Array>elements.subarray(offset, offset + count)
          );
        } else {
          this._gl.bufferSubData(
            this._gl.UNIFORM_BUFFER,
            0,
            new Float32Array(<number[]>elements).subarray(
              offset,
              offset + count
            )
          );
    ...
    public createVertexBuffer(vertices: FloatArray): WebGLBuffer {
      ...
      if (vertices instanceof Float32Array) {
        ...
        this._gl.bufferData(
          this._gl.ARRAY_BUFFER,
          new Float32Array(<number[]>vertices),
          this._gl.STATIC_DRAW
        );
      }
    ...
    public updateDynamicIndexBuffer(
    ...
    public updateDynamicVertexBuffer(
      ...
      if (count === undefined) {
        ...
        if (vertices instanceof Float32Array) {
          this._gl.bufferSubData(
            this._gl.ARRAY_BUFFER,
            0,
            <Float32Array>vertices.subarray(offset, offset + count)
          );
        } else {
          this._gl.bufferSubData(
            this._gl.ARRAY_BUFFER,
            0,
            new Float32Array(<number[]>vertices).subarray(
              offset,
              offset + count
            )
          );
    ...
    public bindUniformBlock(
      shaderProgram: WebGLProgram,
      blockName: string,
      index: number
    ): void {
      var uniformLocation = this._gl.getUniformBlockIndex(
        shaderProgram,
        blockName
      );
        ...
        if (order >= 0) {
          ...
          if (buffer) {
            this.vertexAttribPointer(
              ...
              this._gl.FLOAT,
              false,
              vertexBuffer.getStrideSize() * 4,
              vertexBuffer.getOffset() * 4
            );
    ...
    public updateAndBindInstancesBuffer(
      ...
      if ((<any>offsetLocations[0]).index !== undefined) {
        ...
        for (let i = 0; i < offsetLocations.length; i++) {
          ...
          this.vertexAttribPointer(
            ...
            ai.index,
            ai.attributeSize,
            ai.attribyteType || this._gl.FLOAT,
            ai.normalized || false,
            stride,
            ai.offset
          );
    ...
    public draw(
      ...
      this.drawElementsType(
        useTriangles ? Material.TriangleFillMode : Material.WireFrameFillMode,
        indexStart,
        indexCount,
        instancesCount
      );
    ...
    public drawElementsType(
      ...
      var indexFormat = this._uintIndicesCurrentlySet
    ...
    public createEffect(
      ...
      var name =
        ...
        (defines
          ? defines
          : (<EffectCreationOptions>attributesNamesOrOptions).defines);
    ...
    public createEffectForParticles(
      fragmentName: string,
    ...
    public createShaderProgram(
      ...
      let program = this._createShaderProgram(
        vertexShader,
        fragmentShader,
        context,
        transformFeedbackVaryings
    ...
    private _createShaderProgram(
      ...
      if (this.webGLVersion > 1 && transformFeedbackVaryings) {
        ...
        this.setTranformFeedbackVaryings(
          shaderProgram,
          transformFeedbackVaryings
        );
      ...
      var linked = context.getProgramParameter(
        shaderProgram,
        context.LINK_STATUS
      );
    ...
    public getUniforms(
      ...
      for (var index = 0; index < uniformsNames.length; index++) {
        results.push(
          this._gl.getUniformLocation(shaderProgram, uniformsNames[index])
        );
      }
    ...
    public getAttributes(
      ...
      for (var index = 0; index < attributesNames.length; index++) {
        try {
          results.push(
            this._gl.getAttribLocation(shaderProgram, attributesNames[index])
          );
        } catch (e) {
          results.push(-1);
        }
    ...
    public setAlphaMode(
      mode: number,
      noDepthWriteChange: boolean = false
    ): void {
    ...
    public createTexture(
      ...
      onError: Nullable<(message: string, exception: any) => void> = null,
      buffer: Nullable<ArrayBuffer | HTMLImageElement> = null,
      ...
      if (isKTX || isTGA || isDDS) {
        if (isKTX) {
          ...
          callback = data => {
            ...
            var loadMipmap =
              (info.isRGB || info.isLuminance || info.mipmapCount > 1) &&
              !noMipmap &&
        ...
        if (!buffer) {
          this._loadFile(
            ...
            undefined,
            scene ? scene.database : undefined,
            true,
            (request?: XMLHttpRequest, exception?: any) => {
              onerror(
                "Unable to load " +
                  (request ? request.responseURL : url, exception)
              );
            }
          );
        } else {
        ...
        var onload = (img: HTMLImageElement) => {
          ...
          this._prepareWebGLTexture(
            ...
            noMipmap,
            false,
            (potWidth, potHeight, continuationCallback) => {
              let gl = this._gl;
              var isPot = img.width === potWidth && img.height === potHeight;
              let internalFormat = format
                ? this._getInternalFormat(format)
              ...
              let source = new InternalTexture(
                this,
                InternalTexture.DATASOURCE_TEMP
              );
        ...
        else onload(<HTMLImageElement>buffer);
      }
    ...
    private _rescaleTexture(
      ...
      if (!this._rescalePostProcess) {
        this._rescalePostProcess = new PassPostProcess(
          "rescale",
          1,
          null,
    ...
    public updateRawTexture(
      ...
      this._gl.pixelStorei(
        ...
        invertY === undefined ? 1 : invertY ? 1 : 0
      );
    ...
    public createRawTexture(
      ...
      // Filters
      var filters = getSamplingParameters(
        samplingMode,
        generateMipMaps,
        this._gl
      );
    ...
    public createDynamicTexture(
      ...
      var texture = new InternalTexture(
        this,
        InternalTexture.DATASOURCE_DYNAMIC
      );
      texture.baseWidth = width;
      texture.baseHeight = height;
      ...
      if (generateMipMaps) {
        width = this.needPOTTextures
          ? Tools.GetExponentOfTwo(width, this._caps.maxTextureSize)
          : width;
        height = this.needPOTTextures
          ? Tools.GetExponentOfTwo(height, this._caps.maxTextureSize)
          : height;
      }
    ...
    public updateTextureSamplingMode(
      samplingMode: number,
      texture: InternalTexture
    ): void {
      var filters = getSamplingParameters(
    ...
    public updateVideoTexture(
      ...
      try {
        ...
        if (!this._videoTextureSupported) {
          ...
          texture._workingContext.drawImage(
            video,
            0,
            0,
            video.videoWidth,
    ...
    public createRenderTargetTexture(
      ...
      if (options !== undefined && typeof options === "object") {
        ...
        fullOptions.generateStencilBuffer =
          fullOptions.generateDepthBuffer && options.generateStencilBuffer;
        fullOptions.type =
      ...
      if (
        fullOptions.type === Engine.TEXTURETYPE_FLOAT &&
        !this._caps.textureFloatLinearFiltering
      ) {
        // if floating point linear (gl.FLOAT) then force to NEAREST_SAMPLINGMODE
        fullOptions.samplingMode = Texture.NEAREST_SAMPLINGMODE;
      } else if (
        fullOptions.type === Engine.TEXTURETYPE_HALF_FLOAT &&
        !this._caps.textureHalfFloatLinearFiltering
      ) {
      ...
      var width =
        (<{ width: number; height: number }>size).width || <number>size;
      var height =
        (<{ width: number; height: number }>size).height || <number>size;
      ...
      gl.texImage2D(
        gl.TEXTURE_2D,
        0,
        this._getRGBABufferInternalSizedFormat(fullOptions.type),
        width,
        height,
        0,
        gl.RGBA,
      ...
      texture._depthStencilBuffer = this._setupFramebufferDepthAttachments(
    ...
    public createMultipleRenderTarget(
      size: any,
      options: any
    ): InternalTexture[] {
      ...
      var depthStencilBuffer = this._setupFramebufferDepthAttachments(
      ...
      for (var i = 0; i < textureCount; i++) {
        ...
        var texture = new InternalTexture(
          this,
          InternalTexture.DATASOURCE_MULTIRENDERTARGET
        );
        var attachment = (<any>gl)[
          this.webGLVersion > 1
            ? "COLOR_ATTACHMENT" + i
            : "COLOR_ATTACHMENT" + i + "_WEBGL"
        ];
      ...
      if (generateDepthTexture && this._caps.depthTextureExtension) {
        // Depth texture
        var depthTexture = new InternalTexture(
          this,
          InternalTexture.DATASOURCE_MULTIRENDERTARGET
        );
    ...
    public updateRenderTargetTextureSampleCount(
      texture: Nullable<InternalTexture>,
      samples: number
    ): number {
    ...
    public updateMultipleRenderTargetTextureSampleCount(
      ...
      if (samples > 1) {
        ...
        let depthStencilBuffer = this._setupFramebufferDepthAttachments(
          textures[0]._generateStencilBuffer,
          textures[0]._generateDepthBuffer,
          textures[0].width,
          textures[0].height,
          samples
        );
    ...
    public _uploadCompressedDataToTexture(
      target: number,
      lod: number,
      internalFormat: number,
      width: number,
    ...
    public createRenderTargetCubeTexture(
      size: number,
    ...
    public createPrefilteredCubeTexture(
      rootUrl: string,
      scene: Nullable<Scene>,
      scale: number,
      offset: number,
      onLoad: Nullable<
        (internalTexture: Nullable<InternalTexture>) => void
      > = null,
      onError: Nullable<(message?: string, exception?: any) => void> = null,
      format?: number,
      ...
      var callback = (loadData: any) => {
        ...
        for (let i = 0; i < mipSlices; i++) {
          ...
          let mipmapIndex = Math.round(
            Math.min(Math.max(lodIndex, 0), maxLODIndex)
          );

          var glTextureFromLod = new InternalTexture(
            this,
            InternalTexture.DATASOURCE_TEMP
          );
          ...
          if (loadData.isDDS) {
            ...
            Tools.Warn(
              "DDS is the only prefiltered cube map supported so far."
            );
          }
    ...
    public createCubeTexture(
      ...
      if (this._textureFormatInUse) {
        ...
        rootUrl =
          (lastDot > -1 ? rootUrl.substring(0, lastDot) : rootUrl) +
      ...
      if (isKTX) {
        ...
        cascadeLoadImgs(
          ...
          scene,
          imgs => {
            var width = this.needPOTTextures
              ? Tools.GetExponentOfTwo(
                  imgs[0].width,
                  this._caps.maxCubemapTextureSize
                )
              : imgs[0].width;
            var height = width;
            ...
            for (var index = 0; index < faces.length; index++) {
              this._workingContext.drawImage(
                imgs[index],
                0,
                0,
                imgs[index].width,
    ...
    private setCubeMapTextureParams(
      gl: WebGLRenderingContext,
      loadMipmap: boolean
    ) {
    ...
    public updateRawCubeTexture(
      ...
      for (let faceIndex = 0; faceIndex < 6; faceIndex++) {
        ...
        if (compression) {
          ...
          if (needConversion) {
            faceData = this._convertRGBtoRGBATextureData(
              faceData,
              texture.width,
              texture.height,
              type
    ...
    public createRawCubeTexture(
      ...
      if (textureType === gl.FLOAT && !this._caps.textureFloatLinearFiltering) {
        ...
        textureType === this._gl.HALF_FLOAT_OES &&
        !this._caps.textureHalfFloatLinearFiltering
      ) {
        gl.texParameteri(
          gl.TEXTURE_CUBE_MAP,
    ...
    public createRawCubeTextureFromUrl(
      url: string,
      scene: Scene,
      size: number,
      format: number,
      ...
      mipmmapGenerator: Nullable<
        ((faces: ArrayBufferView[]) => ArrayBufferView[][])
      >,
      onLoad: Nullable<() => void> = null,
      onError: Nullable<(message?: string, exception?: any) => void> = null,
      ...
      var gl = this._gl;
      var texture = this.createRawCubeTexture(
        null,
        size,
        format,
        type,
        ...
        if (mipmmapGenerator) {
          ...
          for (var level = 0; level < mipData.length; level++) {
            ...
            for (var faceIndex = 0; faceIndex < 6; faceIndex++) {
              ...
              if (needConversion) {
                mipFaceData = this._convertRGBtoRGBATextureData(
                  mipFaceData,
                  mipSize,
                  mipSize,
                  type
    ...
    private _prepareWebGLTexture(
      ...
      var potWidth = this.needPOTTextures
        ? Tools.GetExponentOfTwo(width, this.getCaps().maxTextureSize)
        : width;
      var potHeight = this.needPOTTextures
        ? Tools.GetExponentOfTwo(height, this.getCaps().maxTextureSize)
        : height;
    ...
    private _moveBoundTextureOnTop(internalTexture: InternalTexture): void {
      ...
      // Bind last to it
      this._linkTrackers(
        this._lastBoundInternalTextureTracker.previous,
        internalTexture
      );
    ...
    private _getCorrectTextureChannel(
      ...
      if (this.disableTextureBindingOptimization) {
        // We want texture sampler ID === texture channel
        ...
        if (channel !== internalTexture._designatedSlot) {
          if (internalTexture._designatedSlot > -1) {
            // Texture is already assigned to a slot
            ...
            this._textureCollisions.addCount(1, false);
            return this._removeDesignatedSlot(<InternalTexture>this
    ...
    protected _bindTextureDirectly(
      target: number,
      texture: Nullable<InternalTexture>,
      ...
      if (currentTextureBound !== texture) {
        ...
        if (texture) {
          if (!this.disableTextureBindingOptimization) {
            let slotIndex = this._nextFreeTextureSlots.indexOf(
              this._activeChannel
            );
            if (slotIndex > -1) {
    ...
    public setTextureFromPostProcess(
      channel: number,
    ...
    public unbindAllTextures(): void {
      for (
        var channel = 0;
        channel < this._maxSimultaneousTextures;
        channel++
      ) {
        this._activeChannel = channel;
        this._bindTextureDirectly(this._gl.TEXTURE_2D, null);
    ...
    public setTextureArray(
      ...
      if (
        !this._textureUnits ||
        this._textureUnits.length !== textures.length
      ) {
        this._textureUnits = new Int32Array(textures.length);
      }
      for (let i = 0; i < textures.length; i++) {
        this._textureUnits[i] = this._getCorrectTextureChannel(
          channel + i,
          textures[i].getInternalTexture()
        );
      }
    ...
    public _setAnisotropicLevel(key: number, texture: BaseTexture) {
      ...
      var anisotropicFilterExtension = this._caps
        .textureAnisotropicFilterExtension;
      var value = texture.anisotropicFilteringLevel;

      if (
        internalTexture.samplingMode !== Texture.LINEAR_LINEAR_MIPNEAREST &&
     ...
     */
    public getOrAddExternalDataWithFactory<T>(
      key: string,
      factory: (k: string) => T
    ): T {
      if (!this._externalData) {
    ...
    public unbindAllAttributes() {
      ...
      for (
        var i = 0, ul = this._vertexAttribArraysEnabled.length;
        i < ul;
        i++
      ) {
        if (
    ...
    public attachContextRestoredEvent(
      callback: ((event: WebGLContextEvent) => void)
    ): void {
      if (this._renderingCanvas) {
        this._renderingCanvas.addEventListener(
    ...
    public _readTexturePixels(
      ...
      let readType =
        texture.type !== undefined
          ? this._getWebGLTextureType(texture.type)
          : gl.UNSIGNED_BYTE;
    ...
    public isQueryResultAvailable(query: WebGLQuery): boolean {
      return this._gl.getQueryParameter(
        query,
    ...
    private _getTimeQueryAvailability(query: WebGLQuery): any {
      ...
      if (timerQuery.getQueryObjectEXT) {
        return timerQuery.getQueryObjectEXT(
          query,
          timerQuery.QUERY_RESULT_AVAILABLE_EXT
        );
      }
    ...
    public startTimeQuery(): Nullable<_TimeToken> {
      ...
      if (this._caps.canUseTimestampForTimerQuery) {
        ...
        timerQuery.queryCounterEXT(
          token._startTimeQuery,
          timerQuery.TIMESTAMP_EXT
        );
      } else {
        ...
        if (timerQuery.beginQueryEXT) {
          timerQuery.beginQueryEXT(
            timerQuery.TIME_ELAPSED_EXT,
            token._timeElapsedQuery
          );
        } else {
          this._gl.beginQuery(
            timerQuery.TIME_ELAPSED_EXT,
    ...
    public beginTransformFeedback(usePoints: boolean = true): void {
      this._gl.beginTransformFeedback(
        usePoints ? this._gl.POINTS : this._gl.TRIANGLES
      );
    }
    ...
    public setTranformFeedbackVaryings(
    ...
    public _loadFile(
      ...
      onError?: (request?: XMLHttpRequest, exception?: any) => void
    ): IFileRequest {
      let request = Tools.LoadFile(
        url,
        onSuccess,
        onProgress,
        database,
    ...
    private _partialLoadFile(
      url: string,
      index: number,
      loadedFiles: (string | ArrayBuffer)[],
      scene: Nullable<Scene>,
      onfinish: (files: (string | ArrayBuffer)[]) => void,
      onErrorCallBack: Nullable<
        (message?: string, exception?: any) => void
      > = null
    ): void {
    ...
    private _cascadeLoadFiles(
      rootUrl: string,
      scene: Nullable<Scene>,
      onfinish: (images: (string | ArrayBuffer)[]) => void,
      files: string[],
      onError: Nullable<(message?: string, exception?: any) => void> = null
    ): void {
