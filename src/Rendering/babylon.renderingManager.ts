namespace BABYLON {
  ...
  export class RenderingManager {
    ...
    private _autoClearDepthStencil: {
      [id: number]: RenderingManageAutoClearOptions;
    } = {};
    ...
    public render(
      ...
      let observable = this._scene.onRenderingGroupObservable.hasObservers()
        ? this._scene.onRenderingGroupObservable
        : null;
      ...
      if (renderSprites) {
        for (
          let index = 0;
          index < this._scene.spriteManagers.length;
          index++
        ) {
    ...
    public setRenderingOrder(
      ...
      if (this._renderingGroups[renderingGroupId]) {
        ...
        group.opaqueSortCompareFn = this._customOpaqueSortCompareFn[
          renderingGroupId
        ];
        group.alphaTestSortCompareFn = this._customAlphaTestSortCompareFn[
          renderingGroupId
        ];
        group.transparentSortCompareFn = this._customTransparentSortCompareFn[
          renderingGroupId
        ];
      }
    }
