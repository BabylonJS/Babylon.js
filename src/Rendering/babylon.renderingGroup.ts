namespace BABYLON {
  export class RenderingGroup {
    ...
    private _alphaTestSortCompareFn: Nullable<
    ...
    public render(
      ...
      // Edges
      for (
        var edgesRendererIndex = 0;
        edgesRendererIndex < this._edgesRenderers.length;
        edgesRendererIndex++
      ) {
    ...
    private static renderSorted(
      ...
      for (; subIndex < subMeshes.length; subIndex++) {
        ...
        subMesh._distanceToCamera = subMesh
          .getBoundingInfo()
     ...
     */
    public static defaultTransparentSortCompare(
      a: SubMesh,
      b: SubMesh
    ): number {
      // Alpha index first
    ...
    private _renderParticles(activeMeshes: Nullable<AbstractMesh[]>): void {
      ...
      this._scene.onBeforeParticlesRenderingObservable.notifyObservers(
        this._scene
      );
      for (
        var particleIndex = 0;
        particleIndex < this._scene._activeParticleSystems.length;
        particleIndex++
      ) {
        var particleSystem = this._scene._activeParticleSystems.data[
          particleIndex
        ];

        if (
          (activeCamera &&
            activeCamera.layerMask & particleSystem.layerMask) === 0
        ) {
          continue;
        }
        ...
        if (
          !emitter.position ||
          !activeMeshes ||
          activeMeshes.indexOf(emitter) !== -1
        ) {
    ...
    private _renderSprites(): void {
      ...
      // Sprites
      var activeCamera = this._scene.activeCamera;
      this._scene.onBeforeSpritesRenderingObservable.notifyObservers(
        this._scene
      );
      for (var id = 0; id < this._spriteManagers.length; id++) {
        ...
        if (
          (activeCamera && activeCamera.layerMask & spriteManager.layerMask) !==
          0
        ) {
