namespace BABYLON {
  ...
  export class EdgesRenderer {
    ...
    private _prepareRessources(): void {
      ...
      this._lineShader = new ShaderMaterial(
        "lineShader",
    ...
    public dispose(): void {
      ...
      this._source
        .getScene()
        .getEngine()
        ._releaseBuffer(this._ib);
      this._lineShader.dispose();
    }

    private _processEdgeForAdjacencies(
      pa: number,
      pb: number,
      p0: number,
      p1: number,
    ...
    private _processEdgeForAdjacenciesWithVertices(
      pa: Vector3,
      pb: Vector3,
      p0: Vector3,
      p1: Vector3,
      ...
      if (
        (pa.equalsWithEpsilon(p0) && pb.equalsWithEpsilon(p1)) ||
      ...
      if (
        (pa.equalsWithEpsilon(p1) && pb.equalsWithEpsilon(p2)) ||
      ...
      if (
        (pa.equalsWithEpsilon(p2) && pb.equalsWithEpsilon(p0)) ||
    ...
    _generateEdgesLines(): void {
      ...
      for (index = 0; index < indices.length; index += 3) {
        ...
        faceAdjacencies.p0 = new Vector3(
          positions[p0Index * 3],
          positions[p0Index * 3 + 1],
          positions[p0Index * 3 + 2]
        );
        faceAdjacencies.p1 = new Vector3(
          positions[p1Index * 3],
          positions[p1Index * 3 + 1],
          positions[p1Index * 3 + 2]
        );
        faceAdjacencies.p2 = new Vector3(
          positions[p2Index * 3],
          positions[p2Index * 3 + 1],
          positions[p2Index * 3 + 2]
        );
        var faceNormal = Vector3.Cross(
          faceAdjacencies.p1.subtract(faceAdjacencies.p0),
          faceAdjacencies.p2.subtract(faceAdjacencies.p1)
        );
            ...
            switch (edgeIndex) {
              case 0:
                if (this._checkVerticesInsteadOfIndices) {
                  ...
                  otherEdgeIndex = this._processEdgeForAdjacencies(
                    indices[index * 3],
                    indices[index * 3 + 1],
                    otherP0,
                    otherP1,
      ...
      this._buffers[VertexBuffer.PositionKind] = new VertexBuffer(
        engine,
        this._linesPositions,
        VertexBuffer.PositionKind,
        false
      );
      this._buffers[VertexBuffer.NormalKind] = new VertexBuffer(
        engine,
        this._linesNormals,
    ...
    public render(): void {
      ...
      engine.bindBuffers(
        this._buffers,
        this._ib,
        <Effect>this._lineShader.getEffect()
      );
      ...
      if (scene.activeCamera.mode === Camera.ORTHOGRAPHIC_CAMERA) {
        this._lineShader.setFloat(
          "width",
          this._source.edgesWidth / this.edgesWidthScalerForOrthographic
        );
      } else {
        this._lineShader.setFloat(
          "width",
          this._source.edgesWidth / this.edgesWidthScalerForPerspective
        );
      }
