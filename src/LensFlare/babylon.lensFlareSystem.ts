namespace BABYLON {
  export class LensFlareSystem {
    ...
    constructor(public name: string, emitter: any, scene: Scene) {
      ...
      this._vertexBuffers[VertexBuffer.PositionKind] = new VertexBuffer(
        engine,
        vertices,
    ...
    public computeEffectivePosition(globalViewport: Viewport): boolean {
      ...
      if (position.z > 0) {
        if (
          this._positionX > globalViewport.x &&
          this._positionX < globalViewport.x + globalViewport.width
        ) {
          if (
            this._positionY > globalViewport.y &&
            this._positionY < globalViewport.y + globalViewport.height
          )
            return true;
        }
        return true;
    ...
    public _isVisible(): boolean {
      ...
      var direction = emitterPosition.subtract(
      ...
      var pickInfo = this._scene.pickWithRay(
        ray,
    ...
    public render(): boolean {
      ...
      var globalViewport = viewport.toGlobal(
      ...
      if (this._positionX < this.borderLimit + globalViewport.x) {
        ...
        this._positionX >
        globalViewport.x + globalViewport.width - this.borderLimit
      ) {
        awayX =
          this._positionX -
          globalViewport.x -
      ...
      if (this._positionY < this.borderLimit + globalViewport.y) {
        ...
        this._positionY >
        globalViewport.y + globalViewport.height - this.borderLimit
      ) {
        awayY =
          this._positionY -
          globalViewport.y -
      ...
      for (var index = 0; index < this.lensFlares.length; index++) {
        ...
        var cw = flare.size;
        var ch =
          flare.size * engine.getAspectRatio(this._scene.activeCamera, true);
        ...
        this._effect.setFloat4(
          "color",
          flare.color.r * intensity,
          flare.color.g * intensity,
          flare.color.b * intensity,
          1.0
        );
    ...
    public static Parse(
      ...
      var name =
        parsedLensFlareSystem.name ||
        "lensFlareSystem#" + parsedLensFlareSystem.emitterId;
      ...
      for (
        var index = 0;
        index < parsedLensFlareSystem.flares.length;
        index++
      ) {
    ...
    public serialize(): any {
      ...
      for (var index = 0; index < this.lensFlares.length; index++) {
        ...
        serializationObject.flares.push({
          ...
          textureName: Tools.GetFilename(
            flare.texture ? flare.texture.name : ""
          )
        });
      }
