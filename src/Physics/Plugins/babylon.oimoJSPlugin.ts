namespace BABYLON {
  ...
  export class OimoJSPlugin implements IPhysicsEnginePlugin {
    ...
    constructor(iterations?: number) {
      this.BJSOIMO =
        typeof OIMO !== "undefined"
          ? OIMO
          : typeof require !== "undefined" ? require("./Oimo") : undefined;
    ...
    public applyImpulse(
      ...
      impostor.physicsBody.applyImpulse(
        contactPoint.scale(this.BJSOIMO.INV_SCALE),
        force.scale(this.BJSOIMO.INV_SCALE * mass)
      );
    }
    public applyForce(
      impostor: PhysicsImpostor,
        ...
        impostors.forEach(i => {
          ...
          if (i === impostor) {
            ...
            impostor.object
              ...
              .subtractToRef(center, this._tmpPositionVector);
            ...
            //tmp solution
            bodyConfig.rot.push(
              rot.x / (this.BJSOIMO.degtorad || this.BJSOIMO.TO_RAD)
            );
            bodyConfig.rot.push(
              rot.y / (this.BJSOIMO.degtorad || this.BJSOIMO.TO_RAD)
            );
            bodyConfig.rot.push(
              rot.z / (this.BJSOIMO.degtorad || this.BJSOIMO.TO_RAD)
            );
          } else {
            let localPosition = i.object
              .getAbsolutePosition()
              .subtract(impostor.object.getAbsolutePosition());
          ...
          switch (i.type) {
            case PhysicsImpostor.ParticleImpostor:
              Tools.Warn(
                "No Particle support in this.BJSOIMO.js. using SphereImpostor instead"
              );
    ...
    public generateJoint(impostorJoint: PhysicsImpostorJoint) {
      ...
      var nativeJointData: any = {
        ...
        axe1:
          options.axe1 ||
          (jointData.mainAxis ? jointData.mainAxis.asArray() : null),
        axe2:
          options.axe2 ||
          (jointData.connectedAxis ? jointData.connectedAxis.asArray() : null),
        pos1:
          options.pos1 ||
          (jointData.mainPivot ? jointData.mainPivot.asArray() : null),
        pos2:
          options.pos2 ||
          (jointData.connectedPivot
            ? jointData.connectedPivot.asArray()
            : null),
      ...
      switch (impostorJoint.joint.type) {
        ...
        case PhysicsJoint.SpringJoint:
          Tools.Warn(
            "this.BJSOIMO.js doesn't support Spring Constraint. Simulating using DistanceJoint instead"
          );
          var springData = <SpringJointData>jointData;
          nativeJointData.min = springData.length || nativeJointData.min;
          //Max should also be set, just make sure it is at least min
          nativeJointData.max = Math.max(
            nativeJointData.min,
            nativeJointData.max
          );
      ...
      impostorJoint.joint.physicsJoint = new this.BJSOIMO.Link(
        nativeJointData
      ).joint; //this.world.add(nativeJointData);
    }
    ...
    public setTransformationFromPhysicsBody(impostor: PhysicsImpostor) {
      if (!impostor.physicsBody.sleeping) {
        ...
        if (impostor.physicsBody.shapes.next) {
          ...
          impostor.object.position.x =
            parentShape.position.x * this.BJSOIMO.WORLD_SCALE;
          impostor.object.position.y =
            parentShape.position.y * this.BJSOIMO.WORLD_SCALE;
          impostor.object.position.z =
            parentShape.position.z * this.BJSOIMO.WORLD_SCALE;
        } else {
        ...
        if (impostor.object.rotationQuaternion) {
          impostor.object.rotationQuaternion.copyFrom(
            impostor.physicsBody.getQuaternion()
          );
    ...
    public setPhysicsBodyTransformation(
      ...
      body.position.init(
        newPosition.x * this.BJSOIMO.INV_SCALE,
        newPosition.y * this.BJSOIMO.INV_SCALE,
        newPosition.z * this.BJSOIMO.INV_SCALE
      );

      body.orientation.init(
        newRotation.w,
        newRotation.x,
        newRotation.y,
        newRotation.z
    ...
    public setLinearVelocity(impostor: PhysicsImpostor, velocity: Vector3) {
      impostor.physicsBody.linearVelocity.init(
        velocity.x,
        velocity.y,
        velocity.z
      );
    ...
    public setAngularVelocity(impostor: PhysicsImpostor, velocity: Vector3) {
      impostor.physicsBody.angularVelocity.init(
        velocity.x,
        velocity.y,
        velocity.z
      );
    ...
    public setMotor(
      ...
      var motor = motorIndex
        ...
        : joint.physicsJoint.rotationalLimitMotor1 ||
          joint.physicsJoint.rotationalLimitMotor ||
          joint.physicsJoint.limitMotor;
      if (motor) {
