namespace BABYLON {
  export class PhysicsHelper {
    ...
    public updraft(
      ...
      var event = new PhysicsUpdraftEvent(
        this._scene,
        origin,
        radius,
        strength,
    ...
    public vortex(
      ...
      var event = new PhysicsVortexEvent(
        this._scene,
        origin,
        radius,
        strength,
  ...
  export class PhysicsRadialExplosionEvent {
    ...
    private _sphereOptions: { segments: number; diameter: number } = {
      segments: 32,
      diameter: 1
     ...
     */
    public getImpostorForceAndContactPoint(
      impostor: PhysicsImpostor,
      origin: Vector3,
      radius: number,
      strength: number,
      falloff: PhysicsRadialImpulseFalloff
    ): Nullable<PhysicsForceAndContactPoint> {
      if (impostor.mass === 0) {
        return null;
      }
      ...
      var force = direction.multiplyByFloats(
        multiplier,
        multiplier,
        multiplier
      );
  ...
  export class PhysicsGravitationalFieldEvent {
    ...
    private _tick() {
      ...
      if (this._sphere) {
        ...
        var radialExplosionEvent = this._physicsHelper.applyRadialExplosionForce(
          this._origin,
          this._radius,
          this._strength * -1,
          this._falloff
        ...
        if (radialExplosionEvent) {
          this._sphere = <Mesh>radialExplosionEvent
            .getData()
  ...
  export class PhysicsUpdraftEvent {
    ...
    constructor(
      ...
      this._origin.addToRef(
        new Vector3(0, this._height / 2, 0),
    ...
    private _prepareCylinder(): void {
      if (!this._cylinder) {
        this._cylinder = MeshBuilder.CreateCylinder(
  ...
  export class PhysicsVortexEvent {
    ...
    private getImpostorForceAndContactPoint(
      ...
      var impostorObject = <Mesh>impostor.object;
      var impostorObjectCenter = impostor.getObjectCenter();
      var originOnPlane = new Vector3(
        this._origin.x,
        impostorObjectCenter.y,
        this._origin.z
      ); // the distance to the origin as if both objects were on a plane (Y-axis)
      var originToImpostorDirection = impostorObjectCenter.subtract(
        originOnPlane
      );
      ...
      var perpendicularDirection = Vector3.Cross(
        originOnPlane,
