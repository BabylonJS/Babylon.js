  ...
  export class BoundingSphere {
    ...
    public _update(world: Matrix): void {
      ...
      Vector3.TransformNormalFromFloatsToRef(
        ...
        1.0,
        world,
        this._tempRadiusVector
      );
      this.radiusWorld =
        Math.max(
          Math.abs(this._tempRadiusVector.x),
          Math.abs(this._tempRadiusVector.y),
          Math.abs(this._tempRadiusVector.z)
        ) * this.radius;
    }
    ...
    public isInFrustum(frustumPlanes: Plane[]): boolean {
      for (var i = 0; i < 6; i++) {
        if (
          frustumPlanes[i].dotCoordinate(this.centerWorld) <= -this.radiusWorld
        )
          return false;
      }
    ...
    public intersectsPoint(point: Vector3): boolean {
      ...
      if (Math.abs(this.radiusWorld - distance) < Epsilon) return false;
