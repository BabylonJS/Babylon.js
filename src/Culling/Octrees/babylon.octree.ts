namespace BABYLON {
  ...
  export class Octree<T> {
      ...
      for (var x = 0; x < 2; x++) {
        for (var y = 0; y < 2; y++) {
          for (var z = 0; z < 2; z++) {
            ...
            var localMax = worldMin.add(
              blockSize.multiplyByFloats(x + 1, y + 1, z + 1)
            );

            var block = new OctreeBlock<T>(
              localMin,
              localMax,
              maxBlockCapacity,
              currentDepth + 1,
    ...
    public static CreationFuncForMeshes = (
      entry: AbstractMesh,
      ...
      if (
        !entry.isBlocked &&
        boundingInfo.boundingBox.intersectsMinMax(
          block.minPoint,
          block.maxPoint
        )
      ) {
    ...
    public static CreationFuncForSubMeshes = (
      entry: SubMesh,
