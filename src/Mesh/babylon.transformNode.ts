namespace BABYLON {
  export class TransformNode extends Node {
    ...
    // Properties
    @serializeAsVector3() private _rotation = Vector3.Zero();

    @serializeAsQuaternion() private _rotationQuaternion: Nullable<Quaternion>;

    @serializeAsVector3() protected _scaling = Vector3.One();
    protected _isDirty = false;
    private _transformToBoneReferal: Nullable<TransformNode>;

    @serialize() public billboardMode = AbstractMesh.BILLBOARDMODE_NONE;

    @serialize() public scalingDeterminant = 1;

    @serialize() public infiniteDistance = false;

    @serializeAsVector3() public position = Vector3.Zero();
    ...
    public _isSynchronized(): boolean {
      ...
      if (
        this.billboardMode !== this._cache.billboardMode ||
    ...
    public rotate(axis: Vector3, amount: number, space?: Space): TransformNode {
      ...
      if (!space || (space as any) === Space.LOCAL) {
        rotationQuaternion = Quaternion.RotationAxisToRef(
          axis,
          amount,
          AbstractMesh._rotationAxisCache
        );
    ...
    public rotateAround(
      ...
      Tmp.Quaternion[0].multiplyToRef(
        this.rotationQuaternion,
        this.rotationQuaternion
      );
    ...
    public translate(
      ...
      if (!space || (space as any) === Space.LOCAL) {
        var tempV3 = this.getPositionExpressedInLocalSpace().add(
          displacementVector
        );
    ...
    public computeWorldMatrix(force?: boolean): Matrix {
      ...
      // Scaling
      Matrix.ScalingToRef(
        this.scaling.x * this.scalingDeterminant,
        this.scaling.y * this.scalingDeterminant,
        this.scaling.z * this.scalingDeterminant,
        Tmp.Matrix[1]
      );
      ...
      if (this.rotationQuaternion) {
        ...
        if (len) {
          this.rotationQuaternion.multiplyInPlace(
      ...
      if (this.billboardMode !== AbstractMesh.BILLBOARDMODE_NONE && camera) {
        if (
          (this.billboardMode & AbstractMesh.BILLBOARDMODE_ALL) !==
          ...
          if (
            (this.billboardMode & AbstractMesh.BILLBOARDMODE_X) ===
          ...
          if (
            (this.billboardMode & AbstractMesh.BILLBOARDMODE_Y) ===
          ...
          if (
            (this.billboardMode & AbstractMesh.BILLBOARDMODE_Z) ===
      ...
      if (this.parent && this.parent.getWorldMatrix) {
        if (this.billboardMode !== AbstractMesh.BILLBOARDMODE_NONE) {
          ...
          if (this._transformToBoneReferal) {
            this._localWorld.multiplyToRef(
              this.parent.getWorldMatrix(),
              Tmp.Matrix[6]
            );
      ...
      if (this._postMultiplyPivotMatrix) {
        this._worldMatrix.multiplyToRef(
      ...
      if (this.scaling.isNonUniform) {
        ...
        this.parent &&
        (<TransformNode>this.parent)._nonUniformScaling
      ) {
