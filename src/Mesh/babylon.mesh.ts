namespace BABYLON {
  ...
  export class Mesh extends AbstractMesh implements IGetSetVerticesData {
    ...
    public toString(fullDetails?: boolean): string {
      ...
      if (fullDetails) {
        if (this._geometry) {
          ...
          if (vb && ib) {
            ret +=
              ", flat shading: " + (vb.length / 3 === ib.length ? "YES" : "NO");
          }
        } else {
    ...
    public getLOD(
      ...
      var distanceToCamera = bSphere.centerWorld
        .subtract(camera.globalPosition)
        .length();

      if (
        this._LODLevels[this._LODLevels.length - 1].distance > distanceToCamera
      ) {
        if (this.onLODLevelSelection) {
          this.onLODLevelSelection(
            distanceToCamera,
            this,
            this._LODLevels[this._LODLevels.length - 1].mesh
          );
        }
        return this;
      }
    ...
    public isReady(forceInstanceSupport = false): boolean {
      ...
      let hardwareInstancedRendering =
        ...
        (engine.getCaps().instancedArrays && this.instances.length > 0);
    ...
    private _getPositionData(applySkeleton: boolean): Nullable<FloatArray> {
      ...
      if (data && applySkeleton && this.skeleton) {
        ...
        var matricesIndicesData = this.getVerticesData(
          VertexBuffer.MatricesIndicesKind
        );
        var matricesWeightsData = this.getVerticesData(
          VertexBuffer.MatricesWeightsKind
        );
        if (matricesWeightsData && matricesIndicesData) {
          var needExtras = this.numBoneInfluencers > 4;
          var matricesIndicesExtraData = needExtras
            ? this.getVerticesData(VertexBuffer.MatricesIndicesExtraKind)
            : null;
          var matricesWeightsExtraData = needExtras
            ? this.getVerticesData(VertexBuffer.MatricesWeightsExtraKind)
            : null;
          ...
          for (
            ...
            for (inf = 0; inf < 4; inf++) {
              ...
              Matrix.FromFloat32ArrayToRefScaled(
                skeletonMatrices,
                matricesIndicesData[matWeightIdx + inf] * 16,
                weight,
                tempMatrix
              );
      ...
      if (this.subMeshes && this.subMeshes.length > 0) {
        ...
        if (force) {
          ...
          for (var submesh of this.subMeshes) {
            ...
            if (
              submesh.verticesStart + submesh.verticesCount >=
              totalVertices
            ) {
              needToRecreate = true;
              break;
    ...
    public subdivide(count: number): void {
      ...
      for (var index = 0; index < count; index++) {
        ...
        SubMesh.CreateFromIndices(
          0,
          offset,
          Math.min(subdivisionSize, totalIndices - offset),
          this
        );
    ...
    public markVerticesDataAsUpdatable(kind: string, updatable = true) {
      ...
      this.setVerticesData(
        kind,
        <FloatArray>this.getVerticesData(kind),
        updatable
      );
    }
      ...
      if (this._unIndexed) {
        ...
        switch (fillMode) {
          ...
          case Material.WireFrameFillMode:
            indexToBind = subMesh.getLinesIndexBuffer(
    ...
    public _getInstancesRenderList(subMeshId: number): _InstancesBatch {
      ...
      this._batchCache.renderSelf[subMeshId] =
        this.isEnabled() && this.isVisible;
      this._batchCache.visibleInstances[subMeshId] = null;

      if (this._visibleInstances) {
        var currentRenderId = scene.getRenderId();
        var defaultRenderId = scene._isInIntermediateRendering()
          ? this._visibleInstances.intermediateDefaultRenderId
          : this._visibleInstances.defaultRenderId;
        this._batchCache.visibleInstances[subMeshId] = this._visibleInstances[
          currentRenderId
        ];
        ...
        if (!this._batchCache.visibleInstances[subMeshId] && defaultRenderId) {
          ...
          selfRenderId = Math.max(
            this._visibleInstances.selfDefaultRenderId,
            currentRenderId
          );
        }

        let visibleInstancesForSubMesh = this._batchCache.visibleInstances[
          subMeshId
        ];
    ...
    public _renderWithInstances(
      ...
      if (
        !this._instancesData ||
        currentInstancesBufferSize != this._instancesBufferSize
      ) {
    ...
    public _processRendering(
      ...
      if (hardwareInstancedRendering) {
        ...
        if (visibleInstancesForSubMesh) {
          for (
            var instanceIndex = 0;
            instanceIndex < visibleInstancesForSubMesh.length;
            instanceIndex++
          ) {
    ...
    public render(subMesh: SubMesh, enableAlphaMode: boolean): Mesh {
      ...
      if (this._effectiveMaterial.storeEffectOnSubMeshes) {
        if (
          !this._effectiveMaterial.isReadyForSubMesh(
            this,
            subMesh,
            hardwareInstancedRendering
          )
        ...
        !this._effectiveMaterial.isReady(this, hardwareInstancedRendering)
      ) {
        return this;
      }
      ...
      var fillMode = scene.forcePointsCloud
        ...
        : scene.forceWireframe
          ...
          : this._effectiveMaterial.fillMode;
    ...
    public applyDisplacementMapFromBuffer(
      ...
      if (
        ...
        Tools.Warn(
          "Cannot call applyDisplacementMap: Given mesh is not complete. Position, Normal or UV are missing"
        );
        return this;
      }

      var positions = <FloatArray>this.getVerticesData(
        VertexBuffer.PositionKind
      );
      ...
      for (var index = 0; index < positions.length; index += 3) {
        ...
        // Compute height
        var u =
          ((Math.abs(uv.x * uvScale.x + uvOffset.x) * heightMapWidth) %
            heightMapWidth) |
          0;
        var v =
          ((Math.abs(uv.y * uvScale.y + uvOffset.y) * heightMapHeight) %
            heightMapHeight) |
          0;
    ...
    public convertToFlatShadedMesh(): Mesh {
      ...
      for (
        var submeshIndex = 0;
        submeshIndex < previousSubmeshes.length;
        submeshIndex++
      ) {
    ...
    public _syncGeometryWithMorphTargetManager() {
      ...
      if (morphTargetManager && morphTargetManager.vertexCount) {
        if (morphTargetManager.vertexCount !== this.getTotalVertices()) {
          Tools.Error(
            "Mesh is incompatible with morph targets. Targets and mesh must all have the same vertices count."
          );
          this.morphTargetManager = null;
          return;
        }
        ...
        for (
          var index = 0;
          index < morphTargetManager.numInfluencers;
          index++
        ) {
          ...
          this.geometry.setVerticesData(
            VertexBuffer.PositionKind + index,
            positions,
            false,
            3
          );
          ...
          if (tangents) {
            this.geometry.setVerticesData(
              VertexBuffer.TangentKind + index,
              tangents,
              false,
              3
            );
        ...
        // Positions
        while (
          this.geometry.isVerticesDataPresent(VertexBuffer.PositionKind + index)
        ) {
          this.geometry.removeVerticesData(VertexBuffer.PositionKind + index);

          if (
            this.geometry.isVerticesDataPresent(VertexBuffer.NormalKind + index)
          ) {
    ...
    public static Parse(parsedMesh: any, scene: Scene, rootUrl: string): Mesh {
      ...
      if (parsedMesh.delayLoadingFile) {
        ...
        mesh._boundingInfo = new BoundingInfo(
          Vector3.FromArray(parsedMesh.boundingBoxMinimum),
      ...
      if (parsedMesh.animations) {
        for (
          ...
          animationIndex < parsedMesh.animations.length;
          animationIndex++
        ) {
      ...
      if (parsedMesh.autoAnimate) {
        scene.beginAnimation(
          mesh,
          parsedMesh.autoAnimateFrom,
          parsedMesh.autoAnimateTo,
          parsedMesh.autoAnimateLoop,
          parsedMesh.autoAnimateSpeed || 1.0
        );
      }
        ...
        for (var index = 0; index < parsedMesh.instances.length; index++) {
          ...
          if (parsedInstance.rotationQuaternion) {
            instance.rotationQuaternion = Quaternion.FromArray(
              parsedInstance.rotationQuaternion
            );
    ...
    public applySkeleton(skeleton: Skeleton): Mesh {
      ...
      for (
        ...
        Vector3.TransformCoordinatesFromFloatsToRef(
          this._sourcePositions[index],
          this._sourcePositions[index + 1],
          this._sourcePositions[index + 2],
          finalMatrix,
          tempVector3
        );
        ...
        Vector3.TransformNormalFromFloatsToRef(
          this._sourceNormals[index],
          this._sourceNormals[index + 1],
          this._sourceNormals[index + 2],
          finalMatrix,
          tempVector3
        );
    ...
    public static Center(
      meshesOrMinMaxVector: { min: Vector3; max: Vector3 } | AbstractMesh[]
    ): Vector3 {
      var minMaxVector =
        meshesOrMinMaxVector instanceof Array
          ? Mesh.MinMax(meshesOrMinMaxVector)
