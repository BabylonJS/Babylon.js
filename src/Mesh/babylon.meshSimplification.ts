      ...
      if (task.parallelProcessing) {
        ...
        task.settings.forEach(setting => {
          ...
          simplifier.simplify(setting, newMesh => {
            ...
            if (
              setting.quality ===
                task.settings[task.settings.length - 1].quality &&
        ...
        var runDecimation = (
          setting: ISimplificationSettings,
          callback: () => void
        ) => {
          simplifier.simplify(setting, newMesh => {
            task.mesh.addLODLevel(setting.distance, newMesh);
  ...
  export class QuadraticErrorSimplification implements ISimplifier {
    ...
    private runDecimation(
      ...
      var iterationFunction = (iteration: number, callback: () => void) => {
        setTimeout(() => {
          ...
          var threshold =
            0.000000001 * Math.pow(iteration + 3, this.aggressiveness);

          var trianglesIterator = (i: number) => {
            var tIdx = ~~(
              (this.triangles.length / 2 + i) %
            ...
            for (var j = 0; j < 3; ++j) {
              if (t.error[j] < threshold) {
                ...
                deletedTriangles = this.updateTriangles(
                  v0,
                  v0,
                  deleted0,
                  deletedTriangles
                ...
                if (tCount <= v0.triangleCount) {
                  if (tCount) {
                    for (var c = 0; c < tCount; c++) {
                      this.references[v0.triangleStart + c] = this.references[
                        tStart + c
                      ];
                    }
                  }
    ...
    private initWithMesh(
      ...
      var vertexInit = (i: number) => {
        ...
        var vertex =
          findInVertices(position) ||
          new DecimationVertex(position, this.vertices.length);
      ...
      AsyncLoop.SyncAsyncForLoop(
        ...
        () => {
          var indicesInit = (i: number) => {
            ...
            var v0: DecimationVertex = this.vertices[
              vertexReferences[i0 - submesh.verticesStart]
            ];
            var v1: DecimationVertex = this.vertices[
              vertexReferences[i1 - submesh.verticesStart]
            ];
            var v2: DecimationVertex = this.vertices[
              vertexReferences[i2 - submesh.verticesStart]
            ];
    ...
    private init(callback: Function) {
      var triangleInit1 = (i: number) => {
        ...
        t.normal = Vector3.Cross(
          t.vertices[1].position.subtract(t.vertices[0].position),
          t.vertices[2].position.subtract(t.vertices[0].position)
        ).normalize();
        for (var j = 0; j < 3; j++) {
          t.vertices[j].q.addArrayInPlace(
            QuadraticMatrix.DataFromNumbers(
              t.normal.x,
              t.normal.y,
              t.normal.z,
              -Vector3.Dot(t.normal, t.vertices[0].position)
            )
          );
        }
      };
    ...
    private reconstructMesh(submeshIndex: number) {
      ...
      var newPositionData = <number[]>(this._reconstructedMesh.getVerticesData(
        VertexBuffer.PositionKind
      ) || []);
      var newNormalData = <number[]>(this._reconstructedMesh.getVerticesData(
        VertexBuffer.NormalKind
      ) || []);
      var newUVsData = <number[]>(this._reconstructedMesh.getVerticesData(
        VertexBuffer.UVKind
      ) || []);
      var newColorsData = <number[]>(this._reconstructedMesh.getVerticesData(
      ...
      this._reconstructedMesh.setVerticesData(
        VertexBuffer.PositionKind,
        newPositionData
      );
      this._reconstructedMesh.setVerticesData(
    ...
    private initDecimatedMesh() {
      this._reconstructedMesh = new Mesh(
    ...
    private isFlipped(
      ...
      borderFactor: number,
      delTr: Array<DecimationTriangle>
    ): boolean {
      for (var i = 0; i < vertex1.triangleCount; ++i) {
        var t = this.triangles[
          this.references[vertex1.triangleStart + i].triangleId
        ];
    ...
    private updateTriangles(
      ...
      for (var i = 0; i < vertex.triangleCount; ++i) {
        ...
        t.isDirty = true;
        t.error[0] =
          this.calculateError(t.vertices[0], t.vertices[1]) +
          t.borderFactor / 2;
        t.error[1] =
          this.calculateError(t.vertices[1], t.vertices[2]) +
          t.borderFactor / 2;
        t.error[2] =
          this.calculateError(t.vertices[2], t.vertices[0]) +
          t.borderFactor / 2;
        t.error[3] = Math.min(t.error[0], t.error[1], t.error[2]);
    ...
    private identifyBorder() {
      for (var i = 0; i < this.vertices.length; ++i) {
        ...
        for (j = 0; j < v.triangleCount; ++j) {
          var triangle = this.triangles[
            this.references[v.triangleStart + j].triangleId
          ];
    ...
    private updateMesh(identifyBorders: boolean = false) {
      ...
      var newReferences: Array<Reference> = new Array(
      ...
      for (i = 0; i < this.triangles.length; ++i) {
        ...
        for (j = 0; j < 3; ++j) {
          v = t.vertices[j];
          newReferences[v.triangleStart + v.triangleCount] = new Reference(
            j,
            i
          );
          v.triangleCount++;
    ...
    private calculateError(
      ...
      if (qDet !== 0 && !border) {
        ...
        var p3 = vertex1.position
          .add(vertex2.position)
