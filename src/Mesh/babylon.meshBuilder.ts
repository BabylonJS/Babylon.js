namespace BABYLON {
  export class MeshBuilder {
    ...
    public static CreateBox(
      ...
      options.sideOrientation = MeshBuilder.updateSideOrientation(
    ...
    public static CreateRibbon(
      ...
      var sideOrientation = MeshBuilder.updateSideOrientation(
        options.sideOrientation
      );
      ...
      if (instance) {
        ...
        var positionFunction = (positions: FloatArray) => {
          ...
          var i = 0;
          var ns =
            (<Mesh>instance)._originalBuilderSideOrientation === Mesh.DOUBLESIDE
              ? 2
              : 1;
        ...
        var positions = <FloatArray>instance.getVerticesData(
          VertexBuffer.PositionKind
        );
        positionFunction(positions);
        instance._boundingInfo = new BoundingInfo(
          Tmp.Vector3[0],
          Tmp.Vector3[1]
        );
        instance._boundingInfo.update(instance._worldMatrix);
        instance.updateVerticesData(
          VertexBuffer.PositionKind,
          positions,
          false,
          false
        ...
        if (options.colors) {
          var colors = <FloatArray>instance.getVerticesData(
        ...
        if (!instance.areNormalsFrozen || instance.isFacetDataEnabled) {
          ...
          var normals = <FloatArray>instance.getVerticesData(
            VertexBuffer.NormalKind
          );
          var params = instance.isFacetDataEnabled
            ? instance.getFacetDataParameters()
            : null;
     ...
     * The mesh can be set to updatable with the boolean parameter `updatable` (default false) if its internal geometry is supposed to change once created.
     */
    public static CreateLineSystem(
      name: string,
      options: {
        lines: Vector3[][];
        updatable: boolean;
    ...
    public static CreateLines(
      ...
      var lines = MeshBuilder.CreateLineSystem(
        name,
        {
    ...
    public static CreateDashedLines(
      ...
      if (instance) {
        ...
        var positionFunction = (positions: FloatArray): void => {
          ...
          shft = lg / nbSeg;
          dashshft =
            (<any>instance).dashSize *
            shft /
            ((<any>instance).dashSize + (<any>instance).gapSize);
          for (i = 0; i < points.length - 1; i++) {
            points[i + 1].subtractToRef(points[i], curvect);
    ...
    public static CreateLathe(
      ...
      var arc: number = options.arc
        ? options.arc <= 0 || options.arc > 1 ? 1.0 : options.arc
        : 1.0;
      var closed: boolean =
        options.closed === undefined ? true : options.closed;
      var shape = options.shape;
      ...
      for (i = 0; i <= tessellation; i++) {
        ...
        if (cap == Mesh.CAP_START || cap == Mesh.CAP_ALL) {
          ...
          path.push(
            new Vector3(
              Math.cos(i * step) * shape[0].x * radius,
              shape[0].y,
              Math.sin(i * step) * shape[0].x * radius
            )
          );
        }
        for (p = 0; p < shape.length; p++) {
          rotated = new Vector3(
            Math.cos(i * step) * shape[p].x * radius,
            shape[p].y,
            Math.sin(i * step) * shape[p].x * radius
          );
          path.push(rotated);
        }
        if (cap == Mesh.CAP_END || cap == Mesh.CAP_ALL) {
          path.push(
            new Vector3(
              Math.cos(i * step) * shape[shape.length - 1].x * radius,
              shape[shape.length - 1].y,
              Math.sin(i * step) * shape[shape.length - 1].x * radius
            )
          );
      ...
      // lathe ribbon
      var lathe = MeshBuilder.CreateRibbon(
        name,
        {
          pathArray: paths,
          closeArray: closed,
    ...
    public static CreatePlane(
      ...
      if (options.sourcePlane) {
        ...
        var product = Math.acos(
          Vector3.Dot(options.sourcePlane.normal, Axis.Z)
        );
    ...
    public static CreateGroundFromHeightMap(
      ...
      var onload = (img: HTMLImageElement) => {
        ...
        var buffer = <Uint8Array>(<any>context.getImageData(
          0,
          0,
          bufferWidth,
          bufferHeight
    ...
    public static CreatePolygon(
      ...
      if (
        contours[0].equalsWithEpsilon(contours[contours.length - 1], epsilon)
      ) {
        contours.pop();
      }
      ...
      var vertexData = VertexData.CreatePolygon(
        polygon,
        options.sideOrientation,
        options.faceUV,
        options.faceColors,
    ...
    public static CreateTube(
      ...
      options.arc =
        options.arc && (options.arc <= 0.0 || options.arc > 1.0)
          ? 1.0
          : options.arc || 1.0;
      ...
      var tubePathArray = (
        ...
        var pi2 = Math.PI * 2;
        var step = pi2 / tessellation * arc;
        var returnRadius: { (i: number, distance: number): number } = () =>
          radius;
        var radiusFunctionFinal: { (i: number, distance: number): number } =
          radiusFunction || returnRadius;
      ...
      if (instance) {
        ...
        pathArray = tubePathArray(
          path,
          path3D,
          (<any>instance).pathArray,
          radius,
          (<any>instance).tessellation,
          radiusFunction,
          (<any>instance).cap,
          arc
        );
      ...
      var tube = MeshBuilder.CreateRibbon(
        name,
        {
          pathArray: pathArray,
          closePath: true,
    ...
    public static CreateDecal(
      ...
      if (!normal) {
        ...
        var cameraWorldTarget = Vector3.TransformCoordinates(
          target,
      ...
      // Matrix
      var decalWorldMatrix = Matrix.RotationYawPitchRoll(
        yaw,
        pitch,
        angle
      ...
      var extractDecalVector3 = (indexId: number): PositionNormalVertex => {
        ...
        result.position = new Vector3(
          positions[vertexId * 3],
          positions[vertexId * 3 + 1],
          positions[vertexId * 3 + 2]
        );

        // Send vector to decal local world
        result.position = Vector3.TransformCoordinates(
          result.position,
          transformMatrix
        );
      ...
      var clip = (
        ...
        var clipVertices = (
          v0: PositionNormalVertex,
          v1: PositionNormalVertex
        ): PositionNormalVertex => {
          var clipFactor = Vector3.GetClipFactor(
            v0.position,
            v1.position,
            axis,
            clipSize
    ...
    private static _ExtrudeShapeGeneric(
      ...
      var extrusionPathArray = (
        ...
        var angle = 0;
        var returnScale: { (i: number, distance: number): number } = () => {
          return scale !== null ? scale : 1;
        };
        var returnRotation: { (i: number, distance: number): number } = () => {
          return rotation !== null ? rotation : 0;
        };
        var rotate: { (i: number, distance: number): number } =
          custom && rotateFunction ? rotateFunction : returnRotation;
        var scl: { (i: number, distance: number): number } =
          custom && scaleFunction ? scaleFunction : returnScale;
        ...
        for (var i = 0; i < curve.length; i++) {
          ...
          for (var p = 0; p < shape.length; p++) {
            ...
            var planed = tangents[i]
              .scale(shape[p].z)
              .add(normals[i].scale(shape[p].x))
              .add(binormals[i].scale(shape[p].y));
      ...
      var extrudedGeneric = MeshBuilder.CreateRibbon(
        name,
        {
          pathArray: pathArray,
          closeArray: rbCA,
