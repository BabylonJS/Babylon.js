module BABYLON {
    ...
    export class MeshBuilder {
        ...
        private static _UpdateUVForHandedness(scene: Nullable<Scene>, vertexData: VertexData): void {
            const hostScene = scene || Engine.LastCreatedScene;

            if (hostScene && hostScene.useRightHandedSystem) {
                if (vertexData.uvs) {
                    for (var index = 0; index < vertexData.uvs.length; index += 2) {
                        vertexData.uvs[index] = 1.0 - vertexData.uvs[index];
                    }
                }
            }
        }
        ...
        public static CreateBox(name: string, options: { size?: number, width?: number, height?: number, depth?: number, faceUV?: Vector4[], faceColors?: Color4[], sideOrientation?: number, frontUVs?: Vector4, backUVs?: Vector4, updatable?: boolean }, scene: Nullable<Scene> = null): Mesh {
            ...
            MeshBuilder._UpdateUVForHandedness(scene, vertexData);
