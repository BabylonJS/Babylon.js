namespace BABYLON {
      ...
      switch (polygonType) {
        ...
        case SPANNING:
          ...
          for (i = 0; i < polygon.vertices.length; i++) {
            ...
            if ((ti | tj) === SPANNING) {
              t =
                (this.w - Vector3.Dot(this.normal, vi.pos)) /
                Vector3.Dot(this.normal, vj.pos.subtract(vi.pos));
  ...
  class Polygon {
    ...
    constructor(vertices: Vertex[], shared: any) {
      ...
      this.shared = shared;
      this.plane = <Plane>Plane.FromPoints(
        vertices[0].pos,
        vertices[1].pos,
        vertices[2].pos
      );
  ...
  export class CSG {
      ...
      for (var sm = 0, sml = subMeshes.length; sm < sml; sm++) {
        for (
          var i = subMeshes[sm].indexStart,
            il = subMeshes[sm].indexCount + subMeshes[sm].indexStart;
          i < il;
          i += 3
        ) {
          vertices = [];
          for (var j = 0; j < 3; j++) {
            var sourceNormal = new Vector3(
              normals[indices[i + j] * 3],
    ...
    public buildMeshGeometry(
      ...
      for (var i = 0, il = polygons.length; i < il; i++) {
        ...
        subMesh_obj = (<any>subMesh_dict)[polygon.shared.meshId][
          polygon.shared.subMeshId
        ];
        ...
        for (var j = 2, jl = polygon.vertices.length; j < jl; j++) {
          ...
          for (var k = 0; k < 3; k++) {
            ...
            subMesh_obj.indexStart = Math.min(
              currentIndex,
      ...
      if (keepSubMeshes) {
        ...
        for (var m in subMesh_dict) {
          ...
          for (var sm in (<any>subMesh_dict)[m]) {
            ...
            materialMaxIndex = Math.max(
              subMesh_obj.materialIndex,
              materialMaxIndex
            );
          }
