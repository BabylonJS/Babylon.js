...
(function (BABYLON) {
    ...
    var SIMDVector3 = (function () {
        ...
        SIMDVector3.TransformCoordinatesFromFloatsToRefSIMD = function (x, y, z, transformation, result) {
            ...
            var m2 = load(m, 8);
            var m3 = load(m, 12);
            var r = add(add(mul(splat(x), m0), mul(splat(y), m1)), add(mul(splat(z), m2), m3));
    ...
    var SIMDMatrix = (function () {
        ...
        SIMDMatrix.prototype.multiplyToArraySIMD = function (other, result, offset) {
            ...
            for (var i = 0; i < 16; i += 4) {
                store(result, i + offset, add(mul(splat(tm[i]), m0), add(mul(splat(tm[i + 1]), m1), add(mul(splat(tm[i + 2]), m2), mul(splat(tm[i + 3]), m3)))));
            }
            return this;
        };
