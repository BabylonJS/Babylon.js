namespace BABYLON {
  export class SceneLoaderProgressEvent {
    ...
    public static FromProgressEvent(
      event: ProgressEvent
    ): SceneLoaderProgressEvent {
      return new SceneLoaderProgressEvent(
  ...
  export interface ISceneLoaderPluginAsync {
    ...
    importMeshAsync: (
      ...
      onProgress?: (event: SceneLoaderProgressEvent) => void,
      onError?: (message: string, exception?: any) => void
    ) => void;
    loadAsync: (
      scene: Scene,
    ...
    loadAssetsAsync: (
      ...
      data: string,
      rootUrl: string,
      onSuccess?: (assets: Nullable<AssetContainer>) => void,
      onProgress?: (event: SceneLoaderProgressEvent) => void,
      onError?: (message: string, exception?: any) => void
    ) => void;
  }
  ...
  export class SceneLoader {
    ...
    private static _getPluginForExtension(
      ...
      Tools.Warn(
        "Unable to find a plugin to load " +
          extension +
          " files. Trying to use .babylon default plugin."
      );
    ...
    private static _getPluginForFilename(
      ...
      var extension = sceneFilename
        .substring(dotPosition, sceneFilename.length)
        .toLowerCase();
    ...
    private static _loadData(
      ...
      onProgress: ((event: SceneLoaderProgressEvent) => void) | undefined,
      onError: (message: string, exception?: any) => void,
      onDispose: () => void,
      ...
      let onDisposeObservable = (plugin as any)
        .onDisposeObservable as Observable<
        ISceneLoaderPlugin | ISceneLoaderPluginAsync
      >;
      ...
      if (rootUrl.indexOf("file:") === -1) {
        ...
        if (fileOrString.name) {
          ...
          request = Tools.ReadFile(
            FilesInput.FilesToLoad[sceneFilename],
            dataCallback,
            onProgress,
            useArrayBuffer
          );
    ...
    // Public functions
    public static GetPluginForExtension(
      extension: string
    ):
      | ISceneLoaderPlugin
      | ISceneLoaderPluginAsync
    ...
    public static ImportMesh(
      ...
      var progressHandler = onProgress
        ? (event: SceneLoaderProgressEvent) => {
            try {
              onProgress(event);
            } catch (e) {
      ...
      return SceneLoader._loadData(
        ...
        (plugin, data, responseURL) => {
          if ((<any>plugin).loadAssets) {
            var syncedPlugin = <ISceneLoaderPlugin>plugin;
            var assetContainer = syncedPlugin.loadAssets(
              scene,
              data,
              rootUrl,
              errorHandler
            ...
            errorHandler(
              "LoadAssetContainer is not supported by this plugin. Plugin did not provide a loadAssets or loadAssetsAsync method."
            );
          }
