namespace BABYLON {
  ...
  export class ActionEvent {
     ...
     * @param evt {Event} The original (browser) event
     */
    public static CreateNewFromSprite(
      source: Sprite,
      scene: Scene,
      evt?: Event,
      additionalData?: any
    ...
    public static CreateNewFromPrimitive(
      prim: any,
      pointerPos: Vector2,
      evt?: Event,
      additionalData?: any
  ...
  export class ActionManager {
    ...
    public get hasPointerTriggers(): boolean {
      for (var index = 0; index < this.actions.length; index++) {
        ...
        if (
          action.trigger >= ActionManager._OnPickTrigger &&
          action.trigger <= ActionManager._OnPointerOutTrigger
        ) {
          return true;
        }
      }
    ...
    public get hasPickTriggers(): boolean {
      for (var index = 0; index < this.actions.length; index++) {
        ...
        if (
          action.trigger >= ActionManager._OnPickTrigger &&
          action.trigger <= ActionManager._OnPickUpTrigger
        ) {
          return true;
        }
      }
    ...
    public static get HasPickTriggers(): boolean {
      for (var t in ActionManager.Triggers) {
        if (ActionManager.Triggers.hasOwnProperty(t)) {
          ...
          if (
            t_int >= ActionManager._OnPickTrigger &&
            t_int <= ActionManager._OnPickUpTrigger
          ) {
            return true;
          }
        }
            ...
            if (
              ...
              if (parameter && parameter !== evt.sourceEvent.keyCode) {
                ...
                if (lowerCase !== evt.sourceEvent.key) {
                  var unicode = evt.sourceEvent.charCode
        ...
        if (triggerOptions && typeof triggerOptions !== "number") {
          if (triggerOptions.parameter instanceof Node) {
            triggerObject.properties.push(
              Action._GetTargetProperty(triggerOptions.parameter)
            );
          } else {
            var parameter = <any>{};
    ...
    public static Parse(
      ...
      if (object === null) scene.actionManager = actionManager;
      else object.actionManager = actionManager;
      ...
      var instanciate = (name: string, params: Array<any>): any => {
        ...
        var newInstance: Object = Object.create(
          Tools.Instantiate("BABYLON." + name).prototype
        );
      ...
      var parseParameter = (
        ...
        if (propertyPath === null) {
          ...
          else return isNaN(floatValue) ? value : floatValue;
        }
        ...
        if (target instanceof Vector3) return Vector3.FromArray(split);

        if (target instanceof Vector4) return Vector4.FromArray(split);

        if (target instanceof Color3) return Color3.FromArray(split);

        if (target instanceof Color4) return Color4.FromArray(split);
      ...
      var traverse = (
        ...
        combineArray: Nullable<Array<Action>> = null
      ) => {
        ...
        // Parameters
        if (parsedAction.type === 2) parameters.push(actionManager);
        else parameters.push(trigger);

        if (combine) {
          var actions = new Array<Action>();
          for (var j = 0; j < parsedAction.combine.length; j++) {
            traverse(
              parsedAction.combine[j],
          ...
          for (var i = 0; i < parsedAction.properties.length; i++) {
            ...
            if (name === "target")
              if (targetType !== null && targetType === "SceneProperties")
                value = target = scene;
              else value = target = scene.getNodeByName(value);
            else if (name === "parent") value = scene.getNodeByName(value);
            else if (name === "sound") value = scene.getSoundByName(value);
        ...
        if (newAction instanceof Condition && condition !== null) {
          ...
          if (action) action.then(nothing);
          else actionManager.registerAction(nothing);
        ...
        if (combineArray === null) {
          if (newAction instanceof Condition) {
            ...
            condition = null;
            if (action) action.then(newAction);
            else actionManager.registerAction(newAction);
          }
        } else {
      ...
      for (var i = 0; i < parsedActions.children.length; i++) {
        ...
        if (trigger.properties.length > 0) {
          ...
          var value =
            trigger.properties[0].targetType === null
              ? param
              : scene.getMeshByName(param);
          ...
          triggerParams = {
            trigger: (<any>ActionManager)[trigger.name],
            parameter: value
          };
        } else triggerParams = (<any>ActionManager)[trigger.name];
