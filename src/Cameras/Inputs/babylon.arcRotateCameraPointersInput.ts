namespace BABYLON {
  export class ArcRotateCameraPointersInput
    ...
    @serialize() public angularSensibilityX = 1000.0;

    @serialize() public angularSensibilityY = 1000.0;

    @serialize() public pinchPrecision = 12.0;

    /**
     * pinchDeltaPercentage will be used instead of pinchPrecision if different from 0.
     * It defines the percentage of current camera.radius to use as delta when pinch zoom is used.
     */
    @serialize() public pinchDeltaPercentage = 0;

    @serialize() public panningSensibility: number = 1000.0;

    @serialize() public multiTouchPanning: boolean = true;

    @serialize() public multiTouchPanAndZoom: boolean = true;
    ...
    public attachControl(element: HTMLElement, noPreventDefault?: boolean) {
      ...
      var cacheSoloPointer: Nullable<{
        x: number;
        y: number;
        pointerId: number;
        type: any;
      }>; // cache pointer object for better perf on camera rotation
      var pointA: Nullable<{
        x: number;
        y: number;
        pointerId: number;
      ...
      var previousMultiTouchPanPosition: {
        ...
        isPaning: boolean;
        isPinching: boolean;
      } = { x: 0, y: 0, isPaning: false, isPinching: false };
          ...
          if (pointA && pointB === null && cacheSoloPointer) {
            if (
              ...
              this.camera.inertialPanningX +=
                -(evt.clientX - cacheSoloPointer.x) / this.panningSensibility;
              this.camera.inertialPanningY +=
                (evt.clientY - cacheSoloPointer.y) / this.panningSensibility;
            } else {
            ...
            if (this.multiTouchPanAndZoom) {
              ...
              if (this.panningSensibility !== 0) {
                ...
                var pointersCenterDistX =
                  pointersCenterX - previousMultiTouchPanPosition.x;
                var pointersCenterDistY =
                  pointersCenterY - previousMultiTouchPanPosition.y;

                previousMultiTouchPanPosition.x = pointersCenterX;
                previousMultiTouchPanPosition.y = pointersCenterY;

                this.camera.inertialPanningX +=
                  -pointersCenterDistX / this.panningSensibility;
                this.camera.inertialPanningY +=
                  pointersCenterDistY / this.panningSensibility;
              }
            } else {
              twoFingerActivityCount++;

              if (
                previousMultiTouchPanPosition.isPinching ||
                (twoFingerActivityCount < 20 &&
                  Math.abs(pinchDistance - initialDistance) >
                ...
                if (
                  cacheSoloPointer &&
                  cacheSoloPointer.pointerId === ed.pointerId &&
                  this.panningSensibility !== 0 &&
                  ...
                  this.camera.inertialPanningX +=
                    -(ed.x - previousMultiTouchPanPosition.x) /
                    this.panningSensibility;
                  this.camera.inertialPanningY +=
                    (ed.y - previousMultiTouchPanPosition.y) /
                    this.panningSensibility;
                }
              }
              ...
              if (
                cacheSoloPointer &&
                cacheSoloPointer.pointerId === evt.pointerId
              ) {
                previousMultiTouchPanPosition.x = ed.x;
                previousMultiTouchPanPosition.y = ed.y;
              }
      ...
      this._observer = this.camera
        ...
        .onPointerObservable.add(
          ...
          PointerEventTypes.POINTERDOWN |
            ...
            PointerEventTypes._POINTERDOUBLETAP
        );
