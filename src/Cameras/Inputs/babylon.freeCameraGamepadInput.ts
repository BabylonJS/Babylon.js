namespace BABYLON {
  export class FreeCameraGamepadInput implements ICameraInput<FreeCamera> {
    ...
    @serialize() public gamepadAngularSensibility = 200;

    @serialize() public gamepadMoveSensibility = 40;
    ...
    detachControl(element: Nullable<HTMLElement>) {
      this.camera
        .getScene()
        .gamepadManager.onGamepadConnectedObservable.remove(
          this._onGamepadConnectedObserver
        );
      this.camera
        .getScene()
        .gamepadManager.onGamepadDisconnectedObservable.remove(
