...
namespace BABYLON {
  ...
  export class WebVRFreeCamera extends FreeCamera implements PoseControlled {
    ...
    constructor(
      ...
      position: Vector3,
      scene: Scene,
      private webVROptions: WebVROptions = {}
    ) {
      ...
      scene.onBeforeCameraRenderObservable.add(camera => {
        if (camera.parent === this && this.rigParenting) {
          this._descendants = this.getDescendants(true, n => {
            ...
            let isController = this.controllers.some(controller => {
    ...
    public dispose(): void {
      this.getEngine().onVRRequestPresentComplete.removeCallback(
        this._onVREnabled
      );
      super.dispose();
    }
    ...
    public detachControl(element: HTMLElement): void {
      this.getScene().gamepadManager.onGamepadConnectedObservable.remove(
    ...
    public _updateCache(ignoreParentClass?: boolean): void {
      if (
        ...
        !this.position.equals(this._cache.position)
      ) {
    ...
    public update() {
      ...
      this._workingMatrix.multiplyToRef(
        this._deviceToWorld,
        this._workingMatrix
      );
      Quaternion.FromRotationMatrixToRef(
        this._workingMatrix,
        this.deviceRotationQuaternion
      );
    ...
    protected _getWebVRViewMatrix(): Matrix {
      //WebVR 1.1
      var viewArray = this._cameraRigParams["left"]
        ? this._cameraRigParams["frameData"].leftViewMatrix
        : this._cameraRigParams["frameData"].rightViewMatrix;
      ...
      parentCamera._worldToDevice.multiplyToRef(
    ...
    protected _getWebVRProjectionMatrix(): Matrix {
      ...
      var projectionArray = this._cameraRigParams["left"]
        ? this._cameraRigParams["frameData"].leftProjectionMatrix
        : this._cameraRigParams["frameData"].rightProjectionMatrix;
    ...
    public initControllers() {
      ...
      this._onGamepadDisconnectedObserver = manager.onGamepadDisconnectedObservable.add(
        gamepad => {
      ...
      this._onGamepadConnectedObserver = manager.onGamepadConnectedObservable.add(
        gamepad => {
          if (gamepad.type === Gamepad.POSE_ENABLED) {
            let webVrController: WebVRController = <WebVRController>gamepad;
            ...
            if (this.webVROptions.controllerMeshes) {
              if (webVrController.defaultModel) {
                ...
                // Load the meshes
                webVrController.initControllerMesh(
                  this.getScene(),
                  loadedMesh => {
                    this.onControllerMeshLoadedObservable.notifyObservers(
                      webVrController
                    );
                    if (this.webVROptions.defaultLightingOnControllers) {
                      if (!this._lightOnControllers) {
                        this._lightOnControllers = new HemisphericLight(
                          "vrControllersLight",
                          new Vector3(0, 1, 0),
                          this.getScene()
                        );
                      }
                      let activateLightOnSubMeshes = function(
                        mesh: AbstractMesh,
                      ...
                      this._lightOnControllers.includedOnlyMeshes.push(
                        loadedMesh
                      );
                      activateLightOnSubMeshes(
                        loadedMesh,
            ...
            if (this.controllers.indexOf(webVrController) === -1) {
              ...
              for (let i = 0; i < this.controllers.length; i++) {
                if (
                  this.controllers[i].controllerType ===
                  PoseEnabledControllerType.VIVE
                ) {
              ...
              if (this.controllers.length >= 2) {
                this.onControllersAttachedObservable.notifyObservers(
                  this.controllers
                );
              }
            }
