namespace BABYLON {
  ...
  export class VRExperienceHelper {
    ...
    constructor(
      ...
      // Parse options
      if (
        webVROptions.createFallbackVRDeviceOrientationFreeCamera === undefined
      ) {
      ...
      // Set non-vr camera
      if (
        webVROptions.createDeviceOrientationCamera ||
        !this._scene.activeCamera
      ) {
        this._deviceOrientationCamera = new DeviceOrientationCamera(
          "deviceOrientationVRHelper",
        ...
        if (this._scene.activeCamera) {
          ...
          if (
            this._scene.activeCamera instanceof TargetCamera &&
            ...
            if (targetCamera.rotationQuaternion) {
              this._deviceOrientationCamera.rotationQuaternion.copyFrom(
                targetCamera.rotationQuaternion
              );
            } else {
              this._deviceOrientationCamera.rotationQuaternion.copyFrom(
                Quaternion.RotationYawPitchRoll(
                  targetCamera.rotation.y,
                  targetCamera.rotation.x,
                  targetCamera.rotation.z
                )
              );
            }
      ...
      this._webVRCamera = new WebVRFreeCamera(
        "WebVRHelper",
        this._position,
        this._scene,
        webVROptions
      ...
      this._onVRDisplayChanged = (eventArgs: IDisplayChangedEventArgs) =>
        this.onVRDisplayChanged(eventArgs);
      ...
      scene
        .getEngine()
        .onVRDisplayChangedObservable.add(this._onVRDisplayChanged);
      scene
        .getEngine()
        .onVRRequestPresentStart.add(this._onVRRequestPresentStart);
      scene
        .getEngine()
        .onVRRequestPresentComplete.add(this._onVRRequestPresentComplete);
      ...
      this._webVRCamera.onControllerMeshLoadedObservable.add(webVRController =>
        this._onDefaultMeshLoaded(webVRController)
      );
      this._scene.gamepadManager.onGamepadConnectedObservable.add(
        this._onNewGamepadConnected
      );
      this._scene.gamepadManager.onGamepadDisconnectedObservable.add(
      ...
      if (!this._fullscreenVRpresenting && this._canvas) {
        ...
        if (!this._useCustomVRButton) {
          this._btnVR.style.top =
            this._canvas.offsetTop + this._canvas.offsetHeight - 70 + "px";
          this._btnVR.style.left =
            this._canvas.offsetLeft + this._canvas.offsetWidth - 100 + "px";
        }
      }
    };
    ...
    private onVrDisplayPresentChange() {
      ...
      if (vrDisplay) {
        ...
        if (wasPresenting && !this._webVRpresenting) this.exitVR();
      } else {
        Tools.Warn(
          "Detected VRDisplayPresentChange on an unknown VRDisplay. Did you can enterVR on the vrExperienceHelper?"
        );
      }
    ...
    private onVRDisplayChanged(eventArgs: IDisplayChangedEventArgs) {
      ...
      this._webVRpresenting =
        eventArgs.vrDisplay && eventArgs.vrDisplay.isPresenting;
    ...
    public enterVR() {
      ...
      if (this._webVRrequesting) return;
      ...
      if (!this._interactionsEnabled) {
        ...
        if (this._rightControllerReady && this._webVRCamera.rightController) {
          this._enableInteractionOnController(
            this._webVRCamera.rightController
          );
        }
    ...
    public enableTeleportation(
      ...
      if (!this._teleportationInitialized) {
        ...
        if (this._leftControllerReady && this._webVRCamera.leftController) {
          this._enableTeleportationOnController(
            this._webVRCamera.leftController
          );
        }
        if (this._rightControllerReady && this._webVRCamera.rightController) {
          this._enableTeleportationOnController(
        ...
        this._passProcessMove = new PassPostProcess(
          "pass",
          1.0,
          this._webVRCamera
        );
          ...
          gamepad.onleftstickchanged(stickValues => {
            if (this._teleportationInitialized && this.teleportationEnabled) {
              ...
              if (
                ...
                (this._leftLaserPointer &&
                  !this._leftLaserPointer.isVisible &&
                  (this._rightLaserPointer &&
                    !this._rightLaserPointer.isVisible))
              ) {
    ...
    private _tryEnableInteractionOnController = (
      ...
      if (webVRController.hand === "left") {
        ...
        if (
          this._interactionsRequested &&
          !this._interactionsEnabledOnLeftController
        ) {
          this._enableInteractionOnController(webVRController);
        }
        if (
          this._teleportationRequested &&
          !this._teleportationEnabledOnLeftController
        ) {
      ...
      if (webVRController.hand === "right") {
        ...
        if (
          this._interactionsRequested &&
          !this._interactionsEnabledOnRightController
        ) {
          this._enableInteractionOnController(webVRController);
        }
        if (
          this._teleportationRequested &&
          !this._teleportationEnabledOnRightController
        ) {
          this._enableTeleportationOnController(webVRController);
        }
      }
    ...
    private _enableInteractionOnController(webVRController: WebVRController) {
      ...
      if (controllerMesh) {
        ...
        for (var i = 0; i < childMeshes.length; i++) {
          if (
            childMeshes[i].name &&
            childMeshes[i].name.indexOf("POINTING_POSE") >= 0
          ) {
            controllerMesh = childMeshes[i];
            break;
          }
        }
        var laserPointer = Mesh.CreateCylinder(
          "laserPointer",
          1,
          0.004,
          0.0002,
        ...
        var laserPointerMaterial = new StandardMaterial(
          "laserPointerMat",
          this._scene
        );
    ...
    private _checkTeleportWithRay(
      stateObject: StickValues,
      webVRController: Nullable<WebVRController> = null
    ) {
      if (!this._teleportationRequestInitiated) {
        ...
        if (
          webVRController == null ||
          (webVRController.hand === "left" &&
            this._leftLaserPointer &&
            this._leftLaserPointer.isVisible) ||
          (webVRController.hand === "right" &&
            this._rightLaserPointer &&
            this._rightLaserPointer.isVisible)
        ) {
          if (
            Math.sqrt(
              stateObject.y * stateObject.y + stateObject.x * stateObject.x
    ...
    private _checkTeleportBackwards(stateObject: StickValues) {
      ...
      if (stateObject.y > this._padSensibilityUp && this._dpadPressed) {
        if (!this._teleportationBackRequestInitiated) {
          ...
          if (
            (<WebVRFreeCamera>this.currentVRCamera).devicePosition &&
            (<WebVRFreeCamera>this.currentVRCamera).deviceRotationQuaternion
          ) {
            rotation = (<WebVRFreeCamera>this.currentVRCamera)
    ...
    private _createGazeTracker() {
      this._gazeTracker = Mesh.CreateTorus(
        "gazeTracker",
        0.0035,
        0.0025,
        20,
    ...
    private _createTeleportationCircles() {
      this._teleportationTarget = Mesh.CreateGround(
        "teleportationTarget",
        2,
        2,
        2,
      ...
      var teleportationCircleMaterial = new StandardMaterial(
        "TextPlaneMaterial",
        this._scene
      );
      ...
      var animationInnerCircle = new Animation(
    ...
    private _rotateCamera(right: boolean) {
      ...
      var target = Quaternion.FromRotationMatrix(
      ...
      var animationPP2 = new Animation(
        "animationPP2",
        "vignetteStretch",
        90,
        Animation.ANIMATIONTYPE_FLOAT,
      ...
      if (hit.pickedPoint) {
        ...
        if (pickNormal) {
          ...
          Vector3.RotationFromAxisToRef(
            ...
            pickNormal,
            axis1,
            this._teleportationTarget.rotation
          );
        }
    ...
    private _teleportCamera(location: Nullable<Vector3> = null) {
      ...
      if (this.webVRCamera.leftCamera) {
        this._workingVector.copyFrom(
          this.webVRCamera.leftCamera.globalPosition
        );
      ...
      this.currentVRCamera.animations = [];
      var animationCameraTeleportation = new Animation(
    ...
    private _castRayAndSelectObject() {
      ...
      if (
        this._leftLaserPointer &&
        this._leftLaserPointer.isVisible &&
        (<any>this.currentVRCamera).leftController
      ) {
        ...
        this._rightLaserPointer &&
        this._rightLaserPointer.isVisible &&
        (<any>this.currentVRCamera).rightController
      ) {
      ...
      if (hit && hit.pickedMesh) {
        ...
        if (
          ...
          if (
            ...
            this.onSelectedMeshUnselected.notifyObservers(
              this._currentMeshSelected
            );
          }
        ...
        if (hit.pickedMesh !== this._currentMeshSelected) {
          if (this.meshSelectionPredicate(hit.pickedMesh)) {
            ...
            try {
              ...
              Tools.Warn(
                "Error in your custom logic onNewMeshSelected: " + err
              );
            }
          } else {
    ...
    public changeLaserColor(color: Color3) {
      if (this._leftLaserPointer && this._leftLaserPointer.material) {
        (<StandardMaterial>this._leftLaserPointer
          .material).emissiveColor = color;
      }
      if (this._rightLaserPointer && this._rightLaserPointer.material) {
        (<StandardMaterial>this._rightLaserPointer
          .material).emissiveColor = color;
      }
    }
    ...
    public dispose() {
      ...
      this._scene
        .getEngine()
        .onVRDisplayChangedObservable.removeCallback(this._onVRDisplayChanged);
      this._scene
        .getEngine()
        .onVRRequestPresentStart.removeCallback(this._onVRRequestPresentStart);
      this._scene
        .getEngine()
        .onVRRequestPresentComplete.removeCallback(
      ...
      this._scene.gamepadManager.onGamepadConnectedObservable.removeCallback(
        this._onNewGamepadConnected
      );
      this._scene.gamepadManager.onGamepadDisconnectedObservable.removeCallback(
        this._onNewGamepadDisconnected
      );
