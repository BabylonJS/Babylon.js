namespace BABYLON {
  export class TargetCamera extends Camera {
    ...
    @serializeAsVector3() public rotation = new Vector3(0, 0, 0);

    public rotationQuaternion: Quaternion;

    @serialize() public speed = 2.0;

    public noRotationConstraint = false;

    @serializeAsMeshReference("lockedTargetId") public lockedTarget: any = null;
    ...
    public _initCache() {
      super._initCache();
      this._cache.lockedTarget = new Vector3(
        Number.MAX_VALUE,
        Number.MAX_VALUE,
        Number.MAX_VALUE
      );
      this._cache.rotation = new Vector3(
        Number.MAX_VALUE,
        Number.MAX_VALUE,
        Number.MAX_VALUE
      );
      this._cache.rotationQuaternion = new Quaternion(
        Number.MAX_VALUE,
        Number.MAX_VALUE,
        Number.MAX_VALUE,
        Number.MAX_VALUE
    ...
    public _isSynchronizedViewMatrix(): boolean {
      ...
      return (
        (this._cache.lockedTarget
          ? this._cache.lockedTarget.equals(lockedTargetPosition)
          : !lockedTargetPosition) &&
        (this.rotationQuaternion
          ? this.rotationQuaternion.equals(this._cache.rotationQuaternion)
    ...
    public _computeLocalCameraSpeed(): number {
      ...
      return (
        this.speed *
        Math.sqrt(engine.getDeltaTime() / (engine.getFps() * 100.0))
      );
    }
    ...
    public _getViewMatrix(): Matrix {
      ...
      Vector3.TransformCoordinatesToRef(
        ...
        this._cameraRotationMatrix,
        this._transformedReferencePoint
      );

      // Computing target and final matrix
      this.position.addToRef(
        this._transformedReferencePoint,
        this._currentTarget
      );
     ...
     */
    public createRigCamera(
      name: string,
      cameraIndex: number
    ): Nullable<Camera> {
      if (this.cameraRigMode !== Camera.RIG_MODE_NONE) {
        var rigCamera = new TargetCamera(
          name,
          this.position.clone(),
          this.getScene()
        );
        if (
          this.cameraRigMode === Camera.RIG_MODE_VR ||
          this.cameraRigMode === Camera.RIG_MODE_WEBVR
        ) {
          if (!this.rotationQuaternion) {
        ...
        case Camera.RIG_MODE_STEREOSCOPIC_OVERUNDER:
          ...
          var rightSign =
            ...
            Camera.RIG_MODE_STEREOSCOPIC_SIDEBYSIDE_CROSSEYED
              ? -1
              : 1;
          this._getRigCamPosition(
            this._cameraRigParams.stereoHalfAngle * leftSign,
            camLeft.position
          );
          this._getRigCamPosition(
            this._cameraRigParams.stereoHalfAngle * rightSign,
            camRight.position
          );
    ...
    private _getRigCamPosition(halfSpace: number, result: Vector3) {
      ...
      Matrix.Translation(-target.x, -target.y, -target.z).multiplyToRef(
        Matrix.RotationY(halfSpace),
        this._rigCamTransformMatrix
      );

      this._rigCamTransformMatrix = this._rigCamTransformMatrix.multiply(
