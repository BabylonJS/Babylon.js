namespace BABYLON {
  export class Camera extends Node {
    ...
    // Members
    @serializeAsVector3() public position: Vector3;

    @serializeAsVector3() public upVector = Vector3.Up();

    @serialize() public orthoLeft: Nullable<number> = null;

    @serialize() public orthoRight: Nullable<number> = null;

    @serialize() public orthoBottom: Nullable<number> = null;

    @serialize() public orthoTop: Nullable<number> = null;

    /**
     * default : 0.8
     * FOV is set in Radians.
     */
    @serialize() public fov = 0.8;

    @serialize() public minZ = 1;

    @serialize() public maxZ = 10000.0;

    @serialize() public inertia = 0.9;

    @serialize() public mode = Camera.PERSPECTIVE_CAMERA;
     ...
     */
    @serialize() public layerMask: number = 0x0fffffff;

    /**
     * default : FOVMODE_VERTICAL_FIXED
     * fovMode sets the camera frustum bounds to the viewport bounds.
     */
    @serialize() public fovMode: number = Camera.FOVMODE_VERTICAL_FIXED;

    // Camera rig members
    @serialize() public cameraRigMode = Camera.RIG_MODE_NONE;

    @serialize() public interaxialDistance: number;

    @serialize() public isStereoscopicSideBySide: boolean;
    ...
    public _initCache() {
      ...
      this._cache.position = new Vector3(
        Number.MAX_VALUE,
        Number.MAX_VALUE,
        Number.MAX_VALUE
      );
      this._cache.upVector = new Vector3(
        Number.MAX_VALUE,
        Number.MAX_VALUE,
        Number.MAX_VALUE
      );
    ...
    public _isSynchronized(): boolean {
      return (
        this._isSynchronizedViewMatrix() &&
        this._isSynchronizedProjectionMatrix()
      );
    }
    ...
    private _cascadePostProcessesToRigCams(): void {
      ...
      for (var i = 0, len = this._rigCameras.length; i < len; i++) {
        ...
        if (rigPostProcess) {
          ...
          cam._postProcesses = this._postProcesses
            .slice(0)
            .concat(rigPostProcess);
    ...
    public attachPostProcess(
      postProcess: PostProcess,
      insertAt: Nullable<number> = null
    ): number {
      if (
        !postProcess.isReusable() &&
        this._postProcesses.indexOf(postProcess) > -1
      ) {
        Tools.Error(
    ...
    public getViewMatrix(force?: boolean): Matrix {
      ...
      if (!this.parent || !this.parent.getWorldMatrix) {
        ...
        this._globalPosition.copyFromFloats(
          this._computedViewMatrix.m[12],
          this._computedViewMatrix.m[13],
          this._computedViewMatrix.m[14]
        );
      ...
      if (this._cameraRigParams && this._cameraRigParams.vrPreViewMatrix) {
        this._computedViewMatrix.multiplyToRef(
          this._cameraRigParams.vrPreViewMatrix,
          this._computedViewMatrix
        );
      }
    ...
    public getProjectionMatrix(force?: boolean): Matrix {
      if (
        this._doNotComputeProjectionMatrix ||
        (!force && this._isSynchronizedProjectionMatrix())
      ) {
    ...
    public setCameraRigMode(mode: number, rigParams: any): void {
      ...
      this._cameraRigParams.interaxialDistance =
        rigParams.interaxialDistance || 0.0637;
      this._cameraRigParams.stereoHalfAngle = Tools.ToRadians(
        this._cameraRigParams.interaxialDistance / 0.0637
      );
      ...
      switch (this.cameraRigMode) {
        case Camera.RIG_MODE_STEREOSCOPIC_ANAGLYPH:
          this._rigCameras[0]._rigPostProcess = new PassPostProcess(
            this.name + "_passthru",
            1.0,
            this._rigCameras[0]
          );
          this._rigCameras[1]._rigPostProcess = new AnaglyphPostProcess(
            this.name + "_anaglyph",
            1.0,
            this._rigCameras
          );
        ...
        case Camera.RIG_MODE_STEREOSCOPIC_OVERUNDER:
          ...
          this._rigCameras[1]._rigPostProcess = new StereoscopicInterlacePostProcess(
        ...
        case Camera.RIG_MODE_VR:
          var metrics =
            rigParams.vrCameraMetrics || VRCameraMetrics.GetDefault();
          ...
          this._rigCameras[0]._cameraRigParams.vrPreViewMatrix =
            metrics.leftPreViewMatrix;
          ...
          this._rigCameras[1]._cameraRigParams.vrPreViewMatrix =
            metrics.rightPreViewMatrix;
          this._rigCameras[1].getProjectionMatrix = this._rigCameras[1]._getVRProjectionMatrix;

          if (metrics.compensateDistortion) {
            this._rigCameras[0]._rigPostProcess = new VRDistortionCorrectionPostProcess(
              "VR_Distort_Compensation_Left",
              this._rigCameras[0],
              false,
              metrics
            );
            this._rigCameras[1]._rigPostProcess = new VRDistortionCorrectionPostProcess(
        ...
        case Camera.RIG_MODE_WEBVR:
          if (rigParams.vrDisplay) {
            ...
            this._rigCameras[0].setCameraRigParameter(
              "frameData",
              rigParams.frameData
            );
            this._rigCameras[0].setCameraRigParameter(
              "parentCamera",
              rigParams.parentCamera
            );
            this._rigCameras[0]._cameraRigParams.vrWorkMatrix = new Matrix();
            ...
            //Right eye
            this._rigCameras[1].viewport = new Viewport(0.5, 0, 0.5, 1.0);
            this._rigCameras[1].setCameraRigParameter(
              "eyeParameters",
              rightEye
            );
    ...
    private _getVRProjectionMatrix(): Matrix {
      ...
      this._cameraRigParams.vrWorkMatrix.multiplyToRef(
        this._cameraRigParams.vrHMatrix,
        this._projectionMatrix
      );
      return this._projectionMatrix;
    }
    ...
    public clone(name: string): Camera {
      return SerializationHelper.Clone(
        Camera.GetConstructorFromName(
          this.getClassName(),
          name,
          this.getScene(),
          this.interaxialDistance,
          this.isStereoscopicSideBySide
        ),
        this
      );
    }
    ...
    static GetConstructorFromName(
      ...
      name: string,
      scene: Scene,
      interaxial_distance: number = 0,
      isStereoscopicSideBySide: boolean = true
    ): () => Camera {
      switch (type) {
        case "ArcRotateCamera":
          return () =>
            new ArcRotateCamera(name, 0, 0, 1.0, Vector3.Zero(), scene);
        ...
        case "VRDeviceOrientationGamepadCamera":
          return () =>
            new VRDeviceOrientationGamepadCamera(name, Vector3.Zero(), scene);
        case "AnaglyphArcRotateCamera":
          return () =>
            new AnaglyphArcRotateCamera(
              name,
              0,
              0,
              1.0,
        ...
        case "StereoscopicArcRotateCamera":
          return () =>
            new StereoscopicArcRotateCamera(
              name,
              0,
              0,
              1.0,
        ...
        case "StereoscopicUniversalCamera":
          return () =>
            new StereoscopicUniversalCamera(
              name,
              Vector3.Zero(),
              interaxial_distance,
              isStereoscopicSideBySide,
    ...
    public static Parse(parsedCamera: any, scene: Scene): Camera {
      ...
      var construct = Camera.GetConstructorFromName(
        type,
        parsedCamera.name,
        scene,
        parsedCamera.interaxial_distance,
        parsedCamera.isStereoscopicSideBySide
      );
      ...
      if (parsedCamera.animations) {
        for (
          ...
          animationIndex < parsedCamera.animations.length;
          animationIndex++
        ) {
      ...
      if (parsedCamera.autoAnimate) {
        scene.beginAnimation(
          camera,
          parsedCamera.autoAnimateFrom,
          parsedCamera.autoAnimateTo,
          parsedCamera.autoAnimateLoop,
          parsedCamera.autoAnimateSpeed || 1.0
        );
      }
