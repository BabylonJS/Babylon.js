namespace BABYLON {
  export class FollowCamera extends TargetCamera {
    @serialize() public radius: number = 12;

    @serialize() public rotationOffset: number = 0;

    @serialize() public heightOffset: number = 4;

    @serialize() public cameraAcceleration: number = 0.05;

    @serialize() public maxCameraSpeed: number = 20;
  ...
  export class ArcFollowCamera extends TargetCamera {
    ...
    private follow(): void {
      ...
      this._cartesianCoordinates.x =
        this.radius * Math.cos(this.alpha) * Math.cos(this.beta);
      this._cartesianCoordinates.y = this.radius * Math.sin(this.beta);
      this._cartesianCoordinates.z =
        this.radius * Math.sin(this.alpha) * Math.cos(this.beta);
