namespace BABYLON {
  export class ArcRotateCamera extends TargetCamera {
    @serialize() public alpha: number;

    @serialize() public beta: number;

    @serialize() public radius: number;

    @serializeAsVector3("target") protected _target: Vector3;
    ...
    @serialize() public inertialAlphaOffset = 0;

    @serialize() public inertialBetaOffset = 0;

    @serialize() public inertialRadiusOffset = 0;

    @serialize() public lowerAlphaLimit: Nullable<number> = null;

    @serialize() public upperAlphaLimit: Nullable<number> = null;

    @serialize() public lowerBetaLimit = 0.01;

    @serialize() public upperBetaLimit = Math.PI;

    @serialize() public lowerRadiusLimit: Nullable<number> = null;

    @serialize() public upperRadiusLimit: Nullable<number> = null;

    @serialize() public inertialPanningX: number = 0;

    @serialize() public inertialPanningY: number = 0;

    @serialize() public pinchToPanMaxDistance: number = 20;

    @serialize() public panningDistanceLimit: Nullable<number> = null;

    @serializeAsVector3() public panningOriginTarget: Vector3 = Vector3.Zero();

    @serialize() public panningInertia = 0.9;

    //-- begin properties for backward compatibility for inputs
    public get angularSensibilityX(): number {
      var pointers = <ArcRotateCameraPointersInput>this.inputs.attached[
        "pointers"
      ];
    ...
    public get keysUp(): number[] {
      var keyboard = <ArcRotateCameraKeyboardMoveInput>this.inputs.attached[
        "keyboard"
      ];
    ...
    public get wheelPrecision(): number {
      var mousewheel = <ArcRotateCameraMouseWheelInput>this.inputs.attached[
        "mousewheel"
      ];
    ...
    @serialize() public zoomOnFactor = 1;

    public targetScreenOffset = Vector2.Zero();

    @serialize() public allowUpsideDown = true;
    ...
    public _initCache(): void {
      super._initCache();
      this._cache._target = new Vector3(
        Number.MAX_VALUE,
        Number.MAX_VALUE,
        Number.MAX_VALUE
      );
    ...
    public _isSynchronizedViewMatrix(): boolean {
      if (!super._isSynchronizedViewMatrix()) return false;
    ...
    public attachControl(
      ...
      useCtrlForPanning: boolean = true,
      panningMouseButton: number = 2
    ): void {
    ...
    public rebuildAnglesAndRadius() {
      ...
      this.alpha = Math.acos(
        radiusv3.x /
          Math.sqrt(Math.pow(radiusv3.x, 2) + Math.pow(radiusv3.z, 2))
      );
    ...
    public setTarget(
      ...
      if ((<any>target).getBoundingInfo) {
        if (toBoundingCenter) {
          this._targetBoundingCenter = (<any>target)
            .getBoundingInfo()
        ...
        if (
          currentTarget &&
          !allowSamePosition &&
          currentTarget.equals(newTarget)
        ) {
          return;
        }
    ...
    protected _onCollisionPositionChange = (
      collisionId: number,
      newPosition: Vector3,
      collidedMesh: Nullable<AbstractMesh> = null
    ) => {
    ...
    public zoomOn(meshes?: AbstractMesh[], doNotUpdateMaxZ = false): void {
      ...
      this.focusOn(
        { min: minMaxVector.min, max: minMaxVector.max, distance: distance },
        doNotUpdateMaxZ
      );
    }
    ...
    public focusOn(
      meshesOrMinMaxVectorAndDistance:
        | AbstractMesh[]
        | { min: Vector3; max: Vector3; distance: number },
    ...
    public createRigCamera(name: string, cameraIndex: number): Camera {
      ...
      var rigCam = new ArcRotateCamera(
        name,
        this.alpha + alphaShift,
        this.beta,
        this.radius,
        this._target,
        this.getScene()
