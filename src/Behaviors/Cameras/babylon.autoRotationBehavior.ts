    ...
    public attach(camera: ArcRotateCamera): void {
      ...
      this._onAfterCheckInputsObserver = camera.onAfterCheckInputsObservable.add(
        () => {
          ...
          let timeToRotation =
            now - this._lastInteractionTime - this._idleRotationWaitTime;
          let scale = Math.max(
            Math.min(timeToRotation / this._idleRotationSpinupTime, 1),
            0
          );
          ...
          if (this._attachedCamera) {
            this._attachedCamera.alpha -=
              this._cameraRotationSpeed * (dt / 1000);
          }
        }
      );
    }
