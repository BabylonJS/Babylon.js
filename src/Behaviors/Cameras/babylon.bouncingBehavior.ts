  ...
  export class BouncingBehavior implements Behavior<ArcRotateCamera> {
    ...
    public set autoTransitionRange(value: boolean) {
      ...
      if (value) {
        ...
        camera.onMeshTargetChangedObservable.remove(
    ...
    private _isRadiusAtLimit(radiusLimit: Nullable<number>): boolean {
      ...
      if (
        this._attachedCamera.radius === radiusLimit &&
        !this._radiusIsAnimating
      ) {
        return true;
      }
      return false;
    ...
    private _applyBoundRadiusAnimation(radiusDelta: number): void {
      ...
      if (!this._radiusBounceTransition) {
        BouncingBehavior.EasingFunction.setEasingMode(
          BouncingBehavior.EasingMode
        );
        this._radiusBounceTransition = Animation.CreateAnimation(
          "radius",
      ...
      let animatable = Animation.TransitionTo(
        "radius",
