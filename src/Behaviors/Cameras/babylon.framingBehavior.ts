namespace BABYLON {
  export class FramingBehavior implements Behavior<ArcRotateCamera> {
    ...
    public attach(camera: ArcRotateCamera): void {
      ...
      this._onPrePointerObservableObserver = scene.onPrePointerObservable.add(
        pointerInfoPre => {
    ...
    public detach(): void {
      ...
      if (this._onPrePointerObservableObserver) {
        scene.onPrePointerObservable.remove(
          this._onPrePointerObservableObserver
        );
      }
    ...
    public zoomOnBoundingInfo(
      ...
      if (!this._vectorTransition) {
        this._vectorTransition = Animation.CreateAnimation(
          "target",
      ...
      if (this._mode === FramingBehavior.FitFrustumSidesMode) {
        let position = this._calculateLowerRadiusFromModelBoundingSphere(
          minimumWorld,
          maximumWorld
        );
        this._attachedCamera.lowerRadiusLimit =
          radiusWorld.length() + this._attachedCamera.minZ;
        radius = position;
      } else if (this._mode === FramingBehavior.IgnoreBoundsSizeMode) {
        radius = this._calculateLowerRadiusFromModelBoundingSphere(
          minimumWorld,
          maximumWorld
        );
      ...
      if (!this._radiusTransition) {
        this._radiusTransition = Animation.CreateAnimation(
          "radius",
    ...
    protected _calculateLowerRadiusFromModelBoundingSphere(
      ...
      let distanceForHorizontalFrustum =
        radius * Math.sqrt(1.0 + 1.0 / (frustumSlope.x * frustumSlope.x));
      let distanceForVerticalFrustum =
      ...
      if (
        ...
        distance =
          distance < camera.lowerRadiusLimit
            ? camera.lowerRadiusLimit
            : distance;
      }
      ...
      if (camera.upperRadiusLimit) {
        distance =
          distance > camera.upperRadiusLimit
    ...
    private _maintainCameraAboveGround(): void {
      ...
      if (
        ...
        this._attachedCamera.beta > limitBeta &&
        ...
        if (!this._betaTransition) {
          this._betaTransition = Animation.CreateAnimation(
            "beta",
            Animation.ANIMATIONTYPE_FLOAT,
            60,
            FramingBehavior.EasingFunction
        ...
        let animatabe = Animation.TransitionTo(
          "beta",
          defaultBeta,
          this._attachedCamera,
          this._attachedCamera.getScene(),
