...
- Improved performance when using the shadow / cascaded shadow generator ([Popov72](https://github.com/Popov72))
...
- Added color/instance color differentiation in shaders for thin instances ([CedricGuillemet](https://github.com/CedricGuillemet))
...
- Spelling of function/variables `xxxByID` renamed to `xxxById` to be consistent over the project. Old `xxxByID` remain as deprecated that forward to the corresponding `xxxById` ([barroij](https://github.com/barroij))
...
- Fix error on XR dispose due to undefined spectator camera ([Alex-MSFT](https://github.com/Alex-MSFT))
...
- Fix clip plane not reset to the right value when using mirrors ([Popov72](https://github.com/Popov72))
...
- Fix issue with WebXR teleportation logic which would cause positional headlocking on teleportation frames ([syntheticmagus](https://github.com/syntheticmagus))
...
- Fix transmission mask being accidentally used in glTF volume materials ([MiiBond](https://github.com/MiiBond/))
...
- Fix for namespace sharing in .scss files; PropertyTab, SceneExplorer ([msDestiny14](https://github.com/msDestiny14))
