...
// I wrote this section using iterators then I realised that IE doesn't support them
// The iterator version is preserved for future generations who don't have to
// deal with IE.
/*
function *prependToIterator<A>(obj: A, iterator: IterableIterator<A>): IterableIterator<A> {
    yield obj;
    yield* iterator;
}
*/
...
export class HistoryDataSource {
    ...
    saveRevision(isMilestone: boolean, frag: string, vert: string, err?: string): number {
        ...
        let revsIterator = this.revisions();
        const nextRev = revsIterator.next();
        if (!nextRev.done) {
            if (this.revisionsAreExactlySame(newRev, nextRev.value)) {
                return 0; // no redraw
            }

            // If we can NOT elide then reset the iterator so that it includes
            // the first item. Otherwise leave it.
            if (!this.canElide(newRev, nextRev.value)) {
                revsIterator = this.revisions(); // reset
                redrawAmount = 2; // full redraw
            }
        }
        const serialized = this.serialize(prependToIterator(newRev, revsIterator));
        */
    ...
    // revisions(): IterableIterator<HistoryRevision>
    ...
    // serialize(revisions: IterableIteartor<HistoryRevision>, stopIfFirstWasDropped = false): string
...
// Feature idea: caps lock + select text to return selected expression as a kind
// of preview.
...
function renderDateString(ts: number) {
    ...
    if (dY !== 0) return `${dY} year`; // Different years
    else if (dM !== 0) return `${dM} month`; // Different months
    else if (dD !== 0) return `${dD} day`; // Different days
    else if (dh !== 0) return `${dh} hour`; // Different hours
    else if (dm !== 0) return `${dm} min`; // Different hours
