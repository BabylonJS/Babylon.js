module BABYLON {
    export class Gamepads {
        ...
        // All ids of connected gamepad. Each gamepad has a unique ID, as described <a href="https://developer.mozilla.org/en-US/docs/Web/Guide/API/Gamepad"> here </a>
        private rawGamepadIds: Array<string> = [];

        private oneGamepadConnected: boolean = false;

        private isMonitoring: boolean = false;
        private gamepadEventSupported: boolean = 'ongamepadconnected' in window;
        ...
        constructor(ongamedpadconnected: (gamepad: Gamepad) => void, ongamepaddisconnected?: (gamepad: Gamepad) => void) {
            this._callbackGamepadConnected = ongamedpadconnected;
            this._callbackGamepadDisconnected = ongamepaddisconnected;
        ...
        private _updateGamepadObjects() {
            ...
            var nbBrowserGamepad = 0;
            for (var i = 0; i < gamepads.length; i++) {
                if (gamepads[i]) {
                    nbBrowserGamepad++;
                    if (this.rawGamepadIds.indexOf(gamepads[i].id) === -1) {
            ...
            if (this.babylonGamepads.length != nbBrowserGamepad) {
                for (var i = 0; i < this.babylonGamepads.length; i++) {
                    var bgp = this.babylonGamepads[i];
                    var found = false;
                    for (var j = 0; j < gamepads.length; j++) {
                        if (gamepads[j]) {
                            if (bgp.browserGamepad.id === gamepads[j].id) {
                                found = true;
                                break;
                            }
                        }
                    ...
                    if (!found) {
                        ...
                        this.rawGamepadIds.splice(i, 1);
                        if (this._callbackGamepadDisconnected) {
                            this._callbackGamepadDisconnected(bgp);
                        }
                        break;
                    }
                }
